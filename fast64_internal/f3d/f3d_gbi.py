...
class F3D:
    def __init__(self, F3D_VER, _HW_VERSION_1):
        ...
        self.G_CC_MODULATEIDECALA_PRIM = (
            "TEXEL0",
            "0",
            "PRIMITIVE",
            "0",
    ...
    def CALC_DXT(self, width, b_txl):
        return int(
            ((1 << self.G_TX_DXT_FRAC) + self.TXL2WORDS(width, b_txl) - 1)
            / self.TXL2WORDS(width, b_txl)
        )
    ...
    def CALC_DXT_4b(self, width):
        return int(
            ((1 << self.G_TX_DXT_FRAC) + self.TXL2WORDS_4b(width) - 1)
...
class GfxFormatter:
    ...
    def vertexScrollTemplate(
        ...
        if (
            scrollDataFields[0].animType == "None"
            and scrollDataFields[1].animType == "None"
        ):
            return ""
        ...
        for i in range(2):
            ...
            if scrollDataFields[i].animType != "None":
                ...
                scrolling += (
                    "\t\tvertices[i].n.tc[" + str(i) + "] += delta" + field + ";\n"
                )
                ...
                if scrollDataFields[i].animType == "Linear":
                    ...
                    raise PluginError(
                        "Unhandled scroll type: " + str(scrollDataFields[i].animType)
                    )
    ...
    def tileScrollStaticMaterialToC(self, fMaterial):
        ...
        if (
            tile_scroll_tex0.s
            or tile_scroll_tex0.t
            or tile_scroll_tex1.s
            or tile_scroll_tex1.t
        ):
...
class VtxList:
    ...
    def set_addr(self, startAddress):
        ...
        print(
            "VtxList " + self.name + ": " + str(startAddress) + ", " + str(self.size())
        )
...
class GfxList:
    ...
    def size_total(self, f3d):
        ...
        for command in self.commands:
            if (
                isinstance(command, SPDisplayList)
                and command.displayList.DLFormat != DLFormat.Static
            ):
...
class FFogData:
    ...
    def __eq__(self, other):
        return tuple(self.position) == tuple(other.position) and tuple(
            self.color
        ) == tuple(other.color)
...
class FModel:
    ...
    def getTextureAndHandleShared(self, imageKey):
        ...
        if imageKey in self.textures:
            ...
            fPalette = (
                self.textures[fImage.paletteKey]
        ...
        if self.parentModel is not None:
            ...
            if imageKey in self.parentModel.textures:
                ...
                fPalette = (
                    self.parentModel.textures[fImage.paletteKey]
                    if fImage.paletteKey is not None
                    else None
                )
                return fImage, fPalette
            ...
            for subModel in self.parentModel.subModels:
                if materialKey in subModel.materials:
                    ...
                    for imageKey in materialItem[0].usedImages:
                        ...
                        if fImage is None:
                            raise PluginError(
                                "Error: If a material exists, its textures should exist too."
                            )
                    ...
                    for lightName in materialItem[0].usedLights:
                        ...
                        if light is None:
                            raise PluginError(
                                "Error: If a material exists, its lights should exist too."
                            )
                    return materialItem
        else:
            return None
    ...
    def save_binary(self, romfile, segments):
        ...
        for materialKey, (fMaterial, texDimensions) in self.materials.items():
            if fMaterial.useLargeTextures and (
                fMaterial.saveLargeTextures[0] or fMaterial.saveLargeTextures[0]
            ):
                raise PluginError(
    ...
    def to_c(self, textureExportSettings, gfxFormatter):
        ...
        texData = self.to_c_textures(
            texCSeparate, savePNG, texDir, gfxFormatter.texArrayBitSize
        )
        ...
        self.texturesSavedLastExport = self.save_textures(
            textureExportSettings.exportPath, not savePNG
        )
        self.freePalettes()
    ...
    def save_textures(self, dirpath, largeTexturesOnly):
        ...
        for (image, texInfo), texture in self.textures.items():
            if texInfo[1] == "PAL" or (
                largeTexturesOnly and not texture.isLargeTexture
            ):
                continue
...
class FTexRect(FModel):
    ...
    def to_c(self, savePNG, texDir, gfxFormatter):
        ...
        for info, texture in self.textures.items():
            if savePNG:
                staticData.append(
                    texture.to_c_tex_separate(texDir, gfxFormatter.texArrayBitSize)
                )
            else:
    ...
    def add_material_call(self, fMaterial):
        ...
        if not sameMaterial:
            ...
            if (
                ...
                and lastCommand.displayList == fMaterial.revert
            ):
...
class FTriGroup:
    def __init__(self, name, index, fMaterial):
        ...
        self.triList = GfxList(
            name + "_tri_" + str(index), GfxListTag.Geometry, DLFormat.Static
        )
...
class FScrollData:
    def __init__(self):
        self.fields = [
            [FScrollDataField(), FScrollDataField()],
            [FScrollDataField(), FScrollDataField()],
        ]
        self.dimensions = [0, 0]
...
class FMaterial:
    def __init__(self, name, DLFormat):
        ...
        self.revert = GfxList(
            "mat_revert_" + name, GfxListTag.MaterialRevert, DLFormat.Static
        )
        self.DLFormat = DLFormat
    ...
    def getScrollDataField(self, material, texIndex, fieldIndex):
        ...
        UVanim1 = (
            material.f3d_mat.UVanim1 if material.mat_ver > 3 else material.UVanim_tex1
        )
...
class Lights:
    ...
    def set_addr(self, startAddress):
        ...
        self.startAddress = startAddress
        print(
            "Lights " + self.name + ": " + str(startAddress) + ", " + str(self.size())
        )
        return (startAddress, startAddress + self.size())
...
class FImage:
    ...
    def to_c_tex_separate(self, texPath, texArrayBitSize):
        return self.to_c_helper(
            '#include "' + texPath + self.filename + '"', texArrayBitSize
        )
    ...
    def to_c_data(self, bitsPerValue):
        ...
        code = "".join(
            [
                format(
                    int.from_bytes(
                        self.data[i * bytesPerValue : (i + 1) * bytesPerValue], "big"
                    ),
                    "#0" + str(digits) + "x",
                )
                + ", "
        ...
        if remainderCount > 0:
            ...
            code += format(
                int.from_bytes(self.data[start:end], "big")
                << (8 * (bytesPerValue - remainderCount)),
                "#0" + str(digits) + "x",
            )
...
def gsDma2p(c, adrs, length, idx, ofs):
    words = _SHIFTL(c, 24, 8) | _SHIFTL((length - 1) / 8, 19, 5) | _SHIFTL(
        ofs / 8, 8, 8
    ) | _SHIFTL(idx, 0, 8), int(adrs)
    return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class SPVertex:
    ...
    def to_binary(self, f3d, segments):
        vertPtr = int.from_bytes(
            encodeSegmentedAddr(
                self.vertList.startAddress + self.offset * VTX_SIZE, segments
            ),
            "big",
        )
        ...
        if f3d.F3DEX_GBI_2:
            words = (
                _SHIFTL(f3d.G_VTX, 24, 8)
                | _SHIFTL(self.count, 12, 8)
                | _SHIFTL(self.index + self.count, 1, 7),
                vertPtr,
            )
...
class SPViewport:
    ...
    def to_binary(self, f3d, segments):
        vpPtr = int.from_bytes(
            encodeSegmentedAddr(self.viewport.startAddress, segments), "big"
        )
...
class SPDisplayList:
    ...
    def to_binary(self, f3d, segments):
        dlPtr = int.from_bytes(
            encodeSegmentedAddr(self.displayList.startAddress, segments), "big"
        )
...
def gsImmp3(c, p0, p1, p2):
    words = _SHIFTL((c), 24, 8), (
        _SHIFTL((p0), 16, 16) | _SHIFTL((p1), 8, 8) | _SHIFTL((p2), 0, 8)
    )
    return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class SP1Triangle:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DEX_GBI_2:
            words = (
                _SHIFTL(f3d.G_TRI1, 24, 8)
                | _gsSP1Triangle_w1f(self.v0, self.v1, self.v2, self.flag, f3d),
                0,
            )
        else:
...
class SPLine3D:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DEX_GBI_2:
            words = (
                _SHIFTL(f3d.G_LINE3D, 24, 8)
                | _gsSPLine3D_w1f(self.v0, self.v1, 0, self.flag, f3d),
                0,
            )
        else:
    ...
    def to_sm64_decomp_s(self):
        return (
            "gsSPLine3D " + str(self.v0) + ", " + str(self.v1) + ", " + str(self.flag)
        )
...
class SPLineW3D:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DEX_GBI_2:
            words = (
                _SHIFTL(f3d.G_LINE3D, 24, 8)
                | _gsSPLine3D_w1f(self.v0, self.v1, self.wd, self.flag, f3d),
                0,
            )
        else:
            words = _SHIFTL(f3d.G_LINE3D, 24, 8), _gsSPLine3D_w1f(
...
class SP2Triangles:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DLP_GBI or f3d.F3DEX_GBI:
            words = (
                _SHIFTL(f3d.G_TRI2, 24, 8)
                | _gsSP1Triangle_w1f(self.v00, self.v01, self.v02, self.flag0, f3d)
...
class SPCullDisplayList:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DLP_GBI or f3d.F3DEX_GBI:
            words = _SHIFTL(f3d.G_CULLDL, 24, 8) | _SHIFTL(
                (self.vstart) * 2, 0, 16
            ), _SHIFTL((self.vend) * 2, 0, 16)
        else:
...
class SPClipRatio:
    ...
    def to_binary(self, f3d, segments):
        ...
        shortRatioPos = int.from_bytes(
            (-self.ratio).to_bytes(2, "big", signed=True), "big", signed=False
        )
        shortRatioNeg = int.from_bytes(
            self.ratio.to_bytes(2, "big", signed=True), "big", signed=False
        )
...
class SPModifyVertex:
    ...
    def to_binary(self, f3d, segments):
        if f3d.F3DLP_GBI or f3d.F3DEX_GBI:
            words = (
                _SHIFTL(f3d.G_MODIFYVTX, 24, 8)
                | _SHIFTL((self.where), 16, 8)
                | _SHIFTL((self.vtx) * 2, 0, 16),
                self.val,
            )
            return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
        else:
            return gsMoveWd(
                f3d.G_MW_POINTS, (self.vtx) * 40 + (self.where), self.val, f3d
            )
    ...
    def to_c(self, static=True):
        ...
        return (
            header + str(self.vtx) + ", " + str(self.where) + ", " + str(self.val) + ")"
        )
...
class SPBranchLessZraw:
    ...
    def to_binary(self, f3d, segments):
        dlPtr = int.from_bytes(
            encodeSegmentedAddr(self.dl.startAddress, segments), "big"
        )

        words0 = _SHIFTL(f3d.G_RDPHALF_1, 24, 8), dlPtr
        words1 = (
            _SHIFTL(f3d.G_BRANCH_Z, 24, 8)
            | _SHIFTL((self.vtx) * 5, 12, 12)
            | _SHIFTL((self.vtx) * 2, 0, 12),
            self.zval,
        )
...
class SPLightColor:
    ...
    def to_binary(self, f3d, segments):
        return gsMoveWd(
            f3d.G_MW_LIGHTCOL, f3d.getLightMWO_a(self.n), self.col, f3d
        ), +gsMoveWd(f3d.G_MW_LIGHTCOL, f3d.getLightMWO_b(self.n), self.col, f3d)
...
class SPSetLights:
    ...
    def to_binary(self, f3d, segments):
        ...
        if len(self.lights.l) == 0:
            ...
            data += SPLight(self.lights.getLightPointer(0), "LIGHT_1").to_binary(
                f3d, segments
            )
            data += SPLight(self.lights.getAmbientPointer(), "LIGHT_2").to_binary(
                f3d, segments
            )
        else:
            for i in range(len(self.lights.l)):
                data += SPLight(
                    self.lights.getLightPointer(i), "LIGHT_" + str(i + 1)
                ).to_binary(f3d, segments)
            data += SPLight(
                self.lights.getAmbientPointer(), "LIGHT_" + str(len(self.lights.l) + 1)
            ).to_binary(f3d, segments)
        return data
...
class SPLookAt:
    ...
    def to_binary(self, f3d, segments):
        light0Ptr = int.from_bytes(
            encodeSegmentedAddr(self.la.startAddress, segments), "big"
        )
...
class SPFogFactor:
    ...
    def to_binary(self, f3d, segments):
        return gsMoveWd(
            f3d.G_MW_FOG,
            f3d.G_MWO_FOG,
            (_SHIFTL(self.fm, 16, 16) | _SHIFTL(self.fo, 0, 16)),
            f3d,
        )
    ...
    def to_binary(self, f3d, segments):
        return gsMoveWd(
            ...
            (
                ...
                | _SHIFTL(
                    ((500 - (self.minVal)) * 256 / ((self.maxVal) - (self.minVal))),
                    0,
                    16,
                )
            ),
...
class SPGeometryMode:
    ...
    def to_c(self, static=True):
        ...
        data += (
            (" | ".join(self.clearFlagList)) if len(self.clearFlagList) > 0 else "0"
        ) + ", "
        data += (
            (" | ".join(self.setFlagList)) if len(self.setFlagList) > 0 else "0"
        ) + ")"
        return data
...
class DPPipelineMode:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_PIPELINE, 1, modeVal, f3d
        )
...
class DPSetCycleType:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_CYCLETYPE, 2, modeVal, f3d
        )
...
class DPSetTexturePersp:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTPERSP, 1, modeVal, f3d
        )
...
class DPSetTextureDetail:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTDETAIL, 2, modeVal, f3d
        )
...
class DPSetTextureLOD:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTLOD, 1, modeVal, f3d
        )
...
class DPSetTextureLUT:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTLUT, 2, modeVal, f3d
        )
...
class DPSetTextureFilter:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTFILT, 2, modeVal, f3d
        )
...
class DPSetTextureConvert:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_TEXTCONV, 3, modeVal, f3d
        )

    def to_c(self, static=True):
        header = (
            "gsDPSetTextureConvert(" if static else "gDPSetTextureConvert(glistp++, "
        )
        return header + self.mode + ")"
...
class DPSetCombineKey:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_COMBKEY, 1, modeVal, f3d
        )
...
class DPSetColorDither:
    ...
    def to_binary(self, f3d, segments):
        if not f3d._HW_VERSION_1:
            ...
            return gsSPSetOtherMode(
                f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_RGBDITHER, 2, modeVal, f3d
            )
        else:
            if self.mode == "G_CD_ENABLE":
                ...
                modeVal = f3d.G_CD_DISABLE
            return gsSPSetOtherMode(
                f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_COLORDITHER, 1, modeVal, f3d
            )
...
class DPSetAlphaDither:
    ...
    def to_binary(self, f3d, segments):
        if not f3d._HW_VERSION_1:
            ...
            return gsSPSetOtherMode(
                f3d.G_SETOTHERMODE_H, f3d.G_MDSFT_ALPHADITHER, 2, modeVal, f3d
            )
        else:
...
class DPSetAlphaCompare:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_L, f3d.G_MDSFT_ALPHACOMPARE, 2, maskVal, f3d
        )
...
class DPSetDepthSource:
    ...
    def to_binary(self, f3d, segments):
        ...
        return gsSPSetOtherMode(
            f3d.G_SETOTHERMODE_L, f3d.G_MDSFT_ZSRCSEL, 1, srcVal, f3d
        )
...
class DPSetRenderMode:
    ...
    def to_binary(self, f3d, segments):
        ...
        if not self.use_preset:
            return gsSPSetOtherMode(
                f3d.G_SETOTHERMODE_L,
                f3d.G_MDSFT_RENDERMODE,
                29,
                flagWord | self.getGBL_c(f3d),
                f3d,
            )
        else:
            return gsSPSetOtherMode(
                f3d.G_SETOTHERMODE_L, f3d.G_MDSFT_RENDERMODE, 29, flagWord, f3d
            )
    ...
    def to_c(self, static=True):
        ...
        if not self.use_preset:
            ...
            if len(self.flagList) != 2:
                raise PluginError(
                    "For a rendermode preset, only two fields should be used."
                )
...
class DPSetTextureImage:
    ...
    def to_binary(self, f3d, segments):
        ...
        imagePtr = int.from_bytes(
            encodeSegmentedAddr(self.image.startAddress, segments), "big"
        )
...
class DPSetCombineMode:
    # all strings
    def __init__(
        self, a0, b0, c0, d0, Aa0, Ab0, Ac0, Ad0, a1, b1, c1, d1, Aa1, Ab1, Ac1, Ad1
    ):
        self.a0 = a0
        self.b0 = b0
        self.c0 = c0
    ...
    def to_binary(self, f3d, segments):
        words = _SHIFTL(f3d.G_SETCOMBINE, 24, 8) | _SHIFTL(
            GCCc0w0(
                f3d.CCMUXDict[self.a0],
                f3d.CCMUXDict[self.c0],
                f3d.ACMUXDict[self.Aa0],
                f3d.ACMUXDict[self.Ac0],
            )
            | GCCc1w0(f3d.CCMUXDict[self.a1], f3d.CCMUXDict[self.c1]),
            0,
            24,
        ), GCCc0w1(
            f3d.CCMUXDict[self.b0],
            f3d.CCMUXDict[self.d0],
            f3d.ACMUXDict[self.Ab0],
            f3d.ACMUXDict[self.Ad0],
        ) | GCCc1w1(
...
class DPSetPrimDepth:
    ...
    def to_binary(self, f3d, segments):
        return gsDPSetColor(
            f3d.G_SETPRIMDEPTH, _SHIFTL(self.z, 16, 16) | _SHIFTL(self.dz, 0, 16)
        )
...
class DPSetPrimColor:
    ...
    def to_binary(self, f3d, segments):
        words = (
            _SHIFTL(f3d.G_SETPRIMCOLOR, 24, 8)
            | _SHIFTL(self.m, 8, 8)
            | _SHIFTL(self.l, 0, 8)
        ), (
            _SHIFTL(self.r, 24, 8)
...
class DPSetOtherMode:
    ...
    def to_binary(self, f3d, segments):
        words = (
            _SHIFTL(f3d.G_RDPSETOTHERMODE, 24, 8) | _SHIFTL(self.mode0, 0, 24),
            self.mode1,
        )
        return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
def gsDPLoadTileGeneric(c, tile, uls, ult, lrs, lrt):
    words = _SHIFTL(c, 24, 8) | _SHIFTL(uls, 12, 12) | _SHIFTL(ult, 0, 12), _SHIFTL(
        tile, 24, 3
    ) | _SHIFTL(lrs, 12, 12) | _SHIFTL(lrt, 0, 12)
    return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class DPSetTileSize:
    ...
    def to_binary(self, f3d, segments):
        return gsDPLoadTileGeneric(
            f3d.G_SETTILESIZE, self.t, self.uls, self.ult, self.lrs, self.lrt
        )
...
class DPLoadTile:
    ...
    def to_binary(self, f3d, segments):
        return gsDPLoadTileGeneric(
            f3d.G_LOADTILE, self.t, self.uls, self.ult, self.lrs, self.lrt
        )
...
class DPLoadBlock:
    ...
    def to_binary(self, f3d, segments):
        words = (
            _SHIFTL(f3d.G_LOADBLOCK, 24, 8)
            | _SHIFTL(self.uls, 12, 12)
            | _SHIFTL(self.ult, 0, 12)
        ), (
            _SHIFTL(self.tile, 24, 3)
...
class DPLoadTLUTCmd:
    ...
    def to_binary(self, f3d, segments):
        words = _SHIFTL(f3d.G_LOADTLUT, 24, 8), _SHIFTL((self.tile), 24, 3) | _SHIFTL(
            (self.count), 14, 10
        )
        return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class DPLoadTextureBlock:
    def __init__(
        self, timg, fmt, siz, width, height, pal, cms, cmt, masks, maskt, shifts, shiftt
    ):
    ...
    def to_binary(self, f3d, segments):
        return (
            DPSetTextureImage(
                self.fmt, self.siz + "_LOAD_BLOCK", 1, self.timg
            ).to_binary(f3d, segments)
            + DPSetTile(
                self.fmt,
            ...
            + DPSetTile(
                ...
                self.siz,
                (
                    (((self.width) * f3d.G_IM_SIZ_VARS[self.siz + "_LINE_BYTES"]) + 7)
                    >> 3
                ),
                0,
                f3d.G_TX_RENDERTILE,
...
class DPLoadTextureBlockYuv:
    ...
    def to_c(self, static=True):
        header = (
            ...
            if static
            else "gDPLoadTextureBlockYuv(glistp++, "
        )
        return (
            header
            + "&"
...
class _DPLoadTextureBlock:
    ...
    def to_c(self, static=True):
        header = (
            "_gsDPLoadTextureBlock(" if static else "_gDPLoadTextureBlock(glistp++, "
        )
        return (
            header
            + "&"
...
class DPLoadTextureBlock_4b:
    def __init__(
        self, timg, fmt, width, height, pal, cms, cmt, masks, maskt, shifts, shiftt
    ):
        self.timg = timg  # FImage object
        self.fmt = fmt  # string
        self.width = width
    ...
    def to_binary(self, f3d, segments):
        return (
            DPSetTextureImage(self.fmt, "G_IM_SIZ_16b", 1, self.timg).to_binary(
                f3d, segments
            )
            + DPSetTile(
                self.fmt,
    ...
    def to_c(self, static=True):
        header = (
            ...
            if static
            else "gDPLoadTextureBlock_4b(glistp++, "
        )
        return (
            header
            + "&"
...
class DPLoadTextureTile:
    ...
    def to_binary(self, f3d, segments):
        return (
            DPSetTextureImage(self.fmt, self.siz, self.width, self.timg).to_binary(
                f3d, segments
            )
            + DPSetTile(
                self.fmt,
                ...
                (
                    (
                        (self.lrs - self.uls + 1)
                        * f3d.G_IM_SIZ_VARS[self.siz + "_TILE_BYTES"]
                        + 7
                    )
                    >> 3
                ),
            ...
            + DPSetTile(
                ...
                (
                    (
                        (
                            (self.lrs - self.uls + 1)
                            * f3d.G_IM_SIZ_VARS[self.siz + "_LINE_BYTES"]
                        )
                        + 7
                    )
                    >> 3
...
class DPLoadTextureTile_4b:
    ...
    def to_binary(self, f3d, segments):
        return (
            DPSetTextureImage(
                self.fmt, "G_IM_SIZ_8b", self.width >> 1, self.timg
            ).to_binary(f3d, segments)
            + DPSetTile(
                self.fmt,
                "G_IM_SIZ_8b",
    ...
    def to_c(self, static=True):
        header = (
            "gsDPLoadTextureTile_4b(" if static else "gDPLoadTextureTile_4b(glistp++, "
        )
        return (
            header
            + "&"
...
class DPLoadTLUT_pal16:
    ...
    def to_binary(self, f3d, segments):
        if not f3d._HW_VERSION_1:
            return (
                DPSetTextureImage(
                    "G_IM_FMT_RGBA", "G_IM_SIZ_16b", 1, self.dram
                ).to_binary(f3d, segments)
    ...
    def to_binary(self, f3d, segments):
        if not f3d._HW_VERSION_1:
            return (
                ...
                + DPTileSync().to_binary(f3d, segments)
                + DPSetTile(
                    "0", "0", 0, 256, f3d.G_TX_LOADTILE, 0, 0, 0, 0, 0, 0, 0
                ).to_binary(f3d, segments)
                + DPLoadSync().to_binary(f3d, segments)
    ...
    def to_binary(self, f3d, segments):
        if not f3d._HW_VERSION_1:
            return (
                ...
                + DPTileSync().to_binary(f3d, segments)
                + DPSetTile(
                    "0", "0", 0, self.tmemaddr, f3d.G_TX_LOADTILE, 0, 0, 0, 0, 0, 0, 0
                ).to_binary(f3d, segments)
                + DPLoadSync().to_binary(f3d, segments)
...
class DPSetConvert:
    ...
    def to_binary(self, f3d, segments):
        words = (
            _SHIFTL(f3d.G_SETCONVERT, 24, 8)
            | _SHIFTL(self.k0, 13, 9)
            | _SHIFTL(self.k1, 4, 9)
            | _SHIFTL(self.k2, 5, 4)
        ), (
            _SHIFTL(self.k2, 27, 5)
            | _SHIFTL(self.k3, 18, 9)
            | _SHIFTL(self.k4, 9, 9)
            | _SHIFTL(self.k5, 0, 9)
        )
        return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class DPSetKeyR:
    ...
    def to_binary(self, f3d, segments):
        words = _SHIFTL(f3d.G_SETKEYR, 24, 8), _SHIFTL(self.wR, 16, 12) | _SHIFTL(
            self.cR, 8, 8
        ) | _SHIFTL(self.sR, 0, 8)
        return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class DPSetKeyGB:
    ...
    def to_binary(self, f3d, segments):
        words = (
            _SHIFTL(f3d.G_SETKEYGB, 24, 8)
            | _SHIFTL(self.wG, 12, 12)
            | _SHIFTL(self.wB, 0, 12)
        ), (
            _SHIFTL(self.cG, 24, 8)
            | _SHIFTL(self.sG, 16, 8)
            | _SHIFTL(self.cB, 8, 8)
            | _SHIFTL(self.sB, 0, 8)
        )
        return words[0].to_bytes(4, "big") + words[1].to_bytes(4, "big")
...
class SPTextureRectangle:
    ...
    def to_binary(self, f3d, segments):
        words = (
            (
                _SHIFTL(f3d.G_TEXRECT, 24, 8)
                | _SHIFTL(self.xh, 12, 12)
                | _SHIFTL(self.yh, 0, 12)
            ),
            (
                _SHIFTL(self.tile, 24, 3)
                | _SHIFTL(self.xl, 12, 12)
                | _SHIFTL(self.yl, 0, 12)
            ),
            gsImmp1(
                f3d.G_RDPHALF_1, (_SHIFTL(self.s, 16, 16) | _SHIFTL(self.t, 0, 16))
            ),
            gsImmp1(
                f3d.G_RDPHALF_2,
                (_SHIFTL(self.dsdx, 16, 16) | _SHIFTL(self.dtdy, 0, 16)),
            ),
        )
