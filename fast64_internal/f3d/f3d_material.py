...
def update_node_combiner(material: bpy.types.Material, combinerInputs, cycleIndex):
    nodes = material.node_tree.nodes

    if cycleIndex == 1:
        cycle_node: bpy.types.Node = nodes["Cycle_1"]
    else:
        cycle_node: bpy.types.Node = nodes["Cycle_2"]
        is_one_cycle = material.f3d_mat.rdp_settings.g_mdsft_cycletype == "G_CYC_1CYCLE"
        cycle_node.mute = is_one_cycle
...
def set_texture_nodes_settings(
    ...
    node3point_key = '3 Point Lerp' if texIndex == 0 else '3 Point Lerp.001'
    tpnode = node_tree.nodes.get(node3point_key)

    if texProperty.tex is None:
        for texNode in iter_tex_nodes(node_tree, texIndex):
            texNode.mute = True
        if tpnode and not tpnode.mute:
            tpnode.mute = True
        return texSize
    
    mute_3point =  f3dMat.rdp_settings.g_mdsft_text_filt != "G_TF_BILERP"
    if tpnode and tpnode.mute != mute_3point:
        tpnode.mute = mute_3point
