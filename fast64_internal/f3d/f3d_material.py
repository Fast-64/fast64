...
def update_draw_layer(self, context):
    if hasattr(context, "material_slot") and context.material_slot is not None:
        material = (
            context.material_slot.material
        )  # Handles case of texture property groups
    ...
    if material.mat_ver > 3:
        ...
        if context.scene.gameEditorMode == "OOT":
            if drawLayer.oot == "Opaque" or drawLayer.oot == "Overlay":
                ...
                if (
                    not f3dMat.rdp_settings.rendermode_advanced_enabled
                    and "TEX_EDGE" not in f3dMat.rdp_settings.rendermode_preset_cycle_1
                    and (
                        "TEX_EDGE" not in f3dMat.rdp_settings.rendermode_preset_cycle_2
                        or f3dMat.rdp_settings.g_mdsft_cycletype != "G_CYC_2CYCLE"
                    )
                ):
...
class DrawLayerProperty(bpy.types.PropertyGroup):
    sm64: bpy.props.EnumProperty(
        items=sm64EnumDrawLayers, default="1", update=update_draw_layer
    )
    oot: bpy.props.EnumProperty(
        items=ootEnumDrawLayers, default="Opaque", update=update_draw_layer
    )
...
def combiner_uses(material, checkList, is2Cycle):
    ...
    for value in checkList:
        if value[:5] == "TEXEL":
            value1 = value
            value2 = (
                value.replace("0", "1") if "0" in value else value.replace("1", "0")
            )
        else:
            value1 = value
            value2 = value
...
def ui_geo_mode(settings, dataHolder, layout, useDropdown):
    ...
    if not useDropdown or dataHolder.menu_geo:
        ...
        if (
            bpy.context.scene.f3d_type == "F3DEX_GBI_2"
            or bpy.context.scene.f3d_type == "F3DEX_GBI"
        ):
...
def ui_upper_mode(settings, dataHolder, layout, useDropdown):
    ...
    if not useDropdown or dataHolder.menu_upper:
        ...
        prop_split(
            inputGroup, settings, "g_mdsft_textpersp", "Texture Perspective Correction"
        )
        prop_split(inputGroup, settings, "g_mdsft_cycletype", "Cycle Type")

        prop_split(
            inputGroup, settings, "g_mdsft_pipeline", "Pipeline Span Buffer Coherency"
        )
...
def ui_lower_mode(settings, dataHolder, layout: bpy.types.UILayout, useDropdown):
    ...
    if settings.g_mdsft_zsrcsel == "G_ZS_PRIM":
        ...
        if settings.prim_depth.dz != 0 and settings.prim_depth.dz & (
            settings.prim_depth.dz - 1
        ):
            prim_box.label(
                text="Warning: DZ should ideally be a power of 2 up to 0x4000",
                icon="TEXTURE_DATA",
            )
...
def ui_other(settings, dataHolder, layout, useDropdown):
    ...
    if not useDropdown or dataHolder.menu_other:
        ...
        if isinstance(dataHolder, bpy.types.Material) or isinstance(
            dataHolder, F3DMaterialProperty
        ):
...
def tmemUsageUI(layout, textureProp):
    ...
    if tex is not None and tex.size[0] > 0 and tex.size[1] > 0:
        tmemUsage = (
            getTmemWordUsage(textureProp.tex_format, tex.size[0], tex.size[1]) * 8
        )
        tmemMax = getTmemMax(textureProp.tex_format)
        layout.label(
            text="TMEM Usage: " + str(tmemUsage) + " / " + str(tmemMax) + " bytes"
        )
        if tmemUsage > tmemMax:
            tmemSizeWarning = layout.box()
            tmemSizeWarning.label(text="WARNING: Texture size is too large.")
            tmemSizeWarning.label(
                text="Note that width will be internally padded to 64 bit boundaries."
            )
...
class F3DPanel(bpy.types.Panel):
    ...
    def ui_image(self, material, layout, textureProp, name, showCheckBox):
        ...
        if textureProp.menu:
            ...
            if textureProp.use_tex_reference:
                prop_split(
                    prop_input, textureProp, "tex_reference", "Texture Reference"
                )
                prop_split(
                    prop_input, textureProp, "tex_reference_size", "Texture Size"
                )
                if textureProp.tex_format[:2] == "CI":
                    prop_split(
                        prop_input, textureProp, "pal_reference", "Palette Reference"
                    )
                    prop_split(
                        prop_input, textureProp, "pal_reference_size", "Palette Size"
                    )
                ...
                if tex is not None:
                    prop_input.label(
                        text="Size: " + str(tex.size[0]) + " x " + str(tex.size[1])
                    )
            ...
            if material.mat_ver > 3 and material.f3d_mat.use_large_textures:
                ...
                if not textureProp.save_large_texture:
                    prop_input.label(
                        text="Most large textures will take forever to convert.",
                        icon="PREVIEW_RANGE",
                    )
            else:
            ...
            if not (material.mat_ver > 3 and material.f3d_mat.use_large_textures):
                ...
                prop_input.prop(
                    textureProp, "autoprop", text="Auto Set Other Properties"
                )
                ...
                if (
                    ...
                    and tex.size[1] > 0
                    and (
                        math.log(tex.size[0], 2) % 1 > 0.000001
                        or math.log(tex.size[1], 2) % 1 > 0.000001
                    )
                ):
                    warnBox = layout.box()
                    warnBox.label(
    ...
    def ui_prim(self, material, layout, setName, setProp, showCheckBox):
        ...
        if material.mat_ver == 4:
            ...
            prop_input.prop(
                nodes["Primitive Color Output"].inputs[0], "default_value", text=""
            )
        else:
            prop_input.prop(
                nodes["Primitive Color"].outputs[0], "default_value", text=""
            )
    ...
    def ui_env(self, material, layout, showCheckBox):
        ...
        if material.mat_ver > 3:
            if showCheckBox:
                prop_input_name.prop(
                    material.f3d_mat, "set_env", text="Environment Color"
                )
            else:
            ...
            prop_input.prop(
                nodes["Environment Color Output"].inputs[0], "default_value", text=""
            )
    ...
    def ui_chroma(self, material, layout, name, setName, setProp, showCheckBox):
        ...
        if material.mat_ver == 4:
            ...
            prop_input.prop(
                nodes["Chroma Key Center"].outputs[0], "default_value", text="Center"
            )
        prop_input.prop(material, "key_scale", text="Scale")
        prop_input.prop(material, "key_width", text="Width")
        if (
            material.key_width[0] > 1
            or material.key_width[1] > 1
            or material.key_width[2] > 1
        ):
            layout.box().label(
                text="NOTE: Keying is disabled for channels with width > 1."
            )
        prop_input.enabled = setProp
        return inputGroup
    ...
    def ui_lights(self, material, layout, name, showCheckBox):
        ...
        if showCheckBox:
            ...
            prop_input_name.label(text=name)
        prop_input_name.enabled = (
            material.rdp_settings.g_lighting and material.rdp_settings.g_shade
        )
        ...
        if material.rdp_settings.g_lighting:
            if material.use_default_lighting:
                ...
                lightSettings.prop(
                    material, "ambient_light_color", text="Ambient Color"
                )

                lightSettings.prop_search(
                    material, "f3d_light1", bpy.data, "lights", text=""
                )
                if material.f3d_light1 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light2", bpy.data, "lights", text=""
                    )
                if material.f3d_light2 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light3", bpy.data, "lights", text=""
                    )
                if material.f3d_light3 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light4", bpy.data, "lights", text=""
                    )
                if material.f3d_light4 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light5", bpy.data, "lights", text=""
                    )
                if material.f3d_light5 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light6", bpy.data, "lights", text=""
                    )
                if material.f3d_light6 is not None:
                    lightSettings.prop_search(
                        material, "f3d_light7", bpy.data, "lights", text=""
                    )
            prop_input.prop(
                material,
                "use_default_lighting",
                text="Use Custom Lighting",
            ...
            prop_input.enabled = (
                material.set_lights
                and material.rdp_settings.g_lighting
                and material.rdp_settings.g_shade
            )
    ...
    def ui_lower_render_mode(self, material, layout, useDropdown):
        ...
        if not useDropdown or material.menu_lower_render:
            inputGroup.prop(
                material.rdp_settings, "set_rendermode", text="Set Render Mode?"
            )

            renderGroup = inputGroup.column()
            renderGroup.prop(
                material.rdp_settings,
            ...
            if not material.rdp_settings.rendermode_advanced_enabled:
                ...
                prop_split(
                    renderGroup, material.rdp_settings, "force_bl", "Force Blending"
                )
    ...
    def ui_uvCheck(self, layout, context):
        if (
            ...
            if uv_layers.active is None or uv_layers.active.name != "UVMap":
                ...
                uvErrorBox.label(
                    text='Warning: This mesh\'s active UV layer is not named "UVMap".'
                )
        ...
        if f3dMat.rdp_settings.g_fog:
            ...
            if f3dMat.set_fog:
                ...
                if f3dMat.use_global_fog:
                    inputGroup.label(
                        text="Only applies to levels (area fog settings).", icon="INFO"
                    )
                else:
    ...
    def drawShadeAlphaNotice(self, layout):
        layout.box().column().label(
            text='There must be a vertex color layer called "Alpha".',
            icon="IMAGE_ALPHA",
        )

    def drawCIMultitextureNotice(self, layout):
        layout.label(
            text="CI textures will break with multitexturing.",
    ...
    def draw_simple(self, f3dMat, material, layout, context):
        ...
        useMultitexture = (
            ...
            and f3dMat.tex0.tex_set
            and f3dMat.tex1.tex_set
        )

        if (
            useMultitexture
            and f3dMat.tex0.tex_format[:2] == "CI"
            or f3dMat.tex1.tex_format[:2] == "CI"
        ):
        ...
        showLightProperty = (
            f3dMat.set_lights
            and f3dMat.rdp_settings.g_lighting
            and f3dMat.rdp_settings.g_shade
        )
    ...
    def draw_full(self, f3dMat, material, layout, context):
        ...
        if menuTab == "Combiner":
            ...
            combinerBox.prop(
                f3dMat, "set_combiner", text="Color Combiner (Color = (A - B) * C + D)"
            )
            combinerCol = combinerBox.row()
    ...
    def draw(self, context):
        ...
        if material.mat_ver > 3:
            ...
            row.operator(
                AddPresetF3D.bl_idname, text="", icon="ZOOM_OUT"
            ).remove_active = True
        else:
            prop_split(layout, material, "f3d_preset", "Preset Material")

        if context.scene.f3d_simple and (
            (material.mat_ver > 3 and f3dMat.presetName != "Custom")
            or (material.mat_ver <= 3 and f3dMat.f3d_preset != "Custom")
        ):
...
def ui_procAnimVecEnum(
    material, procAnimVec, layout, name, vecType, useDropdown, useTex0, useTex1
):
    layout = layout.box()
    box = layout.column()
    if useDropdown:
    ...
    if not useDropdown or procAnimVec.menu:
        ...
        if not yCombined:
            ui_procAnimFieldEnum(
                procAnimVec.x, box, vecType[0], "UV" if xCombined else None
            )
        if not xCombined:
            ui_procAnimFieldEnum(
                procAnimVec.y, box, vecType[1], "UV" if yCombined else None
            )
        if len(vecType) > 2:
...
def ui_procAnimFieldEnum(procAnimField, layout, name, overrideName):
    box = layout
    box.prop(
        procAnimField, "animType", text=name if overrideName is None else overrideName
    )
        ...
        if i in range(0, 4):
            if combinerInputs[i] == "COMBINED" or combinerInputs[i] == "COMBINED_ALPHA":
                ...
                if cycleIndex == 2 and combinerInputs[i][:5] == "TEXEL":
                    ...
                    combinerInput = (
                        value.replace("0", "1")
                        if "0" in value
                        else value.replace("1", "0")
                    )
                else:
                    combinerInput = combinerInputs[i]
                combinerSocket = getSocketFromCombinerToNodeDictColor(
            ...
            if combinerInputs[i] == "COMBINED":
                if cycleIndex == 2:
                    ...
                    combiner2 = nodes["Color Combiner Cycle 2 F3D v3"]
                    material.node_tree.links.new(
                        combiner2.inputs[i], combiner1.outputs[1]
                    )
            else:
                combinerSocket = getSocketFromCombinerToNodeDictAlpha(
                    nodes, f3dVer, combinerInputs[i]
                )
                material.node_tree.links.new(combinerNode.inputs[i], combinerSocket)
...
def update_node_values_of_material(material, context):
    ...
    if material.mat_ver == 1:
        ...
        nodes["Case A 1"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner1.A
        ]
        nodes["Case B 1"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner1.B
        ]
        nodes["Case C 1"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner1.C
        ]
        nodes["Case D 1"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner1.D
        ]
        nodes["Case A Alpha 1"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner1.A_alpha
        ]
        nodes["Case B Alpha 1"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner1.B_alpha
        ]
        nodes["Case C Alpha 1"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner1.C_alpha
        ]
        nodes["Case D Alpha 1"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner1.D_alpha
        ]
        nodes["Case A 2"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner2.A
        ]
        nodes["Case B 2"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner2.B
        ]
        nodes["Case C 2"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner2.C
        ]
        nodes["Case D 2"].outputs[0].default_value = f3dVer.CCMUXDict[
            material.combiner2.D
        ]
        nodes["Case A Alpha 2"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner2.A_alpha
        ]
        nodes["Case B Alpha 2"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner2.B_alpha
        ]
        nodes["Case C Alpha 2"].outputs[0].default_value = f3dVer.ACMUXDict[
            material.combiner2.C_alpha
        ]
        nodes["Case D Alpha 2"].outputs[0].default_value = f3dVer.ACMUXDict[
    ...
    if material.mat_ver >= 3:
        nodes["F3D v3"].inputs[4].default_value = (
            1 if f3dMat.rdp_settings.g_mdsft_cycletype == "G_CYC_2CYCLE" else 0
        )
        ...
        nodes["Get UV 0 F3D v3"].inputs[0].default_value = (
            1 if f3dMat.rdp_settings.g_tex_gen else 0
        )
        nodes["Get UV 0 F3D v3"].inputs[1].default_value = (
            1 if f3dMat.rdp_settings.g_tex_gen_linear else 0
        )
        nodes["Get UV 1 F3D v3"].inputs[0].default_value = (
            1 if f3dMat.rdp_settings.g_tex_gen else 0
        )
        nodes["Get UV 1 F3D v3"].inputs[1].default_value = (
            1 if f3dMat.rdp_settings.g_tex_gen_linear else 0
        )

        nodes["Shade Color"].inputs[0].default_value = (
            0 if not f3dMat.rdp_settings.g_shade else 1
        )
        nodes["Shade Color"].inputs[1].default_value = (
            0 if not f3dMat.rdp_settings.g_lighting else 1
        )
        ...
        nodes["Cycle Type"].outputs[0].default_value = (
            1 if material.rdp_settings.g_mdsft_cycletype == "G_CYC_2CYCLE" else 0
        )
        ...
        nodes["Texture Gen"].outputs[0].default_value = (
            1 if material.rdp_settings.g_tex_gen else 0
        )
        nodes["Texture Gen Linear"].outputs[0].default_value = (
            1 if material.rdp_settings.g_tex_gen_linear else 0
        )
        nodes["Shading"].outputs[0].default_value = (
            0 if not material.rdp_settings.g_shade else 1
        )
        nodes["Lighting"].outputs[0].default_value = (
            0 if not material.rdp_settings.g_lighting else 1
        )
    ...
    nodes["Chroma Key Scale"].outputs[0].default_value = [
        value for value in f3dMat.key_scale
    ] + [1]
    nodes["Primitive LOD Fraction"].outputs[0].default_value = f3dMat.prim_lod_frac
...
def update_tex_values_field(
    ...
    normHalfPixelNode.outputs[0].default_value = 1 / (2 * pixelLength)
    ...
    normMaskNode.outputs[0].default_value = (2 ** mask) / pixelLength if mask > 0 else 0
...
def update_tex_values_field_v2(
    self, fieldProperty, pixelLength, uvBasisScale, scale, autoprop, texIndex, field
):
    ...
    normLNode.inputs[fieldIndex].default_value = (
        fieldProperty.low / pixelLengthAxis * (-1 if field == "T" else 1)
    )
    normHNode.inputs[fieldIndex].default_value = (
        fieldProperty.high + 1
    ) / pixelLengthAxis
    normMaskNode.inputs[fieldIndex].default_value = (
        (2 ** fieldProperty.mask) / pixelLengthAxis if fieldProperty.mask > 0 else 0
    )
    shiftNode.inputs[fieldIndex].default_value = fieldProperty.shift
    scaleNode.inputs[fieldIndex].default_value = (
        scale[fieldIndex] * uvBasisScale[fieldIndex]
    )


def update_tex_values_field_v3(
    self, texProperty, tex_size, uvBasisScale, scale, texIndex
):
    nodes = self.node_tree.nodes
    ...
    nodes["Get UV " + str(texIndex) + " F3D v3"].inputs[4].default_value = (
        ...
        (texProperty.T.high + 1) / tex_size[1],
        0,
    )

    # Clamp
    isTexGen = (
        self.f3d_mat.rdp_settings.g_tex_gen
        or self.f3d_mat.rdp_settings.g_tex_gen_linear
    )
    nodes["Get UV " + str(texIndex) + " F3D v3"].inputs[5].default_value = (
        1 if texProperty.S.clamp and not isTexGen else 0,
        1 if texProperty.T.clamp and not isTexGen else 0,
        0,
    )
    ...
    nodes["Get UV " + str(texIndex) + " F3D v3"].inputs[6].default_value = (
        ...
        (2 ** texProperty.T.mask) / tex_size[1] if texProperty.T.mask > 0 else 0,
        0,
    )
    ...
    nodes["Get UV " + str(texIndex) + " F3D v3"].inputs[7].default_value = (
        ...
        1 if texProperty.T.mirror else 0,
        0,
    )
    ...
    if nodes[texNodeName].image is not None or texProperty.use_tex_reference:
        ...
        if tex_size[0] > 0 and tex_size[1] > 0:
            if self.mat_ver == 1:
                tex_x = (
                    nodes[uvNodeName]
                    .node_tree.nodes["Create Tex Coord"]
                    .node_tree.nodes
                )
                tex_y = (
                    nodes[uvNodeName]
                    .node_tree.nodes["Create Tex Coord.001"]
                    .node_tree.nodes
                )
                imageWidthNode = nodes[uvNodeName].node_tree.nodes["Image Width Factor"]
                imageHeightNode = nodes[uvNodeName].node_tree.nodes[
                    "Image Height Factor"
                ]
                ...
                if self.mat_ver == 3 or hasNodeGraph(self):
                    update_tex_values_field_v3(
                        self, texProperty, tex_size, uvBasisScale, scale, texIndex
                    )
                    # nodes[texNodeName].interpolation = "Closest"
                    nodes[texNodeName].interpolation = (
                        "Closest"
                        if f3dMat.rdp_settings.g_mdsft_text_filt == "G_TF_POINT"
                        else "Linear"
                    )
                else:
                    if texProperty.autoprop:
                ...
                print(
                    "Error: Unhandled material version "
                    + str(self.mat_ver)
            ...
            if self.mat_ver >= 3:  # No nodes, only sockets (0 = color, 1 = alpha)
                if self.mat_ver == 3 or hasNodeGraph(self):
                    ...
                    nodes[getTextureColorName].inputs[2].default_value = (
                        ...
                        if (
                            texFormat[0] == "I"
                            or (texFormat[:2] == "CI" and ciFormat[0] == "I")
                        )
                        else 0
                    )
                    ...
                    nodes[getTextureColorName].inputs[3].default_value = (
                        ...
                        if (
                            "A" in texFormat
                            or (texFormat[:2] == "CI" and "A" in ciFormat)
                        )
                        else 0
                    )
                ...
                nodes[texNodeName + " Has Alpha"].outputs[0].default_value = (
                    1
                    if ("A" in texFormat or (texFormat[:2] == "CI" and "A" in ciFormat))
                    else 0
                )
    ...
    if hasattr(context, "material") and context.material is not None:
        ...
        if material.tex0 == self and material.tex0.tex is not None:
            if isMultiTexture:
                ...
                material.tex0.tex_format = getOptimalFormat(
                    material.tex0.tex, useLargeTextures
                )
        if material.tex1 == self and material.tex1.tex is not None:
            if isMultiTexture:
                material.tex1.tex_format = "RGBA16"
            else:
                material.tex1.tex_format = getOptimalFormat(
                    material.tex1.tex, useLargeTextures
                )
        context.material.f3d_update_flag = False
...
def update_tex_values_manual(self, context):
    ...
    if material.scale_autoprop:
        ...
        if material.tex0.tex is not None and material.tex1.tex is not None:
            tex_size = (
                material.tex0.tex.size
                if material.uv_basis == "TEXEL0"
                else material.tex1.tex.size
            )
        ...
        if isTexGen and tex_size is not None:
            material.tex_scale = ((tex_size[0] - 1) / 1024, (tex_size[1] - 1) / 1024)
        else:
    ...
    if (
        ...
        if material.uv_basis == "TEXEL0":
            uvBasisScale0 = (1, 1)
            uvBasisScale1 = (
                material.tex0.tex.size[0] / material.tex1.tex.size[0],
                material.tex0.tex.size[1] / material.tex1.tex.size[1],
            )
        else:
            uvBasisScale1 = (1, 1)
            uvBasisScale0 = (
                material.tex1.tex.size[0] / material.tex0.tex.size[0],
...
def update_preset(self, context):
    if hasattr(context, "material_slot") and context.material_slot is not None:
        ...
        if material.mat_ver < 4 and material.f3d_preset != "Custom":
            ...
            materialSettings.applyToMaterial(
                material, False, update_node_values_of_material, bpy.context
            )
...
def createF3DMat(obj, preset="Shaded Solid", index=None):
    ...
    if material.mat_ver > 3:
        ...
        if preset in [enumValue[0] for enumValue in enumMaterialPresets]:
            ...
            raise PluginError(
                "Enum '" + preset + "' not found in material preset enum list."
            )
...
def reloadDefaultF3DPresets():
    ...
    for material in bpy.data.materials:
        if material.mat_ver > 3 and material.f3d_mat.presetName in presetNameToFilename:
            update_preset_manual_v4(
                material, presetNameToFilename[material.f3d_mat.presetName]
            )
...
class TextureFieldProperty(bpy.types.PropertyGroup):
    clamp: bpy.props.BoolProperty(name="Clamp", update=update_tex_values)
    mirror: bpy.props.BoolProperty(name="Mirror", update=update_tex_values)
    low: bpy.props.FloatProperty(
        name="Low", min=0, max=1023.75, update=update_tex_values
    )
    high: bpy.props.FloatProperty(
        name="High", min=0, max=1023.75, update=update_tex_values
    )
    mask: bpy.props.IntProperty(min=0, max=15, update=update_tex_values, default=5)
    shift: bpy.props.IntProperty(min=-5, max=10, update=update_tex_values)
...
class TextureProperty(bpy.types.PropertyGroup):
    tex: bpy.props.PointerProperty(
        type=bpy.types.Image, name="Texture", update=update_tex_values_and_formats
    )

    tex_format: bpy.props.EnumProperty(
        name="Format", items=enumTexFormat, default="RGBA16", update=update_tex_values
    )
    ci_format: bpy.props.EnumProperty(
        name="CI Format", items=enumCIFormat, default="RGBA16", update=update_tex_values
    )
    ...
    use_tex_reference: bpy.props.BoolProperty(
        name="Use Texture Reference", default=False, update=update_tex_values
    )
    tex_reference: bpy.props.StringProperty(
        name="Texture Reference", default="0x08000000"
    )
    ...
    pal_reference: bpy.props.StringProperty(
        name="Palette Reference", default="0x08000000"
    )
    pal_reference_size: bpy.props.IntProperty(
        name="Texture Reference Size", min=1, default=16
    )
    ...
    autoprop: bpy.props.BoolProperty(
        name="Autoprop", update=update_tex_values, default=True
    )
    save_large_texture: bpy.props.BoolProperty(
        name="Save Large Texture As PNG", default=True
    )
...
class RDPSettings(bpy.types.PropertyGroup):
    g_zbuffer: bpy.props.BoolProperty(
        name="Z Buffer", default=True, update=update_node_values
    )
    g_shade: bpy.props.BoolProperty(
        name="Shading", default=True, update=update_node_values
    )
    # v1/2 difference
    g_cull_front: bpy.props.BoolProperty(name="Cull Front", update=update_node_values)
    # v1/2 difference
    g_cull_back: bpy.props.BoolProperty(
        name="Cull Back", default=True, update=update_node_values
    )
    g_fog: bpy.props.BoolProperty(name="Fog", update=update_node_values)
    g_lighting: bpy.props.BoolProperty(
        name="Lighting", default=True, update=update_node_values
    )
    g_tex_gen: bpy.props.BoolProperty(
        name="Texture UV Generate", update=update_node_values
    )
    g_tex_gen_linear: bpy.props.BoolProperty(
        name="Texture UV Generate Linear", update=update_node_values
    )
    # v1/2 difference
    g_shade_smooth: bpy.props.BoolProperty(
        name="Smooth Shading", default=True, update=update_node_values
    )
    # f3dlx2 only
    g_clipping: bpy.props.BoolProperty(name="Clipping", update=update_node_values)
    ...
    clip_ratio: bpy.props.IntProperty(
        default=1, min=1, max=2 ** 15 - 1, update=update_node_values
    )
    ...
    cvg_dst: bpy.props.EnumProperty(
        name="Coverage Destination", items=enumCoverage, update=update_node_values
    )
...
class DefaultRDPSettingsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(
            text="If a material setting is a same as a default "
...
def getOptimalFormat(tex, useLargeTextures):
    ...
    if useLargeTextures:
        return "RGBA16"
    if (
        bpy.context.scene.ignoreTextureRestrictions or tex.size[0] * tex.size[1] > 8192
    ):  # Image too big
        return "RGBA32"
    ...
    for j in reversed(range(tex.size[1])):
        for i in range(tex.size[0]):
            ...
            for field in range(tex.channels):
                color[field] = tex.pixels[(j * tex.size[0] + i) * tex.channels + field]
...
class MATERIAL_MT_f3d_presets(Menu):
    ...
    def draw(self, _context):
        ...
        paths = (
            bpy.utils.preset_paths(presetDir)
        ...
        if is_preset_add:
            ...
            try:
                target_path = bpy.utils.user_resource(
                    "SCRIPTS", target_path, create=True
                )
            except:  # 3.0
                target_path = bpy.utils.user_resource(
            ...
            for otherMat in bpy.data.materials:
                if (
                    otherMat.f3d_mat.presetName == presetName
                    and otherMat != context.material
                ):
            ...
            if self.remove_active:
                ...
                if filename in material_presets or filename == "custom":
                    self.report(
                        {"WARNING"}, "Unable to delete/overwrite default presets."
                    )
                    return {"CANCELLED"}
...
class F3DMaterialProperty(bpy.types.PropertyGroup):
    ...
    scale_autoprop: bpy.props.BoolProperty(
        name="Auto Set Scale", default=True, update=update_tex_values
    )
    uv_basis: bpy.props.EnumProperty(
        name="UV Basis", default="TEXEL0", items=enumTexUV, update=update_tex_values
    )
    ...
    tex_scale: bpy.props.FloatVectorProperty(
        min=0, max=1, size=2, default=(1, 1), step=1, update=update_tex_values
    )
    ...
    use_default_lighting: bpy.props.BoolProperty(
        default=True, update=update_node_values_without_preset
    )
    ...
    key_scale: bpy.props.FloatVectorProperty(
        name="Key Scale", min=0, max=1, step=1, update=update_node_values
    )
    key_width: bpy.props.FloatVectorProperty(
        name="Key Width", min=0, max=16, update=update_node_values
    )

    # Convert
    k0: bpy.props.FloatProperty(
        min=-1, max=1, default=175 / 255, step=1, update=update_node_values
    )
    k1: bpy.props.FloatProperty(
        min=-1, max=1, default=-43 / 255, step=1, update=update_node_values
    )
    k2: bpy.props.FloatProperty(
        min=-1, max=1, default=-89 / 255, step=1, update=update_node_values
    )
    k3: bpy.props.FloatProperty(
        min=-1, max=1, default=222 / 255, step=1, update=update_node_values
    )
    k4: bpy.props.FloatProperty(
        min=-1, max=1, default=114 / 255, step=1, update=update_node_values
    )
    k5: bpy.props.FloatProperty(
        min=-1, max=1, default=42 / 255, step=1, update=update_node_values
    )

    # Prim
    prim_lod_frac: bpy.props.FloatProperty(
        name="Prim LOD Frac", min=0, max=1, step=1, update=update_node_values
    )
    prim_lod_min: bpy.props.FloatProperty(
        name="Min LOD Ratio", min=0, max=1, step=1, update=update_node_values
    )
    ...
    f3d_light1: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light2: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light3: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light4: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light5: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light6: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
    f3d_light7: bpy.props.PointerProperty(
        type=bpy.types.Light, update=F3DOrganizeLights
    )
...
class UpdateF3DNodes(bpy.types.Operator):
    ...
    def execute(self, context):
        if (
            context is None
            or not hasattr(context, "material")
            or context.material is None
        ):
...
def mat_register_old():
    bpy.types.Material.f3d_preset = bpy.props.EnumProperty(
        name="F3D Preset",
        items=enumMaterialPresets,
        default="Custom",
        update=update_preset,
    )

    bpy.types.Material.scale_autoprop = bpy.props.BoolProperty(
        name="Auto Set Scale", default=True, update=update_node_values_without_preset
    )
    bpy.types.Material.uv_basis = bpy.props.EnumProperty(
        name="UV Basis", default="TEXEL0", update=update_tex_values, items=enumTexUV
    )

    # Combiners
    bpy.types.Material.presetName = bpy.props.StringProperty(
        name="Preset Name", default="Custom"
    )

    bpy.types.Material.combiner1 = bpy.props.PointerProperty(type=CombinerProperty)
    bpy.types.Material.combiner2 = bpy.props.PointerProperty(type=CombinerProperty)
    ...
    bpy.types.Material.UVanim = bpy.props.PointerProperty(type=ProcAnimVectorProperty)
    ...
    # Should Set?
    bpy.types.Material.set_prim = bpy.props.BoolProperty(
        default=True, update=update_node_values
    )
    bpy.types.Material.set_lights = bpy.props.BoolProperty(
        default=True, update=update_node_values
    )
    bpy.types.Material.set_env = bpy.props.BoolProperty(
        default=False, update=update_node_values
    )
    bpy.types.Material.set_blend = bpy.props.BoolProperty(
        default=False, update=update_node_values
    )
    bpy.types.Material.set_key = bpy.props.BoolProperty(
        default=True, update=update_node_values
    )
    bpy.types.Material.set_k0_5 = bpy.props.BoolProperty(
        default=True, update=update_node_values
    )
    bpy.types.Material.set_combiner = bpy.props.BoolProperty(
        default=True, update=update_node_values
    )
    bpy.types.Material.use_default_lighting = bpy.props.BoolProperty(
        default=True, update=update_node_values_without_preset
    )
    ...
    # Convert
    bpy.types.Material.k0 = bpy.props.FloatProperty(
        min=-1, max=1, default=175 / 255, step=1, update=update_node_values
    )
    bpy.types.Material.k1 = bpy.props.FloatProperty(
        min=-1, max=1, default=-43 / 255, step=1, update=update_node_values
    )
    bpy.types.Material.k2 = bpy.props.FloatProperty(
        min=-1, max=1, default=-89 / 255, step=1, update=update_node_values
    )
    bpy.types.Material.k3 = bpy.props.FloatProperty(
        min=-1, max=1, default=222 / 255, step=1, update=update_node_values
    )
    bpy.types.Material.k4 = bpy.props.FloatProperty(
        min=-1, max=1, default=114 / 255, step=1, update=update_node_values
    )
    bpy.types.Material.k5 = bpy.props.FloatProperty(
        min=-1, max=1, default=42 / 255, step=1, update=update_node_values
    )
    ...
    bpy.types.Material.rdp_settings = bpy.props.PointerProperty(type=RDPSettings)


def findF3DPresetPath(filename):
    try:
        presetPath = bpy.utils.user_resource(
            "SCRIPTS", os.path.join("presets", "f3d"), create=True
        )
    except:  # 3.0
        presetPath = bpy.utils.user_resource(
            "SCRIPTS", path=os.path.join("presets", "f3d"), create=True
        )
...
def getF3DPresetPath(filename, subdir):
    try:
        presetPath = bpy.utils.user_resource(
            "SCRIPTS", os.path.join("presets", subdir), create=True
        )
    except:  # 3.0
        presetPath = bpy.utils.user_resource(
            "SCRIPTS", path=os.path.join("presets", subdir), create=True
        )
...
def mat_register():
    ...
    bpy.types.Scene.f3dUserPresetsOnly = bpy.props.BoolProperty(
        name="User Presets Only"
    )
    bpy.types.Scene.f3d_simple = bpy.props.BoolProperty(
        name="Display Simple", default=True
    )
    ...
    bpy.types.Object.f3d_lod_always_render_farthest = bpy.props.BoolProperty(
        name="Always Render Farthest LOD"
    )
...
sm64_vert_colored_tex_no_vert_alpha.color_combiner = tuple(
...
sm64_vert_colored_tex_transparent.color_combiner = tuple(
...
sm64_fog_shaded_texture_cutout.color_combiner = tuple(S_FOG_SHADED_TEX_CUTOUT)
...
sm64_fog_shaded_texture_transparent.color_combiner = tuple(S_FOG_PRIM_TRANSPARENT_SHADE)
...
enumMaterialPresets = [
    ...
    (
        "Vertex Colored Texture Transparent",
        "Vertex Colored Texture Transparent",
        "Vertex Colored Texture Transparent",
    ),
