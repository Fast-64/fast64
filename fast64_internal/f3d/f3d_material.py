            ...
            if f3dMat.rdp_settings.g_mdsft_textlod == "G_TL_LOD":
                for i in range(1, f3dMat.rdp_settings.num_textures_mipmapped):
                    if i == 1 and useDict["Texture 1"]:
                        continue
                    ui_image(canUseLargeTextures, inputCol, getattr(f3dMat, f"tex{i}"), f"Texture {i}", True)
...
def ui_image(
    ...
    if textureProp.menu:
        ...
        if canUseLargeTextures:
            prop_input_load.label(text="Large texture mode enabled.")
            prop_input_load.label(text="Each triangle must fit in a single tile load.")
            prop_input_load.label(text="UVs must be in the [0, 1024] pixel range.")
        else:
            tmemUsageUI(prop_input_load, textureProp)
