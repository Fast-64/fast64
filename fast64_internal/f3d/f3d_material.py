...
class F3DPanel(Panel):
    ...
    def draw_full(self, f3dMat, material, layout: UILayout, context):
        ...
        if menuTab == "Sources":
            ...
            if useDict["Shade"] and "Lighting" in sources_in_ucode(bpy.context.scene.f3d_type) and not inherit_light_and_fog():
...
def update_fog_nodes(material: Material, context: Context):
    ...
    if shade_alpha_is_fog:
        inherit_fog = f3dMat.use_global_fog or not f3dMat.set_fog or inherit_light_and_fog()
        if inherit_fog:
...
def update_node_values_of_material(material: Material, context):
    ...
    if settings.is_geo_mode_on("g_packed_normals"):
