...
def addNodeListAtWithZeroAddNode(node_tree, nodeDict, x, y, cycleIndex):
    ...
    for label, typename in nodeDict.items():
        ...
        bridge, nextX, nextY = addNodeAt(node_tree, "ShaderNodeMath", name + " Bridge", nextX, y)
...
def createNodeSwitch(node_tree, caseDict, caseSocket, caseName, location, socketDict):
    ...
    if not createGroup:
        ...
        internalSocketDict, nextSocketIndex = socketDictToInternalSocket(
            node_tree, groupNode, None, socketDict, 1, False
        )
        return groupNode
    ...
    for case in reversed(range(len(caseDict))):
        ...
        if case == len(caseDict) - 1:
            ...
            greaterThanNode, mixNodeX, y = addNodeAt(group_tree, "ShaderNodeMath", None, *nodePos)
            greaterThanNode.operation = "GREATER_THAN"
            mixNode, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMixRGB", None, mixNodeX, nodePos[1])

            group_tree.links.new(greaterThanNode.inputs[0], caseNodeInternal)
            group_tree.links.new(mixNode.inputs[0], greaterThanNode.outputs[0])
            greaterThanNode.inputs[1].default_value = case

            # Connect group input to nodes
            group_tree.links.new(mixNode.inputs[1], internalSocketDict[name])
            group_tree.links.new(mixNode.inputs[2], prevSocket)
...
def createNodeCombinerMix(node_tree, nodeASocket, nodeBSocket, nodeCSocket, nodeDSocket, location, isAlpha):
    ...
    nodePos = [0, 0]
    if not isAlpha:
        nodeSubtract, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMixRGB", "Subtract", *nodePos)
        nodeMultiply, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMixRGB", "Multiply", *nodePos)
        nodeAdd, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMixRGB", "Add", *nodePos)
        ...
        nodeSubtract, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMath", "Subtract", *nodePos)
        nodeMultiply, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMath", "Multiply", *nodePos)
        nodeAdd, x, nodePos[1] = addNodeAt(group_tree, "ShaderNodeMath", "Add", *nodePos)
...
def createNodeCombiner(node_tree, cycleIndex):
    ...
    group_tree = bpy.data.node_groups.new(type="ShaderNodeTree", name="Color Combiner F3D v3")
    groupNode.node_tree = group_tree
    groupNode.name = "Color Combiner Cycle " + str(cycleIndex) + " F3D v3"
    input_node = group_tree.nodes.new("NodeGroupInput")
    input_node.location = (-300, 0)
    ...
    out1 = createNodeCombinerMix(
        group_tree,
        caseSocketDict["Case A " + str(cycleIndex)],
    ...
    out_alpha1 = createNodeCombinerMix(
        group_tree,
        caseSocketDict["Case A Alpha " + str(cycleIndex)],
...
def createNodeF3D(node_tree, location):
    ...
    x = 0
    y = 0
    mixCycleNodeRGB, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", "Cycle Mix RGB", x, y)
    mixCycleNodeAlpha, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", "Cycle Mix Alpha", x, y)
    ...
    backFacing, x, y = addNodeAt(group_tree, "ShaderNodeNewGeometry", "Is Backfacing", x, y)

    x += 300
    y = 0
    multCullFront, x, y = addNodeAt(group_tree, "ShaderNodeMath", "Multiply Cull Front", x, y)
    multCullFront.operation = "MULTIPLY"
    multCullBack, x, y = addNodeAt(group_tree, "ShaderNodeMath", "Multiply Cull Back", x, y)
    multCullBack.operation = "MULTIPLY"

    finalCullAlpha, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", "Cull Alpha", x, y)
...
def createNodeToGroupLink(node, outputIndex, groupNode, groupInputNode, node_tree, nodeIndex, name, createSockets):
    return createSocketToGroupLink(
        node.outputs[outputIndex], groupNode, groupInputNode, node_tree, nodeIndex, name, createSockets
    )


def createSocketToGroupLink(socket, groupNode, groupInputNode, node_tree, nodeIndex, name, createSockets):
    if createSockets:
...
def nodeDictToInternalSocket(
    ...
    for name, node in nodeDict.items():
        newDict[name] = createNodeToGroupLink(
            node, 0 if name != "Noise" else 1, groupNode, groupInputNode, node_tree, nodeIndex, name, createSockets
        )
        nodeIndex += 1
        if name in texAlphaList:
            newDict[name + " Alpha"] = createNodeToGroupLink(
                node, 1, groupNode, groupInputNode, node_tree, nodeIndex, name + " Alpha", createSockets
            )
            nodeIndex += 1
            ...
            newDict[name + " Alpha"] = createNodeToGroupLink(
                bridgeNode, 0, groupNode, groupInputNode, node_tree, nodeIndex, name + " Alpha", createSockets
            )
            nodeIndex += 1
...
def socketDictToInternalSocket(node_tree, groupNode, groupInputNode, socketDict, startIndex, createSockets):
    nodeIndex = startIndex
    newDict = {}
    for name, socket in socketDict.items():
        newDict[name] = createSocketToGroupLink(
            socket, groupNode, groupInputNode, node_tree, nodeIndex, name, createSockets
        )
        nodeIndex += 1
    return newDict, nodeIndex
...
def createTexCoordNode(node_tree, location, uvSocket, socketDict, isV):
    ...
    if not createGroup:
        ...
        groupNode.node_tree = group_tree
        socketDict, nextSocketIndex = socketDictToInternalSocket(node_tree, groupNode, None, socketDict, 0, False)
        return groupNode

    group_tree = bpy.data.node_groups.new(type="ShaderNodeTree", name="Create Tex Coord " + verticalString + " F3D v3")
    links = group_tree.links
    groupNode.node_tree = group_tree
    input_node = group_tree.nodes.new("NodeGroupInput")
    output_node = group_tree.nodes.new("NodeGroupOutput")
    ...
    uvSocket = createSocketToGroupLink(uvSocket, groupNode, input_node, node_tree, 0, "UV", True)
    socketDict, nextSocketIndex = socketDictToInternalSocket(node_tree, groupNode, input_node, socketDict, 1, True)
    ...
    # Clamp using H
    clampLow, x, y = addNodeAt(group_tree, "ShaderNodeMath", "Max of NOT zero", 200, 0)
    ...
    clampHighHalfPixelOffset, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, 400, 200)
    ...
    mirrorToggleMix, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, 1800, -600)
...
def createUVGroup(node_tree, location, textureIndex):
    ...
    texGenSocketDict = addSocketList(
        groupNode,
        input_node,
        {
    ...
    x = 0
    y = 0
    horizontalDict, verticalDict, x, y = splitTextureVectorInputs(group_tree, socketDict, x, y)
    ...
    # Get normal
    geometryNode, x, y = addNodeAt(group_tree, "ShaderNodeNewGeometry", None, x, y)

    # Convert to screen space normal
    transformNode, x, y = addNodeAt(group_tree, "ShaderNodeVectorTransform", None, x, y)
    ...
    x += 300
    y = 0
    addOneNode, x, y = addNodeAt(group_tree, "ShaderNodeVectorMath", None, x, y)
    addOneNode.inputs[1].default_value = (1, 1, 1)
    links.new(addOneNode.inputs[0], transformNode.outputs[0])

    separateNode, x, y = addNodeAt(group_tree, "ShaderNodeSeparateXYZ", None, x, y)
    links.new(separateNode.inputs[0], addOneNode.outputs[0])

    divideTwoX, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, x, y)
    divideTwoY, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, x, y)
    ...
    x += 300
    y = 0
    normalizeX, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, x, y)
    normalizeY, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, x, y)
    ...
    texGenCombine, x, y = addNodeAt(group_tree, "ShaderNodeCombineXYZ", None, 1200, -300)
    ...
    texGenLinearAcosX, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, 600, -600)
    texGenLinearAcosY, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, 600, y)
    ...
    # Normalize values based on tex size.
    normalizeLinearX, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, 600, y)
    normalizeLinearY, x, y = addNodeAt(group_tree, "ShaderNodeMath", None, 600, y)
    ...
    texGenLinearCombine, x, y = addNodeAt(group_tree, "ShaderNodeCombineXYZ", None, 1200, -600)
    ...
    mixTexGen, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, 1500, 0)
    ...
    mixTexGenLinear, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, 1500, y)
    ...
    uvSplit, x, y = addNodeAt(group_tree, "ShaderNodeSeparateXYZ", None, 1800, 500)
    ...
    uvCombine, x, y = addNodeAt(group_tree, "ShaderNodeCombineXYZ", None, 2200, 500)
...
def createShadeNode(node_tree, location):
    ...
    group_tree = bpy.data.node_groups.new(type="ShaderNodeTree", name="Get Shade Color F3D v3")
    links = group_tree.links
    groupNode.node_tree = group_tree
    input_node = group_tree.nodes.new("NodeGroupInput")
    output_node = group_tree.nodes.new("NodeGroupOutput")
    ...
    socketDict = addSocketList(
        groupNode,
        input_node,
        {"Shading": "NodeSocketFloat", "Lighting": "NodeSocketFloat", "Ambient Color": "NodeSocketColor"},
    )
    ...
    addAmbient, x, y = addNodeAt(group_tree, "ShaderNodeVectorMath", None, 400, 0)
...
def createTexFormatNodes(node_tree, location, externalColorSocket, externalAlphaSocket):
    ...
    if not createGroup:
        ...
        nodeIndex = 0
        colorSocket = createSocketToGroupLink(
            externalColorSocket, groupNode, None, node_tree, nodeIndex, "Color", createGroup
        )
        nodeIndex += 1
        alphaSocket = createSocketToGroupLink(
            externalAlphaSocket, groupNode, None, node_tree, nodeIndex, "Alpha", createGroup
        )
        nodeIndex += 1
    ...
    nodeIndex = 0
    colorSocket = createSocketToGroupLink(
        externalColorSocket, groupNode, input_node, node_tree, nodeIndex, "Color", createGroup
    )
    nodeIndex += 1
    alphaSocket = createSocketToGroupLink(
        externalAlphaSocket, groupNode, input_node, node_tree, nodeIndex, "Alpha", createGroup
    )
    nodeIndex += 1
    ...
    socketDict = addSocketList(
        groupNode,
        input_node,
        {"Is Greyscale": "NodeSocketFloat", "Has Alpha": "NodeSocketFloat", "Is Intensity": "NodeSocketFloat"},
    )
    ...
    x += 300
    greyNode, x, y = addNodeAt(group_tree, "ShaderNodeSeparateHSV", None, x, y)
    links.new(greyNode.inputs[0], colorSocket)

    greyMix, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, x, y)
    ...
    alphaMix, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, x, y)
    ...
    alphaMixIntensity, x, y = addNodeAt(group_tree, "ShaderNodeMixRGB", None, x, y)
    ...
    def update_GetAlphaFromColor(self, context):
        ...
        if inputSocket.is_linked:
            for link in inputSocket.links:
                if link.is_valid:
                    self.inputs[0].default_value = link.from_socket.default_value
...
class F3DNodeA(ShaderNode):
    ...
    inA: bpy.props.EnumProperty(
        name="A", description="A", items=combiner_enums["Case A"], default="TEXEL0", update=update_F3DNodeA
    )
...
class F3DNodeB(ShaderNode):
    ...
    inB: bpy.props.EnumProperty(
        name="B", description="B", items=combiner_enums["Case B"], default="0", update=update_F3DNodeB
    )
...
class F3DNodeC(ShaderNode):
    ...
    inC: bpy.props.EnumProperty(
        name="C", description="C", items=combiner_enums["Case C"], default="SHADE", update=update_F3DNodeC
    )
...
class F3DNodeD(ShaderNode):
    ...
    inD: bpy.props.EnumProperty(
        name="D", description="D", items=combiner_enums["Case D"], default="0", update=update_F3DNodeD
    )
...
class F3DNodeA_alpha(ShaderNode):
    ...
    inA_alpha: bpy.props.EnumProperty(
        name="A Alpha",
        description="A Alpha",
        items=combiner_enums["Case A Alpha"],
        default="0",
        update=update_F3DNodeA_alpha,
    )
...
class F3DNodeB_alpha(ShaderNode):
    ...
    inB_alpha: bpy.props.EnumProperty(
        name="B Alpha",
        description="B Alpha",
        items=combiner_enums["Case B Alpha"],
        default="0",
        update=update_F3DNodeB_alpha,
    )
...
class F3DNodeC_alpha(ShaderNode):
    ...
    inC_alpha: bpy.props.EnumProperty(
        name="C Alpha",
        description="C Alpha",
        items=combiner_enums["Case C Alpha"],
        default="0",
        update=update_F3DNodeC_alpha,
    )
...
class F3DNodeD_alpha(ShaderNode):
    ...
    inD_alpha: bpy.props.EnumProperty(
        name="D Alpha",
        description="D Alpha",
        items=combiner_enums["Case D Alpha"],
        default="ENVIRONMENT",
        update=update_F3DNodeD_alpha,
    )
