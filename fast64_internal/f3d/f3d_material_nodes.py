...
def createNodeSwitch(node_tree, caseDict, caseSocket, caseName, location, socketDict):
    ...
    group_tree = bpy.data.node_groups.new(
        type="ShaderNodeTree", name="Switch " + caseName + " F3D v3"
    )
    groupNode.node_tree = group_tree
    ...
    for case in reversed(range(len(caseDict))):
        ...
        if case == len(caseDict) - 1:
            ...
            greaterThanNode, mixNodeX, y = addNodeAt(
                group_tree, "ShaderNodeMath", None, *nodePos
            )
            greaterThanNode.operation = "GREATER_THAN"
            mixNode, x, nodePos[1] = addNodeAt(
                group_tree, "ShaderNodeMixRGB", None, mixNodeX, nodePos[1]
            )
...
def createNodeCombinerMix(
    node_tree, nodeASocket, nodeBSocket, nodeCSocket, nodeDSocket, location, isAlpha
):
    groupNode = node_tree.nodes.new("ShaderNodeGroup")
    groupNode.location = location
    location[1] = location[1] - (groupNode.height + 100)
    ...
    createGroup = (
        "Color Combiner " + alphaText + "Mix F3D v3" not in bpy.data.node_groups
    )
    if not createGroup:
    ...
    # (A-B)*C + D
    group_tree = bpy.data.node_groups.new(
        type="ShaderNodeTree", name="Color Combiner " + alphaText + "Mix F3D v3"
    )
    groupNode.node_tree = group_tree
    input_node = group_tree.nodes.new("NodeGroupInput")
    input_node.location = (-300, 0)
    ...
    nodePos = [0, 0]
    if not isAlpha:
        nodeSubtract, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMixRGB", "Subtract", *nodePos
        )
        nodeMultiply, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMixRGB", "Multiply", *nodePos
        )
        nodeAdd, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMixRGB", "Add", *nodePos
        )
        ...
        nodeSubtract, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMath", "Subtract", *nodePos
        )
        nodeMultiply, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMath", "Multiply", *nodePos
        )
        nodeAdd, x, nodePos[1] = addNodeAt(
            group_tree, "ShaderNodeMath", "Add", *nodePos
        )
...
def createNodeF3D(node_tree, location):
    ...
    x = 0
    y = 0
    mixCycleNodeRGB, x, y = addNodeAt(
        group_tree, "ShaderNodeMixRGB", "Cycle Mix RGB", x, y
    )
    mixCycleNodeAlpha, x, y = addNodeAt(
        group_tree, "ShaderNodeMixRGB", "Cycle Mix Alpha", x, y
    )
    ...
    backFacing, x, y = addNodeAt(
        group_tree, "ShaderNodeNewGeometry", "Is Backfacing", x, y
    )

    x += 300
    y = 0
    multCullFront, x, y = addNodeAt(
        group_tree, "ShaderNodeMath", "Multiply Cull Front", x, y
    )
    multCullFront.operation = "MULTIPLY"
    multCullBack, x, y = addNodeAt(
        group_tree, "ShaderNodeMath", "Multiply Cull Back", x, y
    )
...
def socketDictToInternalSocket(
    node_tree, groupNode, groupInputNode, socketDict, startIndex, createSockets
):
    nodeIndex = startIndex
    newDict = {}
...
def createTexCoordNode(node_tree, location, uvSocket, socketDict, isV):
    ...
    createGroup = (
        "Create Tex Coord " + verticalString + " F3D v3" not in bpy.data.node_groups
    )
    if not createGroup:
        group_tree = bpy.data.node_groups[
            "Create Tex Coord " + verticalString + " F3D v3"
        ]
        groupNode.node_tree = group_tree
    ...
    uvSocket = createSocketToGroupLink(
        uvSocket, groupNode, input_node, node_tree, 0, "UV", True
    )
    socketDict, nextSocketIndex = socketDictToInternalSocket(
        node_tree, groupNode, input_node, socketDict, 1, True
    )
    ...
    clampHighHalfPixelOffset, x, y = addNodeAt(
        group_tree, "ShaderNodeMath", None, 400, 200
    )
...
def splitTextureVectorInputs(node_tree, socketDict, x, y):
    ...
    verticalDict = {}
    for name, socket in socketDict.items():
        splitNode, x, y = addNodeAt(
            node_tree, "ShaderNodeSeparateXYZ", "Split Texture Vector", x, y
        )
...
def createUVGroup(node_tree, location, textureIndex):
    ...
    x = 0
    y = 0
    horizontalDict, verticalDict, x, y = splitTextureVectorInputs(
        group_tree, socketDict, x, y
    )
    ...
    texGenCombine, x, y = addNodeAt(
        group_tree, "ShaderNodeCombineXYZ", None, 1200, -300
    )
    ...
    texGenLinearCombine, x, y = addNodeAt(
        group_tree, "ShaderNodeCombineXYZ", None, 1200, -600
    )
    ...
    uv_xNode = createTexCoordNode(
        group_tree, [2000, 500], uvSplit.outputs[0], horizontalDict, False
    )
    uv_yNode = createTexCoordNode(
        group_tree, [2000, 300], uvSplit.outputs[1], verticalDict, True
    )
...
def createShadeNode(node_tree, location):
    ...
    group_tree = bpy.data.node_groups.new(
        type="ShaderNodeTree", name="Get Shade Color F3D v3"
    )
    links = group_tree.links
    groupNode.node_tree = group_tree
    input_node = group_tree.nodes.new("NodeGroupInput")
...
def createTexFormatNodes(node_tree, location, externalColorSocket, externalAlphaSocket):
    ...
    group_tree = bpy.data.node_groups.new(
        type="ShaderNodeTree", name="Get Texture Color F3D v3"
    )
    links = group_tree.links
    groupNode.node_tree = group_tree
    input_node = group_tree.nodes.new("NodeGroupInput")
...
class GetAlphaFromColor(ShaderNode):
    ...
    inColor: bpy.props.FloatVectorProperty(
        name="Input Color", subtype="COLOR", size=4, update=update_GetAlphaFromColor
    )
...
class F3DNodeA_alpha(ShaderNode):
    ...
    def update_F3DNodeA_alpha(self, context):
        ...
        if out.is_linked:
            for link in out.links:
                if link.is_valid:
                    link.to_socket.default_value = F3D("F3D", False).ACMUXDict[
                        self.inA_alpha
                    ]
