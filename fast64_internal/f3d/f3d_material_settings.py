...
class F3DMaterialSettings:
    ...
    def loadFromMaterial(self, material, includeValues):
        ...
        # cycle dependent - (P * A + M - B) / (A + B)
        self.rendermode_advanced_enabled = (
            material.rdp_settings.rendermode_advanced_enabled
        )
    ...
    def applyToMaterial(self, material, includeValues, updateFunc, context):
        ...
        if self.set_rendermode:
            ...
            material.rdp_settings.rendermode_preset_cycle_1 = (
                self.rendermode_preset_cycle_1
            )
            material.rdp_settings.rendermode_preset_cycle_2 = (
        ...
        if includeValues:
            ...
            for name, item in F3DOutputCopyList.items():
                if material.mat_ver == 3 and item == "Primitive Color":
                    nodes["Primitive Color Output"].inputs[0].default_value = getattr(
                        self, "prim_color"
                    )
                elif material.mat_ver == 3 and item == "Environment Color":
                    nodes["Environment Color Output"].inputs[0].default_value = getattr(
                        self, "env_color"
                    )
                else:
