...
class SerializedMaterialNodeTree(SerializedNodeTree):
    ...
    def dump(self, path: Path):
        ...
        with path.open("w", encoding="utf-8") as f:
            json.dump(self.to_json(), f, indent="\t", sort_keys=True)
        for name, node_tree in self.dependencies.items():
            with Path(path.parent / to_valid_file_name(name + ".json")).open("w") as f:
                json.dump(node_tree.to_json(), f, indent="\t", sort_keys=True)
...
def update_f3d_material_nodes(material: Material, ignore_hash=False):
    ...
    try:
        if is_f3d_mat(material) and (
            material.node_tree.get("fast64_cached_hash", None) != SERIALIZED_NODE_LIBRARY.cached_hash or ignore_hash
        ):
...
def update_f3d_materials_nodes(ignore_hash=False):
