        ...
        if not self.first_time:
            ...
            for datablock in depsgraph.ids:
                ...
                if isinstance(datablock, bpy.types.Image):
                    self.draw_data.textures[datablock.name] = F3DRendererTexture(
                        datablock
                    )
                    print("Create Texture: " + str(glGetError()))
                elif isinstance(datablock, bpy.types.Material):
                    self.draw_data.materials[datablock.name] = F3DRendererMaterial(
                        datablock
                    )
                    print("Create Material: " + str(glGetError()))
                elif isinstance(datablock, bpy.types.Mesh):
                    pass
                elif isinstance(datablock, bpy.types.Object) and isinstance(
                    datablock.data, bpy.types.Mesh
                ):
                    self.draw_data.objects[datablock.name] = F3DRendererObject(
                        datablock, self.draw_data
                    )
    ...
    def __init__(self, obj, render_data):
        ...
        for material_index, faces in facesByMat.items():
            ...
            self.submeshes.append(
                F3DRendererSubmesh(f3d_material, obj, faces, render_data)
            )
...
class F3DRendererSubmesh:
    def __init__(self, f3d_material, obj, triangles, render_data):
        ...
        if True:  # TODO: Choose normal or vertex color
            ...
            for loopIndex in loopIndices:
                # TODO: Fix Alpha
                colorOrNormal.extend(
                    color_data[loopIndex][0:3] + [alpha_data[loopIndex][0]]
                )
        ...
        colorOrNormal_location = glGetAttribLocation(
            render_data.shaderProgram, "colorOrNormal"
        )
        print(
            "pos: "
        ...
        glBufferData(
            GL_ARRAY_BUFFER, len(position) * 4, self.position_buffer, GL_STATIC_DRAW
        )
    ...
    def draw(self):
        ...
        transformLocation = glGetUniformLocation(
            self.render_data.shaderProgram, "transform"
        )
