...
def getInfoDict(obj):
    ...
    if len(obj.data.uv_layers) == 0:
        ...
        if uv_data is None:
            raise PluginError(
                "Object '" + obj.name + "' does not have a UV layer named 'UVMap.'"
            )
    for face in mesh.loop_triangles:
        ...
        if material is None:
            raise PluginError(
                "There are some faces on your mesh that are assigned to an empty material slot."
            )
        for vertIndex in face.vertices:
            if vertIndex not in vertDict:
                vertDict[vertIndex] = []
        ...
        for loopIndex in face.loops:
            ...
            f3dVertDict[loopIndex] = getF3DVert(
                mesh.loops[loopIndex], face, convertInfo, mesh
            )
    for face in mesh.loop_triangles:
        for edgeKey in face.edge_keys:
        ...
        for i in range(2):
            ...
            if minDiff > 0:
                applyOffset(
                    ...
                    maxUV,
                    uvOffset,
                    ceil(minDiff / UVinterval[i]) * UVinterval[i],
                    i,
                )
            ...
            if maxDiff > 0:
                applyOffset(
                    ...
                    maxUV,
                    uvOffset,
                    -ceil(maxDiff / UVinterval[i]) * UVinterval[i],
                    i,
                )
...
class TileLoad:
    ...
    def appendTile(self, sl, sh, tl, th):
        ...
        tmemUsage = (
            getTmemWordUsage(self.texFormat, newWidth, newHeight)
            * 8
...
def saveMeshWithLargeTexturesByFaces(
    ...
    for face in faces:
        uvs = [
            UVtoST(obj, loopIndex, uv_data, texDimensions, isPointSampled)
            for loopIndex in face.loops
        ]
    ...
    if fMaterial.texturesLoaded[otherTextureIndex]:
        tmem = (
            getTmemWordUsage(
                otherTex.tex_format, otherTex.tex.size[0], otherTex.tex.size[1]
            )
            * 8
        )
    ...
    for tileLoad, tileFaces in tileLoads:
        ...
        if fMaterial.texturesLoaded[0] and not (
            otherTextureIndex == 0 and otherTexSingleLoad
        ):
        ...
        if fMaterial.texturesLoaded[1] and not (
            otherTextureIndex == 1 and otherTexSingleLoad
        ):
    ...
    for material_index, faces in facesByMat.items():
        ...
        if drawLayer not in fMeshes:
            fMesh = fModel.addMesh(
                obj.original_name, ownerName, drawLayerName, False, obj
            )
...
def addCullCommand(obj, fMesh, transformMatrix, matWriteMethod):
    ...
    if matWriteMethod == GfxMatWriteMethod.WriteDifferingAndRevert:
        ...
        raise PluginError(
            "Unhandled material write method for f3d culling: " + str(matWriteMethod)
        )
    fMesh.draw.commands = cullCommands + fMesh.draw.commands


def exportF3DCommon(
    obj, fModel, transformMatrix, includeChildren, name, DLFormat, convertTextureData
):
    ...
    try:
        ...
        triConverterInfo = TriangleConverterInfo(
            tempObj, None, fModel.f3d, transformMatrix, infoDict
        )
...
def checkForF3DMaterial(obj):
    if len(obj.material_slots) == 0:
        raise PluginError(
            obj.name
            + " has no Fast3D material. Make sure to add a Fast3D material to it."
        )
    ...
    for edgeKey1 in face1.edge_keys:
        for edgeKey2 in face2.edge_keys:
            if edgeKey1 == edgeKey2:
                return edgeKey1
    raise PluginError(
        "No common edge between faces " + str(face1.index) + " and " + str(face2.index)
    )
...
def getNextNeighborFace(
    faces, face, lastEdgeKey, visitedFaces, possibleFaces, infoDict
):
    ...
    while nextEdgeKey not in handledEdgeKeys:
        for linkedFace in infoDict.edge[nextEdgeKey]:
            if linkedFace == face or linkedFace not in faces:
                ...
                edgeValid(infoDict.edgeValid, linkedFace, face)
                and linkedFace not in visitedFaces
            ):
...
def get8bitRoundedNormal(loop, mesh):
    alpha_layer = (
        mesh.vertex_colors["Alpha"].data if "Alpha" in mesh.vertex_colors else None
    )
    ...
    def getTransformMatrix(self, groupIndex):
        if self.armature is None or groupIndex is None:
            ...
            if groupIndex not in self.groupNames:
                self.groupNames[groupIndex] = getGroupNameFromIndex(
                    self.obj, groupIndex
                )
...
class TriangleConverter:
    def __init__(
        ...
        self.convertInfo = LoopConvertInfo(
            uv_data, triConverterInfo.obj, exportVertexColors
        )
        self.texDimensions = texDimensions
    ...
    def processGeometry(self):
        ...
        for groupIndex, bufferVerts in limbVerts.items():
            if groupIndex != self.currentGroupIndex:
                self.triList.commands.append(
                    SPMatrix(
                        self.triConverterInfo.getMatrixAddrFromGroup(groupIndex),
                        "G_MTX_LOAD",
                    )
                )
    ...
    def addFace(self, face):
        ...
        if (
            len(self.vertBuffer) + len(addedVerts)
            > self.triConverterInfo.f3d.vert_load_size
        ):
...
def getF3DVert(loop, face, convertInfo, mesh):
    ...
    colorOrNormal = getLoopColorOrNormal(
        loop,
        face,
        convertInfo.obj.data,
        convertInfo.obj,
        convertInfo.exportVertexColors,
    )
...
def convertVertexData(
    ...
    if exportVertexColors:
        ...
        normal = (
            transformMatrix.inverted().transposed() @ loopColorOrNormal
        ).normalized()
        colorOrNormal = [
...
def getLoopColor(loop, mesh, mat_ver):
    ...
    color_layer = (
        mesh.vertex_colors["Col"].data if "Col" in mesh.vertex_colors else None
    )
    alpha_layer = (
        mesh.vertex_colors["Alpha"].data if "Alpha" in mesh.vertex_colors else None
    )
...
def getTexDimensions(material):
    ...
    if useDict["Texture 0"] and f3dMat.tex0.tex_set:
        if f3dMat.tex0.use_tex_reference:
            ...
            if f3dMat.tex0.tex is None:
                raise PluginError(
                    'In material "' + material.name + '", a texture has not been set.'
                )
    ...
    if texDimensions0 is not None and texDimensions1 is not None:
        texDimensions = (
            texDimensions0 if f3dMat.uv_basis == "TEXEL0" else texDimensions1
        )
...
def saveOrGetF3DMaterial(material, fModel, obj, drawLayer, convertTextureData):
    ...
    materialName = (
        ...
        + (
            ...
            if f3dMat.rdp_settings.set_rendermode and drawLayer is not None
            else ""
        )
        + (
            ("_area" + str(areaIndex))
            if f3dMat.set_fog and f3dMat.use_global_fog and areaKey is not None
            else ""
        )
    )
    ...
    if not material.is_f3d:
        raise PluginError(
            "Material named " + material.name + " is not an F3D material."
        )
    ...
    if f3dMat.set_fog:
        if (
            ...
            and fModel.global_data.getCurrentAreaData() is not None
        ):
    ...
    if fModel.f3d.F3DEX_GBI_2:
        saveGeoModeDefinitionF3DEX2(
            fMaterial, f3dMat.rdp_settings, defaults, fModel.matWriteMethod
        )
    else:
        saveGeoModeDefinition(
            fMaterial, f3dMat.rdp_settings, defaults, fModel.matWriteMethod
        )
    ...
    saveOtherModeLDefinition(
        fMaterial, f3dMat.rdp_settings, defaults, defaultRM, fModel.matWriteMethod
    )
    ...
    if useDict["Shade"] and f3dMat.set_lights:
        fLights = saveLightsDefinition(
            fModel, fMaterial, f3dMat, materialName + "_lights"
        )
        fMaterial.material.commands.extend(
    ...
    if useDict["Key"] and f3dMat.set_key:
        ...
        fMaterial.material.commands.extend(
            [
                ...
                DPSetKeyR(
                    int(center[0] * 255), int(scale[0] * 255), int(width[0] * 2 ** 8)
                ),
                DPSetKeyGB(
                    int(center[1] * 255),
                    int(scale[1] * 255),
    ...
    fModel.onMaterialCommandsBuilt(
        fMaterial.material, fMaterial.revert, material, drawLayer
    )
...
def saveTextureIndex(
    ...
    if tex is not None and (tex.size[0] == 0 or tex.size[1] == 0):
        raise PluginError(
            "Image "
            + tex.name
            + " has either a 0 width or height; image may have been removed from original location."
        )
    ...
    texName = (
        ...
        + "_"
        + (
            getNameFromPath(name, True) + "_" + texFormat.lower()
    ...
    if not bpy.context.scene.ignoreTextureRestrictions and loadTextures:
        ...
        if width > 1024 or height > 1024:
            raise PluginError(
                'Error in "'
                + propName
                + '": Any side of an image cannot be greater '
                + "than 1024."
            )
    ...
    convertTextureData = convertTextureData and not (
        fMaterial.useLargeTextures and fMaterial.saveLargeTextures[index]
    )
    if isCITexture:
        if texProp.use_tex_reference:
            fImage = FImage(
                texProp.tex_reference, None, None, width, height, None, False
            )
            fPalette = FImage(
                texProp.pal_reference,
                None,
                None,
                1,
                texProp.pal_reference_size,
                None,
                False,
            )
        else:
        ...
        if texProp.use_tex_reference:
            ...
            fImage = saveOrGetTextureDefinition(
                fMaterial, fModel, tex, texName, texFormat, convertTextureData
            )
...
def saveTextureLoading(
    ...
    cms = [
        ("G_TX_CLAMP" if clamp_S else "G_TX_WRAP"),
        ("G_TX_MIRROR" if mirror_S else "G_TX_NOMIRROR"),
    ]
    cmt = [
        ("G_TX_CLAMP" if clamp_T else "G_TX_WRAP"),
    ...
    useLoadBlock = (
        not fImage.isLargeTexture
        and isPowerOf2(fImage.width)
        and isPowerOf2(fImage.height)
    )
            ...
            loadTexGfx.commands.extend(
                [
                    ...
                    DPLoadBlock(
                        ...
                        0,
                        0,
                        (((fImage.width) * (fImage.height) + 3) >> 2) - 1,
                        dxt,
                    ),
                ]
            )
...
def savePaletteLoading(
    loadTexGfx, revertTexGfx, fPalette, palFormat, pal, colorCount, f3d, matWriteMethod
):
    palFmt = texFormatOf[palFormat]
    cms = ["G_TX_WRAP", "G_TX_NOMIRROR"]
    cmt = ["G_TX_WRAP", "G_TX_NOMIRROR"]

    loadTexGfx.commands.append(
...
def saveOrGetPaletteDefinition(
    fMaterial, fModelOrTexRect, image, imageName, texFmt, palFmt, convertTextureData
):
    ...
    if image.filepath == "":
        ...
        name = image.filepath
    filename = (
        getNameFromPath(name, True)
        + "."
        + fModelOrTexRect.getTextureSuffixFromFormat(texFmt)
        + ".inc.c"
    )
    paletteFilename = (
        getNameFromPath(name, True)
...
def compactNibbleArray(texture, width, height):
    ...
    nibbleData = [
        ((texture[i * 2] & 0xF) << 4) | (texture[i * 2 + 1] & 0xF)
...
def saveOrGetTextureDefinition(
    fMaterial, fModel, image, imageName, texFormat, convertTextureData
):
    ...
    filename = (
        getNameFromPath(name, True)
        + "."
        + fModel.getTextureSuffixFromFormat(texFormat)
        + ".inc.c"
    )
...
def saveLightsDefinition(fModel, fMaterial, material, lightsName):
    ...
    if material.use_default_lighting:
        ...
        lights.a = Ambient(
            [int(color[0] * 255 / 2), int(color[1] * 255 / 2), int(color[2] * 255 / 2)]
        )
        lights.l.append(
            Light(
...
def getLightRotation(lightData):
    ...
    if lightObj is None:
        raise PluginError(
            "A material is referencing a light that is no longer in the scene (i.e. has been deleted)."
        )
...
def normToSigned8Vector(normal):
    return [
        int.from_bytes(int(value * 127).to_bytes(1, "big", signed=True), "big")
        for value in normal
    ]
...
def saveGeoModeDefinitionF3DEX2(fMaterial, settings, defaults, matWriteMethod):
    ...
    saveBitGeoF3DEX2(
        settings.g_zbuffer, defaults.g_zbuffer, "G_ZBUFFER", geo, matWriteMethod
    )
    saveBitGeoF3DEX2(settings.g_shade, defaults.g_shade, "G_SHADE", geo, matWriteMethod)
    saveBitGeoF3DEX2(
        settings.g_cull_front,
        defaults.g_cull_front,
        "G_CULL_FRONT",
        geo,
        matWriteMethod,
    )
    saveBitGeoF3DEX2(
        settings.g_cull_back, defaults.g_cull_back, "G_CULL_BACK", geo, matWriteMethod
    )
    saveBitGeoF3DEX2(settings.g_fog, defaults.g_fog, "G_FOG", geo, matWriteMethod)
    saveBitGeoF3DEX2(
        settings.g_lighting, defaults.g_lighting, "G_LIGHTING", geo, matWriteMethod
    )

    # make sure normals are saved correctly.
    saveBitGeoF3DEX2(
        settings.g_tex_gen, defaults.g_tex_gen, "G_TEXTURE_GEN", geo, matWriteMethod
    )
    saveBitGeoF3DEX2(
        settings.g_tex_gen_linear,
        defaults.g_tex_gen_linear,
        "G_TEXTURE_GEN_LINEAR",
        geo,
        matWriteMethod,
    )
    saveBitGeoF3DEX2(
        settings.g_shade_smooth,
        defaults.g_shade_smooth,
        "G_SHADING_SMOOTH",
        geo,
        matWriteMethod,
    )
    saveBitGeoF3DEX2(
        settings.g_clipping, defaults.g_clipping, "G_CLIPPING", geo, matWriteMethod
    )
    ...
    if len(geo.clearFlagList) != 0 or len(geo.setFlagList) != 0:
        ...
        if matWriteMethod == GfxMatWriteMethod.WriteAll:
            ...
            fMaterial.revert.commands.append(
                SPGeometryMode(geo.setFlagList, geo.clearFlagList)
            )
...
def saveGeoModeDefinition(fMaterial, settings, defaults, matWriteMethod):
    ...
    saveBitGeo(
        settings.g_shade, defaults.g_shade, "G_SHADE", setGeo, clearGeo, matWriteMethod
    )
    saveBitGeo(
        settings.g_cull_front,
        defaults.g_cull_front,
    ...
    saveBitGeo(
        settings.g_fog, defaults.g_fog, "G_FOG", setGeo, clearGeo, matWriteMethod
    )
    saveBitGeo(
        settings.g_lighting,
        defaults.g_lighting,
...
def saveOtherModeHDefinitionIndividual(fMaterial, settings, defaults, isHWv1):
    saveModeSetting(
        fMaterial,
        settings.g_mdsft_alpha_dither,
        defaults.g_mdsft_alpha_dither,
        DPSetAlphaDither,
    )
    ...
    if not isHWv1:
        saveModeSetting(
            fMaterial,
            settings.g_mdsft_rgb_dither,
            defaults.g_mdsft_rgb_dither,
            DPSetColorDither,
        )

        saveModeSetting(
            fMaterial,
            settings.g_mdsft_combkey,
            defaults.g_mdsft_combkey,
            DPSetCombineKey,
        )

    saveModeSetting(
        fMaterial,
        settings.g_mdsft_textconv,
        defaults.g_mdsft_textconv,
        DPSetTextureConvert,
    )

    saveModeSetting(
        fMaterial,
        settings.g_mdsft_text_filt,
        defaults.g_mdsft_text_filt,
        DPSetTextureFilter,
    )
    ...
    saveModeSetting(
        fMaterial, settings.g_mdsft_textlod, defaults.g_mdsft_textlod, DPSetTextureLOD
    )

    saveModeSetting(
        fMaterial,
        settings.g_mdsft_textdetail,
        defaults.g_mdsft_textdetail,
        DPSetTextureDetail,
    )

    saveModeSetting(
        fMaterial,
        settings.g_mdsft_textpersp,
        defaults.g_mdsft_textpersp,
        DPSetTexturePersp,
    )

    saveModeSetting(
        fMaterial,
        settings.g_mdsft_cycletype,
        defaults.g_mdsft_cycletype,
        DPSetCycleType,
    )
    ...
    if isHWv1:
        saveModeSetting(
            fMaterial,
            settings.g_mdsft_color_dither,
            defaults.g_mdsft_color_dither,
            DPSetColorDither,
        )

    saveModeSetting(
        fMaterial, settings.g_mdsft_pipeline, defaults.g_mdsft_pipeline, DPPipelineMode
    )


def saveOtherModeLDefinition(
    fMaterial, settings, defaults, defaultRenderMode, matWriteMethod
):
    if matWriteMethod == GfxMatWriteMethod.WriteAll:
        saveOtherModeLDefinitionAll(fMaterial, settings, defaults, defaultRenderMode)
    elif matWriteMethod == GfxMatWriteMethod.WriteDifferingAndRevert:
        saveOtherModeLDefinitionIndividual(
            fMaterial, settings, defaults, defaultRenderMode
        )
    else:
        raise PluginError("Unhandled material write method: " + str(matWriteMethod))


def saveOtherModeLDefinitionAll(
    fMaterial: FMaterial, settings, defaults, defaultRenderMode
):
    ...
    if settings.g_mdsft_zsrcsel == "G_ZS_PRIM":
        fMaterial.material.commands.append(
            DPSetPrimDepth(z=settings.prim_depth.z, dz=settings.prim_depth.dz)
        )
        fMaterial.revert.commands.append(DPSetPrimDepth())


def saveOtherModeLDefinitionIndividual(
    fMaterial, settings, defaults, defaultRenderMode
):
    saveModeSetting(
        fMaterial,
        settings.g_mdsft_alpha_compare,
        defaults.g_mdsft_alpha_compare,
        DPSetAlphaCompare,
    )

    saveModeSetting(
        fMaterial, settings.g_mdsft_zsrcsel, defaults.g_mdsft_zsrcsel, DPSetDepthSource
    )
...
def getWriteMethodFromEnum(enumVal):
    if enumVal not in matWriteMethodEnumDict:
        raise PluginError(
            "Enum value " + str(enumVal) + " not found in material write method dict."
        )
    else:
...
def exportF3DtoC(
    ...
    exportData = fModel.to_c(
        TextureExportSettings(texSeparate, savePNG, texDir, modelDirPath), gfxFormatter
    )
    ...
    writeCData(
        staticData,
        os.path.join(modelDirPath, "header.h"),
...
def removeDL(sourcePath, headerPath, DLName):
    ...
    matchResult = re.search(
        "Gfx\s*" + re.escape(DLName) + "\s*\[\s*[0-9x]*\s*\]\s*=\s*\{([^}]*)}\s*;\s*",
        DLDataC,
        re.DOTALL,
    )
