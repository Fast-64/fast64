...
def mat_updater_draw(layout: UILayout, context: Context):
    col = layout.column()
    op = col.operator(MatUpdateConvert.bl_idname)
    op.update_conv_all = context.scene.update_conv_all
    col.prop(context.scene, "update_conv_all")
    col.operator(ReloadDefaultF3DPresets.bl_idname)


mat_updater_classes = (MatUpdateConvert,)


def mat_updater_register():
    for cls in mat_updater_classes:
        register_class(cls)
