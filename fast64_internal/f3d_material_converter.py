                ...
                for switchOption in bone.switch_options:
                    if switchOption.switchType == "Material":
                        if switchOption.materialOverride in materialDict:
                            switchOption.materialOverride = materialDict[
                        ...
                        for i in range(len(switchOption.specificOverrideArray)):
                            ...
                            if material in materialDict:
                                switchOption.specificOverrideArray[
                                    i
                                ].material = materialDict[material]
...
def convertF3DtoNewVersion(obj, index, material, materialDict, version):
    ...
    if version > 3:
        ...
        matSettings.applyToMaterial(
            newMat, True, update_node_values_of_material, bpy.context
        )
...
def convertBSDFtoF3D(obj, index, material, materialDict):
    if not material.use_nodes:
        ...
        tex1Node = material.node_tree.nodes["Principled BSDF"].inputs[
            "Subsurface Color"
        ]
        if len(tex0Node.links) == 0:
            newMaterial = createF3DMat(
                obj, preset=getDefaultMaterialPreset("Shaded Solid"), index=index
            )
            f3dMat = newMaterial.f3d_mat if newMaterial.mat_ver > 3 else newMaterial
            ...
            if isinstance(tex0Node.links[0].from_node, bpy.types.ShaderNodeTexImage):
                if "convert_preset" in material:
                    ...
                    if presetName not in [
                        enumValue[0] for enumValue in enumMaterialPresets
                    ]:
                        raise PluginError(
                ...
                if len(tex1Node.links) > 0 and isinstance(
                    tex1Node.links[0].from_node, bpy.types.ShaderNodeTexImage
                ):
                    f3dMat.tex1.tex = tex1Node.links[0].from_node.image
                updateMatWithName(newMaterial, material, materialDict)
            else:
                print(
                    "Principled BSDF material does not have an Image Node attached to its Base Color."
                )
    else:
