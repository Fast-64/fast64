...
class node_cls():
    ...
    def cam_imp(self,cam,vol,scale):
        ...
        x,y,z = ( (Null(cn.CamXLock1,-9999) + Null(cn.CamXLock2,9999))/(2*scale),
        ...
        x,y,z = ( (Null(cn.CamXLock2,9999)-Null(cn.CamXLock1,-9999))/(2*scale),
    ...
    def path_imp(self, path, scale):
        ...
        path.location = [f/scale for f in (first[0], -first[2], first[1])] #rotation is not applied to location so I have to manually swap coords
...
class level_bin(misc_bin):
    ...
    def decode_node(self,num):
        node = node_cls(num)
        kirb = namedtuple('kirb_node','node EnterEnum w l a DestNode unused shad1 shad2 shad3 unused2 WarpFlag opt1 opt2 opt3 opt4 unused3')
