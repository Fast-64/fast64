...
def cmdMiscSettings(scene, header, cmdCount):
    ...
    cmd.source = (
        ...
        + str(scene.cameraMode)
        + ", "
        + str(scene.mapLocation)
        + "),\n"
    )
    return cmd
...
def cmdSpawnList(scene, header, cmdCount):
    ...
    cmd.source = (
        ...
        + ", "
        + (
            ("&" + scene.startPositionsName(header))
...
def ootMeshEntryToC(meshEntry, meshType):
    opaqueName = (
        meshEntry.DLGroup.opaque.name if meshEntry.DLGroup.opaque is not None else "0"
    )
    transparentName = (
        meshEntry.DLGroup.transparent.name
    ...
    if meshType == "1":
        ...
        data += (
            str(meshEntry.cullGroup.position[0])
            + ", "
            + str(meshEntry.cullGroup.position[1])
            + ", "
        )
        data += (
            str(meshEntry.cullGroup.position[2])
            + ", "
            + str(meshEntry.cullGroup.cullDepth)
            + ", "
        )
...
def ootRoomMeshToC(room, textureExportSettings):
    ...
    meshHeader = CData()
    meshHeader.header = (
        "extern MeshHeader" + str(mesh.meshType) + " " + mesh.headerName() + ";\n"
    )
    meshHeader.source = (
        "MeshHeader"
        + str(mesh.meshType)
    ...
    meshEntries.source += "};\n\n"
    exportData = mesh.model.to_c(
        textureExportSettings, OOTGfxFormatter(ScrollMethod.Vertex)
    )
...
def ootRoomCommandsToC(room, headerIndex):
    ...
    if room.hasAlternateHeaders():
        commands.append(
            cmdAltHeaders(
                room.roomName(), room.alternateHeadersName(), headerIndex, len(commands)
            )
        )
...
def ootAlternateRoomMainToC(scene, room):
    ...
    for i in range(len(room.cutsceneHeaders)):
        altHeader.source += (
            "\t" + room.roomName() + "_header" + format(i + 4, "02") + ",\n"
        )
...
def ootEntranceToC(entrance):
    return (
        "{ "
        + str(entrance.startPositionIndex)
        + ", "
        + str(entrance.roomIndex)
        + " },\n"
    )
...
def ootEntranceListToC(scene, headerIndex):
    data = CData()
    data.header = (
        "extern EntranceEntry " + scene.entranceListName(headerIndex) + "[];\n"
    )
...
def ootExitListToC(scene, headerIndex):
    ...
    data.header = (
        "extern u16 "
        + scene.exitListName(headerIndex)
        + "["
        + str(len(scene.exitList))
        + "];\n"
    )
    data.source = (
        "u16 "
...
def ootLightSettingsToC(scene, useIndoorLighting, headerIndex):
    ...
    data.header = (
        "extern LightSettings "
        + scene.lightListName(headerIndex)
        + "["
        + str(lightArraySize)
        + "];\n"
    )
...
def ootPathListToC(scene):
    data = CData()
    data.header = (
        "extern Path " + scene.pathListName() + "[" + str(len(scene.pathList)) + "];\n"
    )
    data.source = (
        "Path " + scene.pathListName() + "[" + str(len(scene.pathList)) + "] = {\n"
    )
    pathData = CData()
    for i in range(len(scene.pathList)):
        path = scene.pathList[i]
        data.source += (
            "\t" + "{ " + str(len(path.points)) + ", (u32)" + path.pathName() + " },\n"
        )
...
def ootAlternateSceneMainToC(scene):
    ...
    if scene.childNightHeader is not None:
        altHeader.source += (
            "\t" + scene.sceneName() + "_header" + format(1, "02") + ",\n"
        )
    ...
    if scene.adultDayHeader is not None:
        altHeader.source += (
            "\t" + scene.sceneName() + "_header" + format(2, "02") + ",\n"
        )
    ...
    if scene.adultNightHeader is not None:
        altHeader.source += (
            "\t" + scene.sceneName() + "_header" + format(3, "02") + ",\n"
        )
    ...
    for i in range(len(scene.cutsceneHeaders)):
        altHeader.source += (
            "\t" + scene.sceneName() + "_header" + format(i + 4, "02") + ",\n"
        )
...
def ootSceneMainToC(scene, headerIndex):
    ...
    if len(scene.lights) > 0:
        sceneMainC.append(
            ootLightSettingsToC(scene, scene.skyboxLighting == "0x01", headerIndex)
        )
...
def ootLevelToC(scene, textureExportSettings):
    ...
    for i in range(len(scene.rooms)):
        levelC.roomMainC[scene.rooms[i].roomName()] = ootRoomMainToC(
            scene, scene.rooms[i], 0
        )
