...
def getSegmentSymbolEntriesBySceneName(segmentSymbols, sceneName):
    return [
        ...
        if re.match(sceneName + "\_scene", entry.name)
        or re.match(sceneName + "\_room\_(\d)+", entry.name)
    ]
...
def readSegmentSymbols(exportPath):
    ...
    matchResult = re.search(
        "(DECLARE_SEGMENT\s*\(\s*boot\s*\).*)\#endif", fileData, re.DOTALL
    )
    if matchResult is None:
        raise PluginError(
            "segment_symbols.h does not have DECLARE_SEGMENT(boot) in it."
        )
...
def modifySegmentSymbols(scene, exportInfo):
    ...
    entries = getSegmentSymbolEntriesBySceneName(
    ...
    if scene is not None:
        segmentSymbols.insert(
            firstIndex, SegmentSymbolEntry("DECLARE_ROM_SEGMENT", scene.name + "_scene")
        )
        firstIndex += 1
