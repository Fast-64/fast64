...
from ...cutscene.constants import ootEnumCSListTypeListC
...
@dataclass
class Cutscene(CutsceneCommands):
    ...
    def getCutsceneData(self):
        csProp: "OOTCutsceneProperty" = self.csObj.ootCutsceneProperty
    ...
    def getC(self):
        ...
        csData.source = (
            ...
            + (self.getCutsceneData() if not self.motionOnly else "")
            + self.getActorCueListData(False)
            + self.getActorCueListData(True)
            + self.getCameraShotData()
            + "};\n\n"
        )
        ...
        for csObj in self.cutsceneObjects:
            if csObj is not None:
                ...
                if self.props.writeCutscene:
                    self.entries.append(
                        Cutscene(self.getCutsceneName(csObj, csWriteCustom), csObj, self.useMacros, False)
                    )
