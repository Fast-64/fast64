...
class OOT_SearchActorIDEnumOperator(bpy.types.Operator):
	...
	actorID : bpy.props.EnumProperty(items = ootData.actor.ootEnumActorIDLegacy, default = "ACTOR_PLAYER")
...
class OOTActorProperty(bpy.types.PropertyGroup):
	actorID : bpy.props.EnumProperty(name = 'Actor', items = ootData.actor.ootEnumActorIDLegacy, default = 'ACTOR_PLAYER')
...
def drawActorProperty(layout, actorProp, altRoomProp, objName):
	...
	split.label(text = getEnumName(ootData.actor.ootEnumActorIDLegacy, actorProp.actorID))
...
def drawTransitionActorProperty(layout, transActorProp, altSceneProp, roomObj, objName):
	...
	#actorIDBox.box().label(text = "Actor ID: " + getEnumName(ootData.actor.ootEnumActorIDLegacy, transActorProp.actor.actorID))
	searchOp = actorIDBox.operator(OOT_SearchActorIDEnumOperator.bl_idname, icon = 'VIEWZOOM')
	...
	split = actorIDBox.split(factor = 0.5)
	split.label(text = "Actor ID")
	split.label(text = getEnumName(ootData.actor.ootEnumActorIDLegacy, transActorProp.actor.actorID))
