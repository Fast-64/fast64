...
class OOTActorHeaderProperty(bpy.types.PropertyGroup):
    sceneSetupPreset: bpy.props.EnumProperty(
        name="Scene Setup Preset",
        items=ootEnumSceneSetupPreset,
        default="All Scene Setups",
    )
...
def drawActorHeaderProperty(layout, headerProp, propUser, altProp, objName):
    ...
    if headerProp.sceneSetupPreset == "Custom":
        ...
        for i in range(len(headerProp.cutsceneHeaders)):
            drawActorHeaderItemProperty(
                headerSetup,
                propUser,
                headerProp.cutsceneHeaders[i],
                i,
                altProp,
                objName,
            )
        drawAddButton(
            headerSetup, len(headerProp.cutsceneHeaders), propUser, None, objName
        )


def drawActorHeaderItemProperty(
    layout, propUser, headerItemProp, index, altProp, objName
):
    box = layout.box()
    box.prop(
        headerItemProp,
    ...
    if headerItemProp.expandTab:
        ...
        if (
            altProp is not None
            and headerItemProp.headerIndex >= len(altProp.cutsceneHeaders) + 4
        ):
            box.label(text="Header does not exist.", icon="QUESTION")


class OOTActorProperty(bpy.types.PropertyGroup):
    actorID: bpy.props.EnumProperty(
        name="Actor", items=ootEnumActorID, default="ACTOR_PLAYER"
    )
...
def drawActorProperty(layout, actorProp, altRoomProp, objName):
    ...
    searchOp = actorIDBox.operator(
        OOT_SearchActorIDEnumOperator.bl_idname, icon="VIEWZOOM"
    )
    ...
    actorIDBox.prop(
        actorProp, "rotOverride", text="Override Rotation (ignore Blender rot)"
    )
    ...
    drawActorHeaderProperty(
        actorIDBox, actorProp.headerSettings, "Actor", altRoomProp, objName
    )


class OOTTransitionActorProperty(bpy.types.PropertyGroup):
    roomIndex: bpy.props.IntProperty(min=0)
    cameraTransitionFront: bpy.props.EnumProperty(
        items=ootEnumCamTransition, default="0x00"
    )
    cameraTransitionFrontCustom: bpy.props.StringProperty(default="0x00")
    cameraTransitionBack: bpy.props.EnumProperty(
        items=ootEnumCamTransition, default="0x00"
    )
...
def drawTransitionActorProperty(layout, transActorProp, altSceneProp, roomObj, objName):
    ...
    if roomObj is None:
        actorIDBox.label(
            text="This must be part of a Room empty's hierarchy.", icon="OUTLINER"
        )
    else:
        label_split(
            actorIDBox, "Room To Transition From", str(roomObj.ootRoomHeader.roomIndex)
        )
    prop_split(actorIDBox, transActorProp, "roomIndex", "Room To Transition To")
    actorIDBox.label(
        text='Y+ side of door faces toward the "from" room.', icon="ORIENTATION_NORMAL"
    )
    drawEnumWithCustom(
        actorIDBox,
        transActorProp,
        "cameraTransitionFront",
        "Camera Transition Front",
        "",
    )
    drawEnumWithCustom(
        actorIDBox, transActorProp, "cameraTransitionBack", "Camera Transition Back", ""
    )
...
def drawEntranceProperty(layout, obj, altSceneProp, objName):
    ...
    drawActorHeaderProperty(
        box, entranceProp.actor.headerSettings, "Entrance", altSceneProp, objName
    )
