...
class OOTAnimation:
    ...
    def toC(self):
        ...
        # values
        data.source += (
            "s16 " + self.valuesName() + "[" + str(len(self.values)) + "] = {\n"
        )
        counter = 0
...
def ootGetAnimBoneRot(bone, poseBone, convertTransformMatrix, isRoot):
    ...
    if max(finalScale) >= 1.01 or min(finalScale) <= 0.99:
        raise RuntimeError(
            "Animation contains bones with animated scale. OoT SkelAnime does not support this."
        )
...
def ootExportAnimationCommon(armatureObj, convertTransformMatrix, skeletonName):
    ...
    skeleton = ootConvertArmatureToSkeletonWithoutMesh(
        armatureObj, convertTransformMatrix, skeletonName
    )
    ...
    for frameData in singleFrameData:
        ...
        if frame in ootAnim.values:
            ootAnim.indices[frameData.boneIndex][
                frameData.field
            ] = ootAnim.values.index(frame)
        else:
...
def exportAnimationC(armatureObj, exportPath, isCustomExport, folderName, skeletonName):
    ...
    ootAnim = ootExportAnimationCommon(
        armatureObj, convertTransformMatrix, skeletonName
    )
    ...
    for jointIndex in jointIndices:
        if isRootTranslation:
            for propertyIndex in range(3):
                ...
                if jointIndex[propertyIndex] < staticIndexMax:
                    ...
                    for frame in range(frameCount):
                        value = (
                            frameData[jointIndex[propertyIndex] + frame] / actorScale
                        )
            ...
            for propertyIndex in range(3):
                ...
                if jointIndex[propertyIndex] < staticIndexMax:
                    value = math.radians(
                        frameData[jointIndex[propertyIndex]] * 360 / (2 ** 16)
                    )
                    fcurve.keyframe_points.insert(0, value)
                else:
                    for frame in range(frameCount):
                        value = math.radians(
                            frameData[jointIndex[propertyIndex] + frame]
                            * 360
                            / (2 ** 16)
                        )
                        fcurve.keyframe_points.insert(frame, value)
...
def getFrameData(filepath, animData, frameDataName):
    matchResult = re.search(
        re.escape(frameDataName) + "\s*\[\s*[0-9]*\s*\]\s*=\s*\{([^\}]*)\}",
        animData,
        re.DOTALL,
    )
    if matchResult is None:
        raise PluginError(
            "Cannot find animation frame data named "
            + frameDataName
            + " in "
            + filepath
        )
    ...
    frameData = [
        int.from_bytes(
            [int(value.strip()[2:4], 16), int(value.strip()[4:6], 16)],
            "big",
            signed=True,
        )
...
def getJointIndices(filepath, animData, jointIndicesName):
    matchResult = re.search(
        re.escape(jointIndicesName) + "\s*\[\s*[0-9]*\s*\]\s*=\s*\{([^;]*);",
        animData,
        re.DOTALL,
    )
    if matchResult is None:
        raise PluginError(
            "Cannot find animation joint indices data named "
            + jointIndicesName
            + " in "
            + filepath
        )
...
class OOT_ExportAnim(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            exportAnimationC(
                armatureObj, path, isCustomExport, folderName, skeletonName
            )
...
def oot_anim_register():
    bpy.types.Scene.ootAnimIsCustomExport = bpy.props.BoolProperty(
        name="Use Custom Path"
    )
    bpy.types.Scene.ootAnimExportCustomPath = bpy.props.StringProperty(
        name="Folder", subtype="FILE_PATH"
    )
    bpy.types.Scene.ootAnimExportFolderName = bpy.props.StringProperty(
        name="Animation Folder", default="object_geldb"
    )

    bpy.types.Scene.ootAnimIsCustomImport = bpy.props.BoolProperty(
        name="Use Custom Path"
    )
    bpy.types.Scene.ootAnimImportCustomPath = bpy.props.StringProperty(
        name="Folder", subtype="FILE_PATH"
    )
    bpy.types.Scene.ootAnimImportFolderName = bpy.props.StringProperty(
        name="Animation Folder", default="object_geldb"
    )

    bpy.types.Scene.ootAnimSkeletonName = bpy.props.StringProperty(
        name="Skeleton Name", default="gGerudoRedSkel"
    )
    bpy.types.Scene.ootAnimName = bpy.props.StringProperty(
        name="Anim Name", default="gGerudoRedSpinAttackAnim"
    )
