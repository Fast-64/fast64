...
class OOT_CameraPosPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        return context.scene.gameEditorMode == "OOT" and isinstance(
            context.object.data, bpy.types.Camera
        )
    ...
    def draw(self, context):
        ...
        drawEnumWithCustom(
            box, obj.ootCameraPositionProperty, "camSType", "Camera S Type", ""
        )
...
class OOT_CollisionPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if collisionProp.expandTab:
            ...
            drawEnumWithCustom(
                box, collisionProp, "floorProperty", "Floor Property", ""
            )

            box.prop(
                collisionProp, "ignoreCameraCollision", text="Ignore Camera Collision"
            )
            box.prop(
                collisionProp, "ignoreActorCollision", text="Ignore Actor Collision"
            )
            box.prop(
                collisionProp,
                "ignoreProjectileCollision",
                text="Ignore Projectile Collision",
            )
            ...
            if collisionProp.conveyorOption != "None":
                ...
                drawEnumWithCustom(
                    box, collisionProp, "conveyorSpeed", "Conveyor Speed", ""
                )
                if collisionProp.conveyorSpeed != "Custom":
                    box.prop(
                        collisionProp, "conveyorKeepMomentum", text="Keep Momentum"
                    )
...
def exportCollisionCommon(collision, obj, transformMatrix, includeChildren, name):
    ...
    addCollisionTriangles(
        obj, collisionDict, includeChildren, transformMatrix, collision.bounds
    )
    for polygonType, faces in collisionDict.items():
        ...
        for (faceVerts, normal, distance) in faces:
            ...
            collision.polygonGroups[polygonType].append(
                OOTCollisionPolygon(indices, normal, distance)
            )
...
def exportCollisionToC(
    ...
    obj, allObjs = ootDuplicateHierarchy(
        originalObj, None, True, OOTObjectCategorizer()
    )
...
def addCollisionTriangles(obj, collisionDict, includeChildren, transformMatrix, bounds):
    if isinstance(obj.data, bpy.types.Mesh) and not obj.ignore_collision:
        ...
        for face in obj.data.loop_triangles:
            ...
            faceNormal = (
                transformMatrix.inverted().transposed() @ face.normal
            ).normalized()
            ...
            positions = sorted(
                ((x1, y1, z1), (x2, y2, z2), (x3, y3, z3)), key=lambda x: x[1]
            )  # sort by min y to avoid minY bug
...
def ootCollisionPolygonToC(
    ...
    return (
        ...
        + ", "
        + format(
            polygon.convertShort02(ignoreCamera, ignoreActor, ignoreProjectile), "#06x"
        )
        + ", "
...
def ootPolygonTypeToC(polygonType):
    return (
        "{ "
        + format(polygonType.convertHigh(), "#010x")
        + ", "
...
def ootCameraDataToC(camData):
    ...
    if len(camData.camPosDict) > 0:
        ...
        for i in range(len(camData.camPosDict)):
            camC.source += "\t" + ootCameraEntryToC(
                camData.camPosDict[i], camData, camPosIndex
            )
        ...
        posDataName = (
            "Vec3s " + camData.camPositionsName() + "[" + str(camPosIndex) + "]"
        )
...
def oot_col_register():
    ...
    bpy.types.Object.ootCameraPositionProperty = bpy.props.PointerProperty(
        type=OOTCameraPositionProperty
    )
    bpy.types.Material.ootCollisionProperty = bpy.props.PointerProperty(
        type=OOTMaterialCollisionProperty
    )
