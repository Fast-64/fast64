...
ootEnumFloorProperty = [
    ...
    (
        "0x07",
        "Quicksand Crossing (Epona Uncrossable)",
        "Quicksand Crossing (Epona Uncrossable)",
    ),
    ...
    (
        "0x0B",
        "Quicksand Crossing (Epona Crossable)",
        "Quicksand Crossing (Epona Crossable)",
    ),
]
...
def getPolygonType(collisionProp):
    ...
    if collisionProp.conveyorOption != "None":
        polygonType.conveyorRotation = int(
            collisionProp.conveyorRotation / (2 * math.pi) * 0x3F
        )
        polygonType.conveyorSpeed = int(
            getCustomProperty(collisionProp, "conveyorSpeed"), 16
...
class OOTWaterBox(BoxEmpty):
    def __init__(
        self, roomIndex, lightingSetting, cameraSetting, position, scale, emptyScale
    ):
