...
class OOTCSProperty:
    ...
    def draw(self, layout, listProp, listIndex, cmdIndex, objName, collectionType):
        ...
        drawCollectionOps(
            box, cmdIndex, collectionType + "." + self.attrName, listIndex, objName
        )
...
def drawCSListProperty(layout, listProp, listIndex, objName, collectionType):
    ...
    if listProp.listType == "Textbox":
        ...
        addOp = box.operator(
            OOTCollectionAdd.bl_idname,
            text="Add item to " + listProp.listType + " List",
        )
...
def drawCSAddButtons(layout, objName, collectionType):
    def addButton(row):
        nonlocal l
        op = row.operator(
            OOTCSListAdd.bl_idname,
            text=ootEnumCSListType[l][1],
            icon=ootEnumCSListTypeIcons[l],
        )
    ...
    for list in csParent.csLists:
        ...
        for e in list.entries:
            ...
            if list.listType == "Textbox":
                ...
                raise PluginError(
                    "Internal error: invalid cutscene list type " + list.listType
                )
            data.source += "),\n"
    data.source += "\tCS_END(),\n"
    data.source += "};\n\n"
...
class OOT_ExportCutscene(bpy.types.Operator):
    ...
    def execute(self, context):
        try:
            ...
            if (
                activeObj is None
                or activeObj.data is not None
                or activeObj.ootEmptyType != "Cutscene"
            ):
        ...
        try:
            ...
            for obj in context.view_layer.objects:
                ...
                if obj.parent is not None:
                    raise PluginError(
                        "Cutscene object must not be parented to anything"
                    )
