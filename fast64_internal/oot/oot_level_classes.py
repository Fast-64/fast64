...
class OOTCSUnk:
    def __unk__(self):
        self.unk1 = (
            ...
            self.unk7
        ) = self.unk8 = self.unk9 = self.unk10 = self.unk11 = self.unk12 = "0x00000000"
...
class OOTScene:
    ...
    def entranceListName(self, headerIndex):
        return (
            self.sceneName() + "_header" + format(headerIndex, "02") + "_entranceList"
        )
    ...
    def lightListName(self, headerIndex):
        return (
            self.sceneName() + "_header" + format(headerIndex, "02") + "_lightSettings"
        )
    ...
    def validateRoomIndices(self):
        ...
        while count < len(self.rooms):
            if count not in self.rooms:
                raise PluginError(
                    "Error: Room indices do not have a consecutive list of indices. "
                    + "Missing index: "
                    + str(count)
                )
            count = count + 1
    ...
    def addRoom(self, roomIndex, roomName, meshType):
        roomModel = self.model.addSubModel(
            OOTModel(
                self.model.f3d.F3D_VER,
                self.model.f3d._HW_VERSION_1,
                roomName + "_dl",
                self.model.DLFormat,
                None,
            )
        ...
        if roomIndex in self.rooms:
            raise PluginError(
                "Repeat room index " + str(roomIndex) + " for " + str(roomName)
            )
...
class OOTRoomMesh:
    ...
    def addMeshGroup(self, cullGroup):
        meshGroup = OOTRoomMeshGroup(
            cullGroup, self.model.DLFormat, self.roomName, len(self.meshEntries)
        )
    ...
    def addDLCall(self, displayList, drawLayer):
        if drawLayer == "Opaque":
            if self.opaque is None:
                self.opaque = GfxList(
                    self.name + "_opaque", GfxListTag.Draw, self.DLFormat
                )
            self.opaque.commands.append(SPDisplayList(displayList))
        elif drawLayer == "Transparent":
            if self.transparent is None:
                self.transparent = GfxList(
                    self.name + "_transparent", GfxListTag.Draw, self.DLFormat
                )
...
def addActor(owner, actor, actorProp, propName, actorObjName):
    ...
    if (
        ...
        for cutsceneHeader in sceneSetup.cutsceneHeaders:
            ...
            getattr(
                owner.cutsceneHeaders[cutsceneHeader.headerIndex - 4], propName
            ).add(actor)
    else:
        raise PluginError(
            "Unhandled scene setup preset: " + str(sceneSetup.sceneSetupPreset)
        )
...
def addStartPosition(scene, index, actor, actorProp, actorObjName):
    ...
    if (
        ...
        for cutsceneHeader in sceneSetup.cutsceneHeaders:
            ...
            addAtStartPosIndex(
                scene.cutsceneHeaders[cutsceneHeader.headerIndex - 4].startPositions,
                index,
                actor,
            )
    else:
