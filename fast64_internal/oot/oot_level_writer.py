...
def ootExportSceneToC(
    ...
    if not isCustomExport and exportInfo.customSubPath is None:
        ...
        if exportSubdir == "":
            raise PluginError(
                "Scene folder "
                + sceneName
                + " cannot be found in the ootSceneDirs list."
            )

    levelPath = ootGetPath(
        exportPath, isCustomExport, exportSubdir, sceneName, True, True
    )
    levelC = ootLevelToC(
        scene,
        TextureExportSettings(False, savePNG, exportSubdir + sceneName, levelPath),
    )
    ...
    if bpy.context.scene.ootSceneSingleFile:
        ...
        for i in range(len(scene.rooms)):
            ...
            writeCDataSourceOnly(
                ootPreprendSceneIncludes(scene, roomC),
                os.path.join(levelPath, scene.rooms[i].roomName() + ".c"),
            )
    else:
        # Export the scene segment .c files
        writeCDataSourceOnly(
            ootPreprendSceneIncludes(scene, levelC.sceneMainC),
        ...
        for roomName, roomMainC in levelC.roomMainC.items():
            writeCDataSourceOnly(
                ootPreprendSceneIncludes(scene, roomMainC),
                os.path.join(levelPath, roomName + "_main.c"),
            )
        for roomName, roomMeshInfoC in levelC.roomMeshInfoC.items():
            writeCDataSourceOnly(
                ootPreprendSceneIncludes(scene, roomMeshInfoC),
                os.path.join(levelPath, roomName + "_model_info.c"),
            )
        for roomName, roomMeshC in levelC.roomMeshC.items():
            writeCDataSourceOnly(
                ootPreprendSceneIncludes(scene, roomMeshC),
                os.path.join(levelPath, roomName + "_model.c"),
            )

    # Export the scene .h file
    writeCDataHeaderOnly(
        ootCreateSceneHeader(levelC), os.path.join(levelPath, scene.sceneName() + ".h")
    )
...
def readSceneData(scene, scene_properties, sceneHeader, alternateSceneHeaders):
    ...
    if scene.writeCutscene:
        ...
        if scene.csWriteType == "Embedded":
            ...
            scene.csWriteTerminator = getCustomProperty(
                sceneHeader, "csWriteTerminator"
            )
            ...
            if sceneHeader.csWriteObject is None:
                ...
                raise PluginError(
                    "Object selected as cutscene is wrong type, must be empty with Cutscene type"
                )
            elif sceneHeader.csWriteObject.parent is not None:
                raise PluginError(
                    "Cutscene empty object should not be parented to anything"
                )
            else:
    ...
    if alternateSceneHeaders is not None:
        ...
        if not alternateSceneHeaders.childNightHeader.usePreviousHeader:
            ...
            readSceneData(
                scene.childNightHeader,
                scene_properties,
                alternateSceneHeaders.childNightHeader,
                None,
            )
        ...
        if not alternateSceneHeaders.adultDayHeader.usePreviousHeader:
            ...
            readSceneData(
                scene.adultDayHeader,
                scene_properties,
                alternateSceneHeaders.adultDayHeader,
                None,
            )
        ...
        if not alternateSceneHeaders.adultNightHeader.usePreviousHeader:
            ...
            readSceneData(
                scene.adultNightHeader,
                scene_properties,
                alternateSceneHeaders.adultNightHeader,
                None,
            )
...
def getConvertedTransform(transformMatrix, sceneObj, obj, handleOrientation):
    ...
    return getConvertedTransformWithOrientation(
...
def getConvertedTransformWithOrientation(transformMatrix, sceneObj, obj, orientation):
    relativeTransform = (
        transformMatrix @ sceneObj.matrix_world.inverted() @ obj.matrix_world
    )
...
def getLightData(lightProp):
    ...
    if lightProp.useCustomDiffuse0:
        if lightProp.diffuse0Custom is None:
            raise PluginError(
                "Error: Diffuse 0 light object not set in a scene lighting property."
            )
    ...
    if lightProp.useCustomDiffuse1:
        if lightProp.diffuse1Custom is None:
            raise PluginError(
                "Error: Diffuse 1 light object not set in a scene lighting property."
            )
...
def readRoomData(room, roomHeader, alternateRoomHeaders):
    ...
    room.windVector = normToSigned8Vector(
        mathutils.Vector(roomHeader.windVector).normalized()
    )
    room.windStrength = int(
        0xFF * max(mathutils.Vector(roomHeader.windVector).length, 1)
    )
    ...
    room.objectList.extend(
        [getCustomProperty(item, "objectID") for item in roomHeader.objectList]
    )
    if len(room.objectList) > 15:
        raise PluginError(
            "Error: A scene can only have a maximum of 15 objects (OOT, not blender objects)."
        )
    ...
    if alternateRoomHeaders is not None:
        if not alternateRoomHeaders.childNightHeader.usePreviousHeader:
            ...
            readRoomData(
                room.childNightHeader, alternateRoomHeaders.childNightHeader, None
            )
        ...
        if not alternateRoomHeaders.adultNightHeader.usePreviousHeader:
            ...
            readRoomData(
                room.adultNightHeader, alternateRoomHeaders.adultNightHeader, None
            )
...
def readCamPos(camPosProp, obj, scene, sceneObj, transformMatrix):
    ...
    (
        ...
        scale,
        orientedRotation,
    ) = getConvertedTransformWithOrientation(
        transformMatrix, sceneObj, obj, orientation
    )
    ...
    scene.collision.cameraData.camPosDict[index] = OOTCameraPosData(
        getCustomProperty(camPosProp, "camSType"),
...
def readPathProp(pathProp, obj, scene, sceneObj, sceneName, transformMatrix):
    ...
    if index in scene.pathList:
        raise PluginError(
            "Error: " + obj.name + "has a repeated spline index: " + str(index)
        )
...
def ootConvertScene(
    ...
    if originalSceneObj.data is not None or originalSceneObj.ootEmptyType != "Scene":
        raise PluginError(
            originalSceneObj.name + ' is not an empty with the "Scene" empty type.'
        )
    ...
    sceneObj, allObjs = ootDuplicateHierarchy(
        originalSceneObj, None, True, OOTObjectCategorizer()
    )
    ...
    roomObjs = [
        child
        for child in sceneObj.children
        if child.data is None and child.ootEmptyType == "Room"
    ]
    if len(roomObjs) == 0:
    ...
    try:
        scene = OOTScene(
            sceneName, OOTModel(f3dType, isHWv1, sceneName + "_dl", DLFormat, None)
        )
        readSceneData(
            scene,
            sceneObj.fast64.oot.scene,
            sceneObj.ootSceneHeader,
            sceneObj.ootAlternateSceneHeaders,
        )
        processedRooms = set()

        for obj in sceneObj.children:
            translation, rotation, scale, orientedRotation = getConvertedTransform(
                transformMatrix, sceneObj, obj, True
            )
            ...
            if obj.data is None and obj.ootEmptyType == "Room":
                ...
                room = scene.addRoom(
                    roomIndex, sceneName, roomObj.ootRoomHeader.meshType
                )
                readRoomData(
                    room, roomObj.ootRoomHeader, roomObj.ootAlternateRoomHeaders
                )
        ...
        scene.entranceList = sorted(
            scene.entranceList, key=lambda x: x.startPositionIndex
        )
        exportCollisionCommon(
            scene.collision, sceneObj, transformMatrix, True, sceneName
        )
...
def ootProcessMesh(
    ...
    if obj.data is None and obj.ootEmptyType == "Cull Group":
        ...
        triConverterInfo = TriangleConverterInfo(
            obj, None, roomMesh.model.f3d, relativeTransform, getInfoDict(obj)
        )
...
def ootProcessLOD(
    ...
    opaqueLOD = roomMesh.model.addLODGroup(
    ...
    for childObj in obj.children:
        ...
        opaqueLOD.add_lod(
            childDLGroup.opaque, childObj.f3d_lod_z * bpy.context.scene.ootBlenderScale
        )
        transparentLOD.add_lod(
            childDLGroup.transparent,
            childObj.f3d_lod_z * bpy.context.scene.ootBlenderScale,
        )
...
def ootProcessEmpties(scene, room, sceneObj, obj, transformMatrix):
    ...
    if obj.data is None:
        if obj.ootEmptyType == "Actor":
            ...
            addActor(
                ...
                OOTActor(
                    ...
                    if not actorProp.rotOverride
                        ...
                        actorProp.rotOverrideX,
                        actorProp.rotOverrideY,
                        actorProp.rotOverrideZ,
                    ),
                ),
                actorProp,
                "actorList",
            ...
            addActor(
                scene,
                OOTEntrance(room.roomIndex, spawnIndex),
                entranceProp.actor,
                "entranceList",
                obj.name,
            )
            ...
            addStartPosition(
                ...
                OOTActor(
                    "ACTOR_PLAYER"
                    if not entranceProp.customActor
                    else entranceProp.actor.actorIDCustom,
                    translation,
                    rotation,
        ...
        readPathProp(
            obj.ootSplineProperty, obj, scene, sceneObj, scene.name, transformMatrix
        )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if obj is None:
                ...
                raise PluginError(
                    "The input object is not an empty with the Scene type."
                )
        ...
        try:
            ...
            if context.scene.ootSceneCustomExport:
                exportInfo = ExportInfo(
                    True,
                    bpy.path.abspath(context.scene.ootSceneExportPath),
                    None,
                    levelName,
                )
            else:
                ...
                exportInfo = ExportInfo(
                    False,
                    bpy.path.abspath(context.scene.ootDecompPath),
                    subfolder,
                    levelName,
                )
...
class OOT_RemoveScene(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.ootSceneCustomExport:
            operator.report(
                {"ERROR"}, "You can only remove scenes from your decomp path."
            )
            return {"FINISHED"}
        ...
        if context.scene.ootSceneOption == "Custom":
            ...
            subfolder = None
        exportInfo = ExportInfo(
            False, bpy.path.abspath(context.scene.ootDecompPath), subfolder, levelName
        )
    ...
    def draw(self, context):
        ...
        if context.scene.ootSceneCustomExport:
            ...
            col.box().column().label(
                text=getEnumName(ootEnumSceneID, context.scene.ootSceneOption)
            )
...
def oot_level_register():
    ...
    bpy.types.Scene.ootSceneName = bpy.props.StringProperty(
        name="Name", default="spot03"
    )
    bpy.types.Scene.ootSceneSubFolder = bpy.props.StringProperty(
        name="Subfolder", default="overworld"
    )
    bpy.types.Scene.ootSceneOption = bpy.props.EnumProperty(
        name="Scene", items=ootEnumSceneID, default="SCENE_YDAN"
    )
    ...
    bpy.types.Scene.ootSceneExportObj = bpy.props.PointerProperty(
        type=bpy.types.Object, poll=isSceneObj
    )
