...
class OOTModel(FModel):
    def __init__(self, f3dType, isHWv1, name, DLFormat, drawLayerOverride):
        ...
        FModel.__init__(
            self, f3dType, isHWv1, name, DLFormat, GfxMatWriteMethod.WriteAll
        )
        ...
        if contextObj is not None and hasattr(contextObj, "ootDynamicTransform"):
            if contextObj.ootDynamicTransform.billboard:
                fMesh.draw.commands.append(
                    SPMatrix("0x01000000", "G_MTX_MODELVIEW | G_MTX_NOPUSH | G_MTX_MUL")
                )
...
class OOTTriangleConverterInfo(TriangleConverterInfo):
    def __init__(self, obj, armature, f3d, transformMatrix, infoDict):
        TriangleConverterInfo.__init__(
            self, obj, armature, f3d, transformMatrix, infoDict
        )

    def getMatrixAddrFromGroup(self, groupIndex):
        return format(
            (0x0D << 24)
            + MTX_SIZE * self.vertexGroupInfo.vertexGroupToMatrixIndex[groupIndex],
            "#010x",
        )
...
class OOTF3DContext(F3DContext):
    ...
    def setCurrentTransform(self, name):
        if name[:4].lower() == "0x0d":
            self.currentTransformName = self.getLimbName(
                self.dlList[int(int(name[4:], 16) / MTX_SIZE)].limbIndex
            )
        else:
            try:
    ...
    def processDLName(self, name):
        ...
        try:
            ...
            if segment >= 0x08 and segment <= 0x0D:
                ...
                setattr(
                    self.materialContext.ootMaterial.opaque,
                    "segment" + format(segment, "1X"),
                    True,
                )
                setattr(
                    self.materialContext.ootMaterial.transparent,
                    "segment" + format(segment, "1X"),
                    True,
                )
            return None
