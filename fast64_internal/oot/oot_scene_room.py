...
class OOT_SearchMusicSeqEnumOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.headerIndex == 0:
            ...
            sceneHeader = obj.ootAlternateSceneHeaders.cutsceneHeaders[
                self.headerIndex - 4
            ]
...
class OOT_SearchObjectEnumOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.headerIndex == 0:
            ...
            roomHeader = obj.ootAlternateRoomHeaders.cutsceneHeaders[
                self.headerIndex - 4
            ]
...
def drawAlternateRoomHeaderProperty(layout, headerProp, objName):
    ...
    if headerProp.headerMenuTab == "Child Night":
        drawRoomHeaderProperty(
            headerSetupBox, headerProp.childNightHeader, None, 1, objName
        )
    elif headerProp.headerMenuTab == "Adult Day":
        drawRoomHeaderProperty(
            headerSetupBox, headerProp.adultDayHeader, None, 2, objName
        )
    elif headerProp.headerMenuTab == "Adult Night":
        drawRoomHeaderProperty(
            headerSetupBox, headerProp.adultNightHeader, None, 3, objName
        )
    elif headerProp.headerMenuTab == "Cutscene":
        prop_split(headerSetup, headerProp, "currentCutsceneIndex", "Cutscene Index")
        drawAddButton(
            headerSetup, len(headerProp.cutsceneHeaders), "Room", None, objName
        )
        index = headerProp.currentCutsceneIndex
        if index - 4 < len(headerProp.cutsceneHeaders):
            drawRoomHeaderProperty(
                headerSetup, headerProp.cutsceneHeaders[index - 4], None, index, objName
            )
        else:
            headerSetup.label(
                text="No cutscene header for this index.", icon="QUESTION"
            )
...
def drawExitProperty(layout, exitProp, index, headerIndex, objName):
    ...
    box.prop(
        exitProp,
        "expandTab",
        text="Exit " + str(index + 1),
        icon="TRIA_DOWN" if exitProp.expandTab else "TRIA_RIGHT",
    )
    if exitProp.expandTab:
        ...
        if exitProp.exitIndex != "Custom":
            ...
            drawEnumWithCustom(
                exitGroup, exitProp, "fadeInAnim", "Fade In Animation", ""
            )
            drawEnumWithCustom(
                exitGroup, exitProp, "fadeOutAnim", "Fade Out Animation", ""
            )
...
def drawObjectProperty(layout, objectProp, headerIndex, index, objName):
    ...
    objItemBox.prop(
        ...
        "expandTab",
        text=objectName,
        icon="TRIA_DOWN" if objectProp.expandTab else "TRIA_RIGHT",
    )
    if objectProp.expandTab:
        drawCollectionOps(objItemBox, index, "Object", headerIndex, objName)

        objSearch = objItemBox.operator(
            OOT_SearchObjectEnumOperator.bl_idname, icon="VIEWZOOM"
        )
...
def drawLightProperty(
    layout, lightProp, name, showExpandTab, index, sceneHeaderIndex, objName
):
    if showExpandTab:
        ...
        box.prop(
            ...
            "expandTab",
            text=name,
            icon="TRIA_DOWN" if lightProp.expandTab else "TRIA_RIGHT",
        )
    ...
    if expandTab:
        ...
        if lightProp.useCustomDiffuse0:
            ...
            box.label(
                text="Make sure light is not part of scene hierarchy.",
                icon="FILE_PARENT",
            )
        else:
...
class OOTSceneHeaderProperty(bpy.types.PropertyGroup):
    ...
    globalObject: bpy.props.EnumProperty(
        name="Global Object", default="0x0002", items=ootEnumGlobalObject
    )
    globalObjectCustom: bpy.props.StringProperty(
        name="Global Object Custom", default="0x00"
    )
    naviCup: bpy.props.EnumProperty(
        name="Navi Hints", default="0x00", items=ootEnumNaviHints
    )
    ...
    skyboxCloudiness: bpy.props.EnumProperty(
        name="Cloudiness", items=ootEnumCloudiness, default="0x00"
    )
    skyboxCloudinessCustom: bpy.props.StringProperty(
        name="Cloudiness ID", default="0x00"
    )
    skyboxLighting: bpy.props.EnumProperty(
        name="Skybox Lighting", items=ootEnumSkyboxLighting, default="0x00"
    )
    skyboxLightingCustom: bpy.props.StringProperty(
        name="Skybox Lighting Custom", default="0x00"
    )

    mapLocation: bpy.props.EnumProperty(
        name="Map Location", items=ootEnumMapLocation, default="0x00"
    )
    mapLocationCustom: bpy.props.StringProperty(
        name="Skybox Lighting Custom", default="0x00"
    )
    cameraMode: bpy.props.EnumProperty(
        name="Camera Mode", items=ootEnumCameraMode, default="0x00"
    )
    cameraModeCustom: bpy.props.StringProperty(
        name="Camera Mode Custom", default="0x00"
    )

    musicSeq: bpy.props.EnumProperty(
        name="Music Sequence", items=ootEnumMusicSeq, default="0x02"
    )
    musicSeqCustom: bpy.props.StringProperty(name="Music Sequence ID", default="0x00")
    nightSeq: bpy.props.EnumProperty(
        name="Nighttime SFX", items=ootEnumNightSeq, default="0x00"
    )
    ...
    audioSessionPresetCustom: bpy.props.StringProperty(
        name="Audio Session Preset", default="0x00"
    )

    timeOfDayLights: bpy.props.PointerProperty(
        type=OOTLightGroupProperty, name="Time Of Day Lighting"
    )
    ...
    csWriteType: bpy.props.EnumProperty(
        name="Cutscene Data Type", items=ootEnumCSWriteType, default="Embedded"
    )
    csWriteCustom: bpy.props.StringProperty(name="CS hdr var:", default="")
    csWriteObject: bpy.props.PointerProperty(
        name="Cutscene Object", type=bpy.types.Object
    )
    ...
    extraCutscenes: bpy.props.CollectionProperty(
        type=OOTExtraCutsceneProperty, name="Extra Cutscenes"
    )
...
def drawSceneHeaderProperty(layout, sceneProp, dropdownLabel, headerIndex, objName):
    ...
    if menuTab == "General":
        ...
        drawEnumWithCustom(
            skyboxAndSound, sceneProp, "skyboxCloudiness", "Cloudiness", ""
        )
        drawEnumWithCustom(skyboxAndSound, sceneProp, "musicSeq", "Music Sequence", "")
        musicSearch = skyboxAndSound.operator(
            OOT_SearchMusicSeqEnumOperator.bl_idname, icon="VIEWZOOM"
        )
        ...
        drawEnumWithCustom(
            skyboxAndSound, sceneProp, "audioSessionPreset", "Audio Session Preset", ""
        )

        cameraAndWorldMap = layout.column()
        cameraAndWorldMap.box().label(text="Camera And World Map")
        drawEnumWithCustom(
            cameraAndWorldMap, sceneProp, "mapLocation", "Map Location", ""
        )
        drawEnumWithCustom(
            cameraAndWorldMap, sceneProp, "cameraMode", "Camera Mode", ""
        )
        ...
        if sceneProp.skyboxLighting == "0x00":  # Time of Day
            ...
            drawAddButton(
                lighting, len(sceneProp.lightList), "Light", headerIndex, objName
            )
        ...
        if sceneProp.writeCutscene:
            ...
            if sceneProp.csWriteType == "Custom":
                ...
                # of cutscene data.
                cutscene.label(
                    text='Embedded cutscenes are deprecated. Please use "Object" instead.'
                )
                ...
                collectionType = "CSHdr." + str(
                    0 if headerIndex is None else headerIndex
                )
        ...
        if headerIndex is None or headerIndex == 0:
            ...
            for i in range(len(sceneProp.exitList)):
                drawExitProperty(
                    exitBox, sceneProp.exitList[i], i, headerIndex, objName
                )

            drawAddButton(
                exitBox, len(sceneProp.exitList), "Exit", headerIndex, objName
            )
        else:
...
class OOTRoomHeaderProperty(bpy.types.PropertyGroup):
    ...
    linkIdleMode: bpy.props.EnumProperty(
        name="Link Idle Mode", items=ootEnumLinkIdle, default="0x00"
    )
    linkIdleModeCustom: bpy.props.StringProperty(
        name="Link Idle Mode Custom", default="0x00"
    )
    ...
    customBehaviourX: bpy.props.StringProperty(
        name="Custom Behaviour X", default="0x00"
    )

    customBehaviourY: bpy.props.StringProperty(
        name="Custom Behaviour Y", default="0x00"
    )

    setWind: bpy.props.BoolProperty(name="Set Wind")
    windVector: bpy.props.FloatVectorProperty(name="Wind Vector", size=3)

    leaveTimeUnchanged: bpy.props.BoolProperty(
        name="Leave Time Unchanged", default=True
    )
    ...
    timeSpeed: bpy.props.FloatProperty(
        name="Time Speed", default=1, min=-13, max=13
    )  # 0xA
    ...
    defaultCullDistance: bpy.props.IntProperty(
        name="Default Cull Distance", min=1, default=100
    )
...
def drawRoomHeaderProperty(layout, roomProp, dropdownLabel, headerIndex, objName):
    ...
    if menuTab == "General":
        ...
        drawEnumWithCustom(
            behaviourBox, roomProp, "roomBehaviour", "Room Behaviour", ""
        )
...
class OOTAlternateSceneHeaderProperty(bpy.types.PropertyGroup):
    childNightHeader: bpy.props.PointerProperty(
        name="Child Night Header", type=OOTSceneHeaderProperty
    )
    adultDayHeader: bpy.props.PointerProperty(
        name="Adult Day Header", type=OOTSceneHeaderProperty
    )
    adultNightHeader: bpy.props.PointerProperty(
        name="Adult Night Header", type=OOTSceneHeaderProperty
    )
...
def drawAlternateSceneHeaderProperty(layout, headerProp, objName):
    ...
    if headerProp.headerMenuTab == "Child Night":
        ...
        prop_split(headerSetup, headerProp, "currentCutsceneIndex", "Cutscene Index")
        drawAddButton(
            headerSetup, len(headerProp.cutsceneHeaders), "Scene", None, objName
        )
        index = headerProp.currentCutsceneIndex
        if index - 4 < len(headerProp.cutsceneHeaders):
            drawSceneHeaderProperty(
...
class OOTAlternateRoomHeaderProperty(bpy.types.PropertyGroup):
    childNightHeader: bpy.props.PointerProperty(
        name="Child Night Header", type=OOTRoomHeaderProperty
    )
    adultDayHeader: bpy.props.PointerProperty(
        name="Adult Day Header", type=OOTRoomHeaderProperty
    )
    adultNightHeader: bpy.props.PointerProperty(
        name="Adult Night Header", type=OOTRoomHeaderProperty
    )
...
def drawParentSceneRoom(box, obj):
    ...
    if obj.ootObjectMenu == "Scene":
        if sceneObj is not None:
            drawSceneHeaderProperty(
                box, sceneObj.ootSceneHeader, None, None, sceneObj.name
            )
            if sceneObj.ootSceneHeader.menuTab == "Alternate":
                drawAlternateSceneHeaderProperty(
                    box, sceneObj.ootAlternateSceneHeaders, sceneObj.name
                )
        else:
            box.label(
                text="This object is not part of any Scene hierarchy.", icon="OUTLINER"
            )
        ...
        if roomObj is not None:
            ...
            if roomObj.ootRoomHeader.menuTab == "Alternate":
                drawAlternateRoomHeaderProperty(
                    box, roomObj.ootAlternateRoomHeaders, roomObj.name
                )
        else:
            box.label(
                text="This object is not part of any Room hierarchy.", icon="OUTLINER"
            )
