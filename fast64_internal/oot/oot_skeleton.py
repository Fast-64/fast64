...
class OOTSkeleton:
    ...
    def toC(self):
        ...
        data.source += (
            "void* " + self.limbsName() + "[" + str(self.getNumLimbs()) + "] = {\n"
        )
...
def ootDuplicateArmature(originalArmatureObj):
    ...
    for originalMeshObj in [
        obj
        for obj in originalArmatureObj.children
...
def ootConvertArmatureToSkeletonWithoutMesh(
    originalArmatureObj, convertTransformMatrix, name
):
...
def ootConvertArmatureToSkeleton(
    ...
    try:
        ...
        getGroupIndices(
            ...
            armatureObj,
            meshObj,
            getGroupIndexFromname(meshObj, startBoneName),
        )
...
def ootProcessBone(
    ...
    if isinstance(parentLimb, OOTSkeleton):
        ...
        limb = OOTLimb(
            parentLimb.skeletonName, boneName, nextIndex, translate, DL, None
        )
...
def ootConvertArmatureToC(
    ...
    exportData = fModel.to_c(
        TextureExportSettings(False, savePNG, "test"),
        OOTGfxFormatter(ScrollMethod.Vertex),
    )
    ...
    path = ootGetPath(
        exportPath, isCustomExport, "assets/objects/", folderName, False, False
    )
    writeCData(
        data,
...
def ootGetLimb(skeletonData, limbName, continueOnError):
    matchResult = re.search(
        "([A-Za-z0-9\_]*)Limb\s*" + re.escape(limbName), skeletonData
    )
    ...
    if matchResult is None:
        if continueOnError:
            ...
            raise PluginError(
                "Cannot handle skeleton limb named " + limbName + " of type " + limbType
            )
    return matchResult
...
def ootBuildSkeleton(
    ...
    isLOD = ootAddLimbRecursively(
        0, skeletonData, obj, armatureObj, transformMatrix, None, f3dContext, useFarLOD
    )
...
def ootAddBone(armatureObj, boneName, parentBoneName, currentTransform, loadDL):
    ...
    bone.tail = bone.head + (
        currentTransform.to_quaternion() @ mathutils.Vector((0, 0.3, 0))
    )
    ...
    if parentBoneName is not None:
        ...
        if (
            nodeOffsetVector.angle(bone.parent.tail - bone.parent.head, 1) < 0.0001
            and loadDL
        ):
            ...
            bone.tail += currentTransform.to_quaternion() @ mathutils.Vector(
                (0, 0.2, 0)
            )
...
def ootAddLimbRecursively(
    ...
    currentTransform = parentTransform @ mathutils.Matrix.Translation(
        mathutils.Vector(translation)
    )
...
def ootRemoveSkeleton(filepath, objectName, skeletonName):
    ...
    if matchResult is None:
        return
    skeletonDataC = (
        skeletonDataC[: matchResult.start(0)] + skeletonDataC[matchResult.end(0) :]
    )
    limbsName = matchResult.group(2)
    ...
    if headerMatch is not None:
        skeletonDataH = (
            skeletonDataH[: headerMatch.start(0)] + skeletonDataH[headerMatch.end(0) :]
        )
    ...
    for limb in limbList:
        ...
        if matchResult is not None:
            skeletonDataC = (
                skeletonDataC[: matchResult.start(0)]
                + skeletonDataC[matchResult.end(0) :]
            )
        headerMatch = getDeclaration(skeletonDataH, limb)
        if headerMatch is not None:
            skeletonDataH = (
                skeletonDataH[: headerMatch.start(0)]
                + skeletonDataH[headerMatch.end(0) :]
            )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if not isCustomImport:
                filepaths.append(
                    os.path.join(
                        bpy.context.scene.ootDecompPath,
...
class OOT_ExportSkeletonPanel(OOT_Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.ootSkeletonExportOptimize:
            b = col.box().column()
            b.label(
                icon="LIBRARY_DATA_BROKEN", text="Do not draw anything in SkelAnime"
            )
...
class OOT_BonePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (
            context.bone.ootBoneType == "Custom DL"
            or context.bone.ootBoneType == "Ignore"
        ):
            col.label(
                text="Make sure no geometry is skinned to this bone.", icon="BONE_DATA"
            )
...
def oot_skeleton_register():
    ...
    bpy.types.Scene.ootActorRemoveDoubles = bpy.props.BoolProperty(
        name="Remove Doubles On Import", default=True
    )
    bpy.types.Scene.ootActorImportNormals = bpy.props.BoolProperty(
    ...
    bpy.types.Scene.ootActorImportDrawLayer = bpy.props.EnumProperty(
        name="Import Draw Layer", items=ootEnumDrawLayers
    )

    bpy.types.Object.ootFarLOD = bpy.props.PointerProperty(
        type=bpy.types.Object, poll=pollArmature
    )

    bpy.types.Bone.ootBoneType = bpy.props.EnumProperty(
        name="Bone Type", items=ootEnumBoneType
    )
    bpy.types.Bone.ootDynamicTransform = bpy.props.PointerProperty(
        type=OOTDynamicTransformProperty
    )
    bpy.types.Bone.ootCustomDLName = bpy.props.StringProperty(
        name="Custom DL", default="gEmptyDL"
    )
