...
def ootSelectMeshChildrenOnly(obj, includeEmpties):
    ...
    isEmpty = (
        ...
        or isinstance(obj.data, bpy.types.Curve)
    ) and includeEmpties
...
def getSceneObj(obj):
    while not (
        obj is None
        or (obj is not None and obj.data is None and obj.ootEmptyType == "Scene")
    ):
        obj = obj.parent
    if obj is None:
        return None
...
def getRoomObj(obj):
    while not (
        obj is None
        or (obj is not None and obj.data is None and obj.ootEmptyType == "Room")
    ):
        obj = obj.parent
    if obj is None:
        return None
...
def ootGetPath(
    exportPath, isCustomExport, subPath, folderName, makeIfNotExists, useFolderForCustom
):
    if isCustomExport:
        path = bpy.path.abspath(
            os.path.join(exportPath, (folderName if useFolderForCustom else ""))
        )
    else:
        if bpy.context.scene.ootDecompPath == "":
            raise PluginError("Decomp base path is empty.")
        path = bpy.path.abspath(
            os.path.join(bpy.context.scene.ootDecompPath, subPath + folderName)
        )
...
def getStartBone(armatureObj):
    startBoneNames = [
        ...
        if bone.parent is None and bone.ootBoneType != "Ignore"
    ]
    if len(startBoneNames) == 0:
        raise PluginError(
            armatureObj.name
            + ' does not have any root bones that are not of the "Ignore" type.'
        )
...
def checkUniformScale(scale, obj):
    if (
        abs(scale[0] - scale[1]) > 0.01
        or abs(scale[1] - scale[2]) > 0.01
        or abs(scale[0] - scale[2]) > 0.01
    ):
...
def convertIntTo2sComplement(value, length, signed):
    return int.from_bytes(
        int(round(value)).to_bytes(length, "big", signed=signed), "big"
    )
...
def ootConvertRotation(rotation):
    # see BINANG_TO_DEGF
    return [
        int(round((math.degrees(value) % 360) / 360 * (2 ** 16))) % (2 ** 16)
        for value in rotation.to_euler()
    ]
...
def drawCollectionOps(layout, index, collectionType, subIndex, objName, allowAdd=True):
    ...
    removeOp = buttons.operator(
        OOTCollectionRemove.bl_idname, text="Delete", icon="REMOVE"
    )
    ...
    moveDown = buttons.operator(
        OOTCollectionMove.bl_idname, text="Down", icon="TRIA_DOWN"
    )
