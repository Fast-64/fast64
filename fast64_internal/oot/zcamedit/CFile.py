...
class CFileImport(CFileIO):
    ...
    def ImportPos(self, pos):
        ...
        return [float(pos[0]) / self.scale,
            -float(pos[2]) / self.scale,
            float(pos[1]) / self.scale]
    ...
    def CSToBlender(self, csname, poslists, atlists, actionlists):
        ...
        for l in actionlists:
            ...
            for p in l['data']:
                ...
                point = CreateActionPoint(self.context, al_object, False,
                    self.ImportPos([p['startX'], p['startY'], p['startZ']]),
                    p['startFrame'], p['action'])
            ...
            point = CreateActionPoint(self.context, al_object, False,
                self.ImportPos([lastx, lasty, lastz]), lastf, '0x0000')
