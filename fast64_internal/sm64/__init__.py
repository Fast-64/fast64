    ...
    def execute(self, context):
        ...
        try:
            ...
            levelParsed = parseLevelAtPointer(
                romfileSrc, level_pointers[context.scene.levelConvert]
            )
...
class SM64_AddressConvertPanel(SM64_Panel):
    ...
    def draw(self, context):
        col = self.layout.column()
        segToVirtOp = col.operator(
            SM64_AddrConv.bl_idname, text="Convert Segmented To Virtual"
        )
        segToVirtOp.segToVirt = True
        virtToSegOp = col.operator(
            SM64_AddrConv.bl_idname, text="Convert Virtual To Segmented"
        )
...
class SM64_Properties(bpy.types.PropertyGroup):
    ...
    # UI Selection
    showImportingMenus: bpy.props.BoolProperty(
        name="Show Importing Menus", default=False
    )
    exportType: bpy.props.EnumProperty(
        items=enumExportType, name="Export Type", default="C"
    )
    goal: bpy.props.EnumProperty(
        items=sm64GoalTypeEnum, name="Export Goal", default="All"
    )
...
def sm64_register(registerPanels):
    ...
    bpy.types.Scene.blenderToSM64Scale = bpy.props.FloatProperty(
        name="Blender To SM64 Scale", default=100
    )  # 212.766
