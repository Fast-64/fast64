...
def exportAnimationCommon(armatureObj, loopAnim, name):
    ...
    frameInterval[0] = max(bpy.context.scene.frame_start, int(round(anim.frame_range[0])))

    frameInterval[1] = max(min(bpy.context.scene.frame_end, int(round(anim.frame_range[1]))), frameInterval[0]) + 1
