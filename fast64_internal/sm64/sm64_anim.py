...
class SM64_Animation:
    ...
    def to_binary(self, segmentData, isDMA, startAddress):
        return (
            self.header.to_binary(segmentData, isDMA, startAddress)
...
class SM64_AnimationHeader:
    ...
    def to_binary(self, segmentData, isDMA, startAddress):
        ...
        data.extend(
            self.marioYOffset.to_bytes(2, byteorder="big")
        )  # y offset, only used for mario
        data.extend(
            [0x00, 0x00]
        )  # unknown, common with secondary anims, variable length animations?
...
def exportAnimationC(
    ...
    dirPath, texDir = getExportDir(
        customExport, dirPath, headerType, levelName, "", dirName
    )
    ...
    if not os.path.exists(tableFilePath):
        ...
        tableFile.write(
            "const struct Animation *const " + animsName + "[] = {\n\tNULL,\n};\n"
        )
        tableFile.close()
...
def exportAnimationBinary(
    romfile, exportRange, armatureObj, DMAAddresses, segmentData, isDMA, loopAnim
):
    ...
    if not isDMA:
        ...
        if DMAAddresses is not None:
            ...
            romfile.write(
                (startAddress - DMAAddresses["start"]).to_bytes(4, byteorder="big")
            )
...
def exportAnimationInsertableBinary(filepath, armatureObj, isDMA, loopAnim):
    ...
    writeInsertableFile(
        filepath,
        insertableBinaryTypes["Animation"],
        sm64_anim.get_ptr_offsets(isDMA),
        startAddress,
        animData,
    )
...
def exportAnimationCommon(armatureObj, loopAnim, name):
    ...
    translationData, armatureFrameData = convertAnimationData(
        anim, armatureObj, frameInterval[1]
    )
    ...
    for translationFrameProperty in translationData:
        ...
        for value in translationFrameProperty.frames:
            sm64_anim.values.shortData.append(
                int.from_bytes(
                    value.to_bytes(2, "big", signed=True), byteorder="big", signed=False
                )
            )
    ...
    animSize = (
        headerSize
        + len(sm64_anim.indices.shortData) * 2
...
def convertAnimationData(anim, armatureObj, frameEnd):
    ...
    for frame in range(frameEnd):
        ...
        for boneIndex in range(len(animBones)):
            ...
            rotationValue = (
                currentBone.matrix.to_4x4().inverted() @ currentPoseBone.matrix
            ).to_quaternion()
...
def importAnimationToBlender(romfile, startAddress, armatureObj, segmentData, isDMA):
    ...
    animationHeader, armatureFrameData = readAnimation(
        "sm64_anim", romfile, startAddress, segmentData, isDMA
    )
            ...
            for propertyIndex in range(3):
                ...
                for frame in range(len(boneFrameData[propertyIndex])):
                    fcurve.keyframe_points.insert(
                        frame, boneFrameData[propertyIndex][frame]
                    )
...
def readAnimation(name, romfile, startAddress, segmentData, isDMA):
    ...
    print(
        "Frames: "
        + str(animationHeader.frameInterval[1])
        + " / Nodes: "
        + str(animationHeader.nodeCount)
    )
    ...
    boneFrameData[0] = [
        n
        for n in getKeyFramesTranslation(
            romfile, animationHeader.transformValuesStart, rootIndexNode.x
        )
    ]
    boneFrameData[1] = [
        n
        for n in getKeyFramesTranslation(
            romfile, animationHeader.transformValuesStart, rootIndexNode.y
        )
    ]
    boneFrameData[2] = [
        n
        for n in getKeyFramesTranslation(
            romfile, animationHeader.transformValuesStart, rootIndexNode.z
        )
    ]
    ...
    for boneIndexNode in animationHeader.transformIndices[1:]:
        ...
        boneFrameData[0] = [
            n
            for n in getKeyFramesRotation(
                romfile, animationHeader.transformValuesStart, boneIndexNode.x
            )
        ]
        boneFrameData[1] = [
            n
            for n in getKeyFramesRotation(
                romfile, animationHeader.transformValuesStart, boneIndexNode.y
            )
        ]
        boneFrameData[2] = [
            n
            for n in getKeyFramesRotation(
                romfile, animationHeader.transformValuesStart, boneIndexNode.z
            )
        ]
...
def getKeyFramesTranslation(romfile, transformValuesStart, boneIndex):
    ...
    for frame in range(boneIndex.numFrames):
        romfile.seek(ptrToValue + frame * 2)
        keyframes.append(
            int.from_bytes(romfile.read(2), "big", signed=True)
    ...
    def execute(self, context):
        ...
        try:
            ...
            if len(context.selected_objects) > 1:
                raise PluginError(
                    "Multiple objects selected, make sure to select only one."
                )
        ...
        try:
            ...
            if context.scene.fast64.sm64.exportType == "C":
                ...
                self.report(
                    {"INFO"},
                    "Success! Animation at " + context.scene.animInsertableBinaryPath,
                )
            else:
                ...
                levelParsed = parseLevelAtPointer(
                    romfileOutput, level_pointers[context.scene.levelAnimExport]
                )
                ...
                addrRange, nonDMAListPtr = exportAnimationBinary(
                    romfileOutput,
                    [
                        int(context.scene.animExportStart, 16),
    ...
    def draw(self, context):
        ...
        if context.scene.fast64.sm64.exportType == "C":
            ...
            if context.scene.isDMAExport:
                ...
                if context.scene.animOverwriteDMAEntry:
                    prop_split(
                        col, context.scene, "DMAStartAddress", "DMA Start Address"
                    )
                    prop_split(
                        col, context.scene, "DMAEntryAddress", "DMA Entry Address"
                    )
            else:
                ...
                if context.scene.setAnimListIndex:
                    ...
                    prop_split(
                        col, context.scene, "animListIndexExport", "Anim List Index"
                    )
                    col.prop(context.scene, "overwrite_0x28")
                    if context.scene.overwrite_0x28:
                        prop_split(
                            col, context.scene, "addr_0x28", "28 Command Address"
                        )
...
class SM64_ImportAnimMario(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            levelParsed = parseLevelAtPointer(
                romfileSrc, level_pointers[context.scene.levelAnimImport]
            )
            segmentData = levelParsed.segmentData
...
def sm64_anim_register():
    ...
    bpy.types.Scene.isDMAImport = bpy.props.BoolProperty(
        name="Is DMA Animation", default=True
    )
    bpy.types.Scene.isDMAExport = bpy.props.BoolProperty(name="Is DMA Animation")
    bpy.types.Scene.DMAEntryAddress = bpy.props.StringProperty(
        name="DMA Entry Address", default="4EC008"
    )
    bpy.types.Scene.DMAStartAddress = bpy.props.StringProperty(
        name="DMA Start Address", default="4EC000"
    )
    bpy.types.Scene.levelAnimImport = bpy.props.EnumProperty(
        items=level_enums, name="Level", default="IC"
    )
    bpy.types.Scene.levelAnimExport = bpy.props.EnumProperty(
        items=level_enums, name="Level", default="IC"
    )
    bpy.types.Scene.loopAnimation = bpy.props.BoolProperty(
        name="Loop Animation", default=True
    )
    bpy.types.Scene.setAnimListIndex = bpy.props.BoolProperty(
        name="Set Anim List Entry", default=True
    )
    bpy.types.Scene.overwrite_0x28 = bpy.props.BoolProperty(
        name="Overwrite 0x28 behaviour command", default=True
    )
    ...
    bpy.types.Scene.animLevelName = bpy.props.StringProperty(
        name="Level", default="bob"
    )
