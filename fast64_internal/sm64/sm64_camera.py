...
class CameraSettingsPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        return hasattr(context, "object") and isinstance(
...
def saveCameraSettingsToGeolayout(geolayoutGraph, areaObj, rootObj, meshGeolayoutName):
    ...
    if not areaObj.fast64.sm64.area.disable_background:
        ...
        if areaObj.areaOverrideBG:
            ...
            bgNode = TransformNode(
                BackgroundNode(rootObj.useBackgroundColor, background)
            )
    ...
    frustumNode = TransformNode(
        FrustumNode(areaObj.fov, areaObj.clipPlanes[0], areaObj.clipPlanes[1])
    )
    ...
    cameraNode.children.append(
        TransformNode(
            FunctionNode(
                "802761D0",
                areaObj.envOption if areaObj.envOption != "Custom" else areaObj.envType,
            )
        )
    )
