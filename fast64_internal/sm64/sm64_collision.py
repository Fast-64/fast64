...
class CollisionTriangle:
    ...
    def to_binary(self):
        ...
        if len(self.indices) > 3:
            raise PluginError(
                "Triangle indices should not be "
...
class Collision:
    ...
    def to_c(self):
        ...
        for collisionType, triangles in self.triangles.items():
            data.source += (
                "\tCOL_TRI_INIT(" + collisionType + ", " + str(len(triangles)) + "),\n"
            )
...
def exportCollisionBinary(
    ...
    collision = exportCollisionCommon(
        obj, transformMatrix, includeSpecials, includeChildren, obj.name, None
    )
...
def exportCollisionC(
    ...
    dirPath, texDir = getExportDir(
        customExport, dirPath, headerType, levelName, "", name
    )
    ...
    collision = exportCollisionCommon(
        obj, transformMatrix, includeSpecials, includeChildren, name, None
    )
...
def exportCollisionInsertableBinary(
    obj, transformMatrix, filepath, includeSpecials, includeChildren
):
    collision = exportCollisionCommon(
        obj, transformMatrix, includeSpecials, includeChildren, obj.name, None
    )
    ...
    writeInsertableFile(
        filepath, insertableBinaryTypes["Collision"], [], collision.startAddress, data
    )

    return data


def exportCollisionCommon(
    obj, transformMatrix, includeSpecials, includeChildren, name, areaIndex
):
    ...
    try:
        addCollisionTriangles(
            tempObj, collisionDict, includeChildren, transformMatrix, areaIndex
        )
    ...
    for collisionType, faces in collisionDict.items():
        ...
        for (faceVerts, specialParam, room) in faces:
            ...
            collision.triangles[collisionType].append(
                CollisionTriangle(indices, specialParam, room)
            )
    if includeSpecials:
...
def addCollisionTriangles(
    obj, collisionDict, includeChildren, transformMatrix, areaIndex
):
    if isinstance(obj.data, bpy.types.Mesh) and not obj.ignore_collision:
        ...
        for face in obj.data.loop_triangles:
            ...
            collisionDict[colType].append(
                (((x1, y1, z1), (x2, y2, z2), (x3, y3, z3)), specialParam, obj.room_num)
            )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if context.scene.fast64.sm64.exportType == "C":
                ...
                self.report(
                    {"INFO"},
                    "Success! Collision at " + context.scene.colInsertableBinaryPath,
                )
            else:
                ...
                levelParsed = parseLevelAtPointer(
                    romfileOutput, level_pointers[context.scene.colExportLevel]
                )
...
class SM64_ExportCollisionPanel(SM64_Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.fast64.sm64.exportType == "C":
            ...
            if context.scene.set_addr_0x2A:
                prop_split(
                    col, context.scene, "addr_0x2A", "0x2A Behaviour Command Address"
                )
...
def sm64_col_register():
    ...
    bpy.types.Scene.colStartAddr = bpy.props.StringProperty(
        name="Start Address", default="11D8930"
    )
    bpy.types.Scene.colEndAddr = bpy.props.StringProperty(
        name="Start Address", default="11FFF00"
    )
    ...
    bpy.types.Scene.colGroupName = bpy.props.StringProperty(
...
enumCollisionType = [
    ...
    (
        ...
        "Noise Very Slippery 73",
        "Noise Very Slippery 73",
    ),
    (
        "SURFACE_NOISE_VERY_SLIPPERY_74",
        "Noise Very Slippery 74",
        "Noise Very Slippery 74",
    ),
    ...
    (
        ...
        "No Cam Collision Very Slippery",
        "No Cam Collision Very Slippery",
    ),
    (
