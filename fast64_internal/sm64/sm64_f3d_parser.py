    ...
    def execute(self, context):
        ...
        try:
            ...
            levelParsed = parseLevelAtPointer(
                romfileSrc, level_pointers[context.scene.levelDLImport]
            )
            segmentData = levelParsed.segmentData
            start = (
                decodeSegmentedAddr(
                    int(context.scene.DLImportStart, 16).to_bytes(4, "big"), segmentData
                )
...
def sm64_dl_parser_register():
    ...
    bpy.types.Scene.levelDLImport = bpy.props.EnumProperty(
        items=level_enums, name="Level", default="CG"
    )
