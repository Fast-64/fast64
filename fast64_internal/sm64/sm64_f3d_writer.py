...
class SM64Model(FModel):
    ...
    def getRenderMode(self, drawLayer):
        cycle1 = getattr(
            bpy.context.scene.world, "draw_layer_" + str(drawLayer) + "_cycle_1"
        )
        cycle2 = getattr(
            bpy.context.scene.world, "draw_layer_" + str(drawLayer) + "_cycle_2"
        )
...
class SM64GfxFormatter(GfxFormatter):
        ...
        if self.functionNodeDraw:
            ...
            for command in self.commands:
                if (
                    ...
                    and command.displayList.tag == GfxListTag.Material
                ):
                    data.source += (
                        "\t"
                        + "glistp = "
    ...
    def tileScrollMaterialToC(self, f3d, fMaterial: FMaterial):
        ...
        data.header = (
            "Gfx* " + fMaterial.material.name + "(Gfx* glistp, int s, int t);\n"
        )
                    ...
                    if scrollField.animType == "Linear":
                        if axisIndex == 0:
                            fMaterial.tileSizeCommands[texIndex].uls = (
                                str(fMaterial.tileSizeCommands[0].uls)
                                + " + s * "
                                + str(scrollField.speed)
                            )
                        else:
                            fMaterial.tileSizeCommands[texIndex].ult = (
                                str(fMaterial.tileSizeCommands[0].ult)
                                + " + s * "
                                + str(scrollField.speed)
                            )
...
def exportTexRectToC(
    ...
    if exportToProject:
        ...
        overwriteData(
            "void\s*", fTexRect.name, code, hudPath, projectExportData[1], True
        )

    else:
        singleFileData = ""
        singleFileData += (
            "// Copy this function to src/game/hud.c or src/game/ingame_menu.c.\n"
        )
...
def exportTexRectCommon(texProp, f3dType, isHWv1, name, convertTextureData):
    ...
    fTexRect = FTexRect(
        f3dType, isHWv1, toAlnum(name), GfxMatWriteMethod.WriteDifferingAndRevert
    )
    ...
    fTexRect.draw.commands.append(
        SPScisTextureRectangle(
            0, 0, (texDimensions[0] - 1) << 2, (texDimensions[1] - 1) << 2, 0, 0, 0
        )
    )
...
def sm64ExportF3DtoC(
    ...
    dirPath, texDir = getExportDir(
        customExport, basePath, headerType, levelName, texDir, name
    )
    ...
    modifyTexScrollFiles(
        basePath, modelDirPath, cDefineScroll, scroll_data, hasScrolling
    )
    ...
    if not customExport:
        if headerType == "Actor":
            ...
            texscrollIncludeC = (
                '#include "levels/' + levelName + "/" + name + '/texscroll.inc.c"'
            )
            texscrollIncludeH = (
                '#include "levels/' + levelName + "/" + name + '/texscroll.inc.h"'
            )
...
def exportF3DtoBinary(
    ...
    fMesh = exportF3DCommon(
        obj, fModel, transformMatrix, includeChildren, obj.name, DLFormat.Static, True
    )
    fModel.freePalettes()
...
def exportF3DtoBinaryBank0(
    ...
    return (
        fMesh.draw.startAddress,
        (startAddress, startAddress + len(data)),
        segPointerData,
    )


def exportF3DtoInsertableBinary(
    filepath, transformMatrix, obj, f3dType, isHWv1, includeChildren
):
    ...
    writeInsertableFile(
        filepath,
        insertableBinaryTypes["Display List"],
        address_ptrs,
        fMesh.draw.startAddress,
        data,
    )
        ...
        try:
            ...
            if context.scene.fast64.sm64.exportType == "C":
                ...
                fileStatus = sm64ExportF3DtoC(
                    ...
                    obj,
                    DLFormat.Static
                    if context.scene.DLExportisStatic
                    else DLFormat.Dynamic,
                    finalTransform,
                ...
                self.report(
                    {"INFO"},
                    "Success! DL at " + context.scene.DLInsertableBinaryPath + ".",
                )
            else:
                ...
                levelParsed = parseLevelAtPointer(
                    romfileOutput, level_pointers[context.scene.levelDLExport]
                )
            ...
            if context.scene.texrect.tex is None:
                ...
                if context.scene.TexRectCustomExport:
                    ...
                    if context.scene.decompPath == "":
                        raise PluginError(
                            "Decomp path has not been set in File Settings."
                        )
...
def sm64_dl_writer_register():
    ...
    bpy.types.Scene.levelDLExport = bpy.props.EnumProperty(
        items=level_enums, name="Level", default="WF"
    )
    ...
    bpy.types.Scene.DLRAMAddr = bpy.props.StringProperty(
    ...
    bpy.types.Scene.DLGroupName = bpy.props.StringProperty(
    ...
    bpy.types.Scene.texrectImageTexture = bpy.props.PointerProperty(
        type=bpy.types.ImageTexture
    )
    bpy.types.Scene.TexRectExportPath = bpy.props.StringProperty(
        name="Export Path", subtype="FILE_PATH"
    )
    bpy.types.Scene.TexRectTexDir = bpy.props.StringProperty(
        name="Include Path", default="textures/segment2"
    )
    bpy.types.Scene.TexRectName = bpy.props.StringProperty(
        name="Name", default="render_hud_image"
    )
    bpy.types.Scene.TexRectCustomExport = bpy.props.BoolProperty(
        name="Custom Export Path"
    )
    bpy.types.Scene.TexRectExportType = bpy.props.EnumProperty(
        name="Export Type", items=enumHUDExportLocation
    )
