...
enumBoneType = [
    ...
    (
        ...
        "Animated Part (0x13)",
        "Animated Part (Animatable Bone)",
    ),
    ("CustomAnimated", "Custom Animated", "Custom Bone used for animation"),
    (
        "CustomNonAnimated",
        "Custom (Non-animated)",
        "Custom geolayout bone, non animated",
    ),
]
...
def drawGeoInfo(panel: bpy.types.Panel, bone: bpy.types.Bone):
    ...
    if bone.geo_cmd == "Scale":
        ...
        infoBox2 = col.box()
        infoBox2.label(
            text="This affects the next sibling bone in " + "alphabetical order."
        )
        ...
        infoBoxRenderArea.label(
            text="See the object properties window for the armature instead."
        )
        ...
        if bone.geo_cmd == "CustomNonAnimated":
            ...
            infobox = col.box()
            infobox.label(
                text="Command's args will be filled with layer, translate, and rotate",
                icon="INFO",
            )
            infobox.label(
                text="e.g. `GEO_CUSTOM(layer, tX, tY, tZ, rX, rY, rZ, displayList)`"
            )
...
def drawLayerWarningBox(layout, prop, data):
    ...
    warningBox.label(
        text="This applies to v3 materials and down only.", icon="LOOP_FORWARDS"
    )
...
class GeolayoutObjectPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if obj.use_render_area:
            ...
            renderAreaBox.label(
                text="This only applies if this is the root object of an object geolayout."
            )
            renderAreaBox.label(
                text="For armature geolayouts, see the armature's object properties instead."
            )
            prop_split(col, obj, "culling_radius", "Culling Radius")
...
class SwitchOptionProperty(bpy.types.PropertyGroup):
    ...
    optionArmature: bpy.props.PointerProperty(
        name="Option Armature", type=bpy.types.Object
    )
    materialOverride: bpy.props.PointerProperty(
        type=bpy.types.Material, name="Material Override"
    )
    materialOverrideType: bpy.props.EnumProperty(
        name="Material Override Type", items=enumMatOverrideOptions
    )
...
def drawSwitchOptionProperty(layout, switchOption, index):
    ...
    if switchOption.expand:
        ...
        if switchOption.switchType == "Material":
            ...
            prop_split(
                box, switchOption, "materialOverrideType", "Material Override Type"
            )
        ...
        buttons.operator(
            RemoveSwitchOption.bl_idname, text="Remove Option"
        ).option = index
        buttons.operator(AddSwitchOption.bl_idname, text="Add Option").option = (
            index + 1
        )
...
class RemoveSwitchOptionMat(bpy.types.Operator):
    ...
    def execute(self, context):
        if self.isSpecific:
            context.bone.switch_options[self.option].specificOverrideArray.remove(
                self.index
            )
        else:
            context.bone.switch_options[self.option].specificIgnoreArray.remove(
                self.index
            )
        self.report({"INFO"}, "Success!")
        return {"FINISHED"}
...
def getSwitchOptionBone(switchArmature):
    ...
    for poseBone in switchArmature.pose.bones:
        if (
            poseBone.bone_group is not None
            and poseBone.bone_group.name == "SwitchOption"
        ):
            optionBones.append(poseBone.name)
    if len(optionBones) > 1:
        raise PluginError(
            "There should only be one switch option bone in "
            + switchArmature.name
            + "."
        )
...
class SM64_BoneProperties(bpy.types.PropertyGroup):
    ...
    custom_geo_cmd_macro: bpy.props.StringProperty(
        name="Geo Command Macro", default="GEO_BONE"
    )
...
def sm64_bone_register():
    ...
    bpy.types.Object.use_render_range = bpy.props.BoolProperty(
        name="Use Render Range (LOD)"
    )
    bpy.types.Object.render_range = bpy.props.FloatVectorProperty(
        name="Render Range", size=2, default=(0, 100)
    )
