...
def addFuncAddress(command, func):
    try:
        ...
        raise PluginError(
            'In geolayout node, could not convert function "'
            + str(func)
            + '" to hexadecimal.'
        )
...
class GeolayoutGraph:
    ...
    def checkListSorted(self):
        if not self.sortedListGenerated:
            raise PluginError(
                "Must generate sorted geolayout list first "
            ...
            for calledGeolayout in self.geolayoutCalls[geolayout]:
                ...
                if calledGeolayout in geolayoutList:
                    ...
                    if callIndex < geoIndex:
                        ...
                        raise PluginError(
                            "Circular geolayout dependency." + str(callOrder)
                        )
                else:
                    geolayoutList.insert(
                        geolayoutList.index(geolayout), calledGeolayout
                    )
    ...
    def generateSortedList(self):
        self.sortedList = self.sortGeolayouts(
            [self.startGeolayout], self.startGeolayout, [self.startGeolayout]
        )
...
class TransformNode:
    ...
    def to_c(self, depth):
        if self.node is not None:
            ...
            if (
                nodeC is not None
            ):  # Should only be the case for DisplayListNode with no DL
        ...
        if len(self.children) > 0:
            ...
            for child in self.children:
                data += child.to_c(
                    depth + (1 if type(self.node) in nodeGroupClasses else 0)
                )
            if type(self.node) in nodeGroupClasses:
    ...
    def getDrawLayers(self):
        ...
        for child in self.children:
            if hasattr(
                child, "getDrawLayers"
            ):  # not every child will have draw layers (e.g. GEO_ASM)
...
class JumpNode:
    ...
    def to_binary(self, segmentData):
        ...
        command = bytearray(
            [GEO_BRANCH, 0x01 if self.storeReturn else 0x00, 0x00, 0x00]
        )
...
class TranslateRotateNode(BaseDisplayListNode):
    def __init__(
        self, drawLayer, fieldLayout, hasDL, translate, rotate, dlRef: str = None
    ):
    ...
    def to_binary(self, segmentData):
        params = ((1 if self.hasDL else 0) << 7) & (self.fieldLayout << 4) | int(
            self.drawLayer
        )
        ...
        if self.fieldLayout == 0:
            ...
            writeEulerVectorToShorts(
                command, 10, self.rotate.to_euler(geoNodeRotateOrder)
            )
        elif self.fieldLayout == 1:
            command.extend(bytearray([0x00] * 6))
            writeVectorToShorts(command, 2, self.translate)
            ...
            command.extend(bytearray([0x00] * 6))
            writeEulerVectorToShorts(
                command, 2, self.rotate.to_euler(geoNodeRotateOrder)
            )
        elif self.fieldLayout == 3:
            ...
            return self.c_func_macro(
                ...
                str(
                    convertEulerFloatToShort(
                        self.rotate.to_euler(geoNodeRotateOrder)[0]
                    )
                ),
                str(
                    convertEulerFloatToShort(
                        self.rotate.to_euler(geoNodeRotateOrder)[1]
                    )
                ),
                str(
                    convertEulerFloatToShort(
                        self.rotate.to_euler(geoNodeRotateOrder)[2]
                    )
                ),
            )
        elif self.fieldLayout == 1:
...
class StartRenderAreaNode:
    ...
    def to_c(self):
        ...
        return (
            "GEO_CULLING_RADIUS(" + str(convertFloatToShort(self.cullingRadius)) + "),"
        )
