...
def parseGeoLayout(
    ...
    if useArmature:
        ...
        for i in range(len(armatureMeshGroups)):
            ...
            switchArmatureObj.matrix_world = (
                switchArmatureObj.matrix_world
...
def parseNode(
    ...
    while currentCmd[0] is not endCmd and currentCmd[0] is not GEO_END:
        isSwitchOption = (
            singleChildStack[-1] and nodeProcessed and completeNodeProcessed
        )
        ...
        if currentCmd[0] == GEO_BRANCH_STORE and not ignoreNode:  # 0x00
            currentAddress = parseBranchStore(
                romfile, currentCmd, currentAddress, jumps, segmentData=segmentData
            )
...
def processBoneMeta(armatureObj, boneName, parentName):
    ...
    if parentName is not None:
        ...
        visualChildren = [
            child for child in parentVisualBone.children if child.name[:5] != "meta_"
        ]
        print(
            str(parentVisualBoneName)
            + " "
            + str([child.name for child in parentVisualBone.children])
        )
        if len(visualChildren) == 0:
            ...
            print(
                ...
                + str(visualBone.name)
                + " "
                + str([child.name for child in visualChildren])
            )
...
def createBone(
    armatureObj, parentBoneName, boneName, currentTransform, boneGroup, loadDL
):
    if bpy.context.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
    bpy.ops.object.select_all(action="DESELECT")
    ...
    bone.tail = bone.head + (
        currentTransform.to_quaternion()
...
def createSwitchOption(
    ...
    bone.tail = (
        bone.head + currentTransform.to_quaternion() @ mathutils.Vector((0, 1, 0)) * 0.2
    )
...
def parseSwitch(
    ...
    if not ignoreNode:
        ...
        if armatureObj is not None:
            boneName = createBone(
                armatureObj, parentBoneName, boneName, currentTransform, "Switch", False
            )
            bone = armatureObj.data.bones[boneName]
...
def parseDLWithOffset(
    ...
    if not ignoreNode:
        if armatureObj is not None:
            # Create bone
            boneName = createBone(
                armatureObj, parentBoneName, boneName, finalTransform, None, hasMeshData
            )
            if bpy.context.mode != "OBJECT":
                bpy.ops.object.mode_set(mode="OBJECT")
            bone = armatureObj.data.bones[boneName]
        ...
        # load mesh data
        if hasMeshData:
            displayListStartAddress = decodeSegmentedAddr(
                segmentedAddr, segmentData=segmentData
            )
...
def handleNodeCommon(
    ...
    if armatureObj is not None:
        # Create bone
        boneName = createBone(
            armatureObj, parentBoneName, boneName, finalTransform, boneGroupName, loadDL
        )
...
def parseTranslateRotate(
    ...
    if fieldLayout == 0:
        ...
        rotation = (
            mathutils.Euler((0, yRot, 0), geoNodeRotateOrder).to_matrix().to_4x4()
        )
        finalTransform = currentTransform @ rotation
...
def parseShadow(
    ...
    if not ignoreNode:
        ...
        if armatureObj is not None:
            boneName = createBone(
                armatureObj, parentBoneName, boneName, currentTransform, "Shadow", False
            )
            bone = armatureObj.data.bones[boneName]
...
def parseStart(
    ...
    if not ignoreNode:
        ...
        if armatureObj is not None:
            boneName = createBone(
                armatureObj, parentBoneName, boneName, currentTransform, "Start", False
            )
    else:
        boneName = None
...
def parseStartWithRenderArea(
    ...
    command = romfile.read(commandSize)
    cullingRadius = (
        int.from_bytes(command[2:4], "big") / bpy.context.scene.blenderToSM64Scale
    )
...
def parseFunction(
    ...
    if armatureObj is not None and not ignoreNode:
        boneName = createBone(
            armatureObj, parentBoneName, boneName, currentTransform, "Function", False
        )
        bone = armatureObj.data.bones[boneName]
...
def assignMarioGeoMetadata(
    obj, commandAddress, geoStartAddress, cmdType, armatureData, lastTransRotAddr=None
):
    ...
    if armatureData is not None:
        obj["sm64_part_names"] = [
            armatureData.findBoneByOffset(commandAddress - geoStartAddress).name
        ]
...
def handleOverlapGeoMetadata(bone):
    ...
    if parentBone is not None and "sm64_geo_meta" in parentBone:
        ...
        if parentBoneMeta["geo_top_overlap_ptr"] is None:
            parentBoneMeta["geo_top_overlap_ptr"] = parentBoneMeta[
                "geo_pointer_relative"
            ]
    ...
    def execute(self, context):
        ...
        try:
            ...
            levelParsed = parseLevelAtPointer(
                romfileSrc, level_pointers[levelGeoImport]
            )
...
def sm64_geo_parser_register():
    ...
    bpy.types.Scene.levelGeoImport = bpy.props.EnumProperty(
