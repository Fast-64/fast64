...
def appendRevertToGeolayout(geolayoutGraph, fModel):
    fModel.materialRevert = GfxList(
        fModel.name + "_" + "material_revert_render_settings",
        GfxListTag.MaterialRevert,
        fModel.DLFormat,
    )
    revertMatAndEndDraw(
        fModel.materialRevert,
        [DPSetEnvColor(0xFF, 0xFF, 0xFF, 0xFF), DPSetAlphaCompare("G_AC_NONE")],
    )
    ...
    for layer in sorted(
        drawLayers
    ):  # Must be sorted, otherwise ordering is random due to `set` behavior
...
def convertArmatureToGeolayout(
    ...
    convertTransformMatrix = (
        convertTransformMatrix @ mathutils.Matrix.Diagonal(armatureObj.scale).to_4x4()
    )
...
def saveGeolayoutC(
    ...
    dirPath, texDir = getExportDir(
        customExport, exportDir, headerType, levelName, texDir, dirName
    )
    ...
    exportData = fModel.to_c(
        TextureExportSettings(texSeparate, savePNG, texDir, texExportPath), gfxFormatter
    )
    ...
    if headerType == "Actor":
        ...
        if not customExport:
            ...
            if groupName == "" or groupName is None:
                raise PluginError(
                    "Actor header type chosen but group name not provided."
                )
        ...
        matCInclude = (
            '#include "levels/' + levelName + "/" + dirName + '/material.inc.c"'
        )
        matHInclude = (
            '#include "levels/' + levelName + "/" + dirName + '/material.inc.h"'
        )
        headerInclude = (
            '#include "levels/' + levelName + "/" + dirName + '/geo_header.h"'
        )

    modifyTexScrollFiles(
        exportDir, geoDirPath, cDefineScroll, scroll_data, hasScrolling
    )
    ...
    if not customExport:
        if headerType == "Actor":
            ...
            if (
                dirName == "transparent_star"
                and bpy.context.scene.replaceTransparentStarRefs
            ):
            ...
            texscrollIncludeC = (
                '#include "levels/' + levelName + "/" + dirName + '/texscroll.inc.c"'
            )
            texscrollIncludeH = (
                '#include "levels/' + levelName + "/" + dirName + '/texscroll.inc.h"'
            )
...
def exportGeolayoutObjectInsertableBinary(
    obj, convertTransformMatrix, f3dType, isHWv1, filepath, camera
):
    geolayoutGraph, fModel = convertObjectToGeolayout(
        obj,
        convertTransformMatrix,
...
def saveGeolayoutInsertableBinary(geolayoutGraph, fModel, filepath, f3d):
    data, startRAM = getBinaryBank0GeolayoutData(
        fModel, geolayoutGraph, 0, [0, 0xFFFFFF]
    )
    ...
    writeInsertableFile(
        filepath,
        insertableBinaryTypes["Geolayout"],
        address_ptrs,
        geolayoutGraph.startGeolayout.startAddress,
        data,
    )
...
def saveGeolayoutBinaryBank0(
    ...
    return (
        (startAddress, startAddress + len(data)),
        startRAM + 0x80000000,
        geoStart + 0x80000000,
    )
    ...
    if isinstance(transformNode.node, JumpNode):
        for node in transformNode.node.geolayout.nodes:
            generateSwitchOptions(
                node, transformNode.node.geolayout, geolayoutGraph, prefix
            )
    overrideNodes = []
        ...
        while i < len(transformNode.children):
            ...
            if isinstance(childNode.node, SwitchOverrideNode):
                ...
                geolayoutGraph.addJumpNode(
                    transformNode, geolayout, optionGeolayout, index
                )
                ...
                if len(option0Nodes) == 1 and isinstance(
                    option0Nodes[0].node, StartNode
                ):
...
def generateOverrideHierarchy(
    ...
    if isinstance(transformNode.node, JumpNode):
        ...
        if len(oldGeolayout.nodes) == 1 and isinstance(
            oldGeolayout.nodes[0].node, StartNode
        ):
        ...
        if material is not None:
            copyNode.node.DLmicrocode = copyNode.node.fMesh.drawMatOverrides[
                (material, specificMat, overrideType)
            ]
...
def getSwitchChildren(areaRoot):
    ...
    alphabeticalChildren = sorted(
        geoChildren, key=lambda childObj: childObj.original_name.lower()
    )
...
def setRooms(obj, roomIndex=None):
    ...
    # Area root object
        ...
        for i in range(len(alphabeticalChildren)):
            setRooms(
                alphabeticalChildren[i], i
            )  # index starts at 1, but 0 is reserved for no room.
...
def processInlineGeoNode(
    ...
    if inlineGeoConfig.name == "Geo Translate/Rotate":
        ...
        node = BillboardNode(
            obj.draw_layer_static, obj.useDLReference, translate, obj.dlReference
        )
        ...
        node = RotateNode(
            obj.draw_layer_static, obj.useDLReference, rotate, obj.dlReference
        )
    elif inlineGeoConfig.name == "Geo Scale":
        node = ScaleNode(
            obj.draw_layer_static, scale, obj.useDLReference, obj.dlReference
        )
    else:
        raise PluginError(
            f"Ooops! Didnt implement inline geo exporting for {inlineGeoConfig.name}"
        )
...
def processMesh(
    ...
    addRooms = (
        ...
        and obj.sm64_obj_type == "Area Root"
        and obj.enableRoomSwitch
    )

    # if useAreaEmpty and areaIndex is not None and obj.areaIndex != areaIndex:
    # 	return

    inlineGeoConfig: InlineGeolayoutObjConfig = inlineGeoLayoutObjects.get(
        obj.sm64_obj_type
    )
    ...
    if useSwitchNode or addRooms:  # Specific empty types
        ...
        parentTransformNode = addParentNode(
            parentTransformNode, SwitchNode(switchFunc, switchParam, obj.original_name)
        )
        alphabeticalChildren = getSwitchChildren(obj)
        for i in range(len(alphabeticalChildren)):
            ...
            if i == 0:  # Outside room system
                ...
                geolayoutGraph.addJumpNode(
                    parentTransformNode, geolayout, optionGeolayout
                )
                if not zeroRotation or not zeroTranslation:
                    startNode = TransformNode(
                        getOptimalNode(
                            translate, rotate, 1, False, zeroTranslation, zeroRotation
                        )
                    )
                else:
                    startNode = TransformNode(StartNode())
        ...
        if useInlineGeo and not processed_inline_geo:
            ...
            if not zeroRotation or not zeroScaleChange:
                ...
                node = DisplayListWithOffsetNode(
                    int(obj.draw_layer_static), True, mathutils.Vector((0, 0, 0))
                )
                ...
                node = DisplayListWithOffsetNode(
                    int(obj.draw_layer_static), True, translate
                )
        ...
        if obj.data is not None and (
            ...
            if obj.use_render_range:
                parentTransformNode = addParentNode(
                    parentTransformNode,
                    RenderRangeNode(obj.render_range[0], obj.render_range[1]),
                )

            if obj.add_shadow:
                parentTransformNode = addParentNode(
                    parentTransformNode,
                    ShadowNode(obj.shadow_type, obj.shadow_solidity, obj.shadow_scale),
                )

            if obj.add_func:
                geo_asm = obj.fast64.sm64.geo_asm
                addParentNode(
                    parentTransformNode, FunctionNode(geo_asm.func, geo_asm.param)
                )
        ...
        if obj.data is None:
            ...
            if len(src_meshes):
                ...
                for src_mesh in src_meshes[1:]:
                    additionalNode = (
                        ...
                        if not isinstance(node, BillboardNode)
                            ...
                            src_mesh["layer"], True, [0, 0, 0], src_mesh["name"]
                        )
                    )
                ...
                if fMeshes:
                    temp_obj["src_meshes"] = [
                        ({"name": fMesh.draw.name, "layer": drawLayer})
            ...
            triConverterInfo = TriangleConverterInfo(
                obj, None, fModel.f3d, transformMatrix, getInfoDict(obj)
            )
            fMeshes = saveStaticModel(
                triConverterInfo,
                fModel,
        ...
        alphabeticalChildren = sorted(
            obj.children, key=lambda childObj: childObj.original_name.lower()
        )
...
def processBone(
    ...
    if lastTranslateName is not None:
        ...
        translate = (
            translateParent.matrix_local.inverted() @ bone.matrix_local
        ).decompose()[0]
    else:
    ...
    if lastRotateName is not None:
        ...
        rotate = (rotateParent.matrix_local.inverted() @ bone.matrix_local).decompose()[
            1
        ]
    else:
        rotateParent = None
    ...
    if bone.geo_cmd in animatableBoneTypes:
        if bone.geo_cmd == "CustomAnimated":
            if not bone.fast64.sm64.custom_geo_cmd_macro:
                raise PluginError(
                    f'Bone "{boneName}" on armature "{armatureObj.name}" needs a geo command macro.'
                )
            node = CustomAnimatedNode(
                bone.fast64.sm64.custom_geo_cmd_macro,
                int(bone.draw_layer),
                translate,
                rotate,
            )
            ...
            if not zeroRotation:
                node = DisplayListWithOffsetNode(
                    int(bone.draw_layer), hasDL, mathutils.Vector((0, 0, 0))
                )
        ...
        node = CustomNode(
            bone.fast64.sm64.custom_geo_cmd_macro, bone.fast64.sm64.custom_geo_cmd_args
        )
        ...
        if bone.geo_func == "":
            raise PluginError(
                "Held object bone " + boneName + " function value is empty."
            )
        ...
        if bone.geo_cmd == "Switch":
            ...
            if bone.geo_func == "":
                raise PluginError(
                    "Switch bone " + boneName + " function value is empty."
                )
            ...
            finalTransform = transformMatrix @ mathutils.Matrix.Scale(
                node.scaleValue, 4
            )
    ...
    if node.hasDL:
        ...
        if (fMeshes is None or len(fMeshes) == 0) and (
            fSkinnedMeshes is None or len(fSkinnedMeshes) == 0
        ):
            # print("No mesh data.")
            node.hasDL = False
            ...
            for drawLayer, fMesh in fMeshes.items():
                drawLayer = int(
                    drawLayer
                )  # IMPORTANT, otherwise 1 and '1' will be considered separate keys
            ...
            if (
                ...
                raise PluginError(
                    "Cannot skin geometry to a Switch Option "
    ...
    # see generateSwitchOptions() for explanation.
        ...
        if len(bone.children) > 0:
            ...
            raise PluginError(
                'Switch bone "'
                + bone.name
                + '" must have child bones with geometry attached.'
            )
        ...
        for switchIndex in range(len(bone.switch_options)):
            ...
            if switchOption.switchType == "Mesh":
                ...
                if switchOption.switchType == "Material":
                    ...
                    if switchOption.materialOverrideType == "Specific":
                        specificMat = tuple(
                            [
                                matPtr.material
                                for matPtr in switchOption.specificOverrideArray
                            ]
                        )
                    else:
                        specificMat = tuple(
                            [
                                matPtr.material
                                for matPtr in switchOption.specificIgnoreArray
                            ]
                        )
                else:
                    material = None
                ...
                texDimensions = (
                    getTexDimensions(material) if material is not None else None
                )
    ...
    for switchOption in switchBone.switch_options:
        if switchOption.switchType == "Material":
            ...
            if switchOption.materialOverrideType == "Specific":
                ...
                specificMat = tuple(
                    [matPtr.material for matPtr in switchOption.specificOverrideArray]
                )
            else:
                ...
                specificMat = tuple(
                    [matPtr.material for matPtr in switchOption.specificIgnoreArray]
                )

            materialOverrides.append(
                (
                    switchOption.materialOverride,
                    specificMat,
                    switchOption.materialOverrideType,
                )
            )
...
# they precede them
def addSkinnedMeshNode(
    armatureObj, boneName, skinnedMesh, transformNode, parentNode, drawLayer
):
    ...
    while highestChildNode.parent is not None and not (
        ...
        hasNonDeform0x13Command |= isinstance(
            highestChildNode.parent.node, DisplayListWithOffsetNode
        )
    ...
    if highestChildNode.parent is None:
        raise PluginError(
            'Issue with "' + boneName + '": Deform parent bone not found for skinning.'
        )
    ...
    if highestChildNode != transformNode:
        if not isFirstChild:
            ...
            while (
                highestChildIndex > 0
                and type(highestChildNode.parent.children[highestChildIndex - 1].node)
                is FunctionNode
            ):

                precedingFunctionCmds.insert(
                    0,
                    copy.deepcopy(
            ...
            while (
                nodeIndex > 0
                and type(highestChildNode.parent.children[nodeIndex - 1].node)
                is FunctionNode
            ):
                nodeIndex -= 1
...
def getAncestorGroups(parentGroup, vertexGroup, armatureObj, obj):
    ...
    return [
        getGroupIndexFromname(obj, bone.name)
        for bone in armatureObj.data.bones
        if bone not in ancestorBones
    ]
...
def saveModelGivenVertexGroup(
    ...
    vertIndices = [
        ...
        if getGroupIndex(vert, armatureObj, obj) == currentGroupIndex
    ]
    parentGroupIndex = (
        getGroupIndexFromname(obj, parentGroup) if parentGroup is not None else -1
    )
    ...
    for vertIndex in vertIndices:
        ...
        for face in infoDict.vert[vertIndex]:
            ...
            for i in range(3):
                vertGroupIndex = getGroupIndex(
                    mesh.vertices[face.vertices[i]], armatureObj, obj
                )
                if vertGroupIndex not in ancestorGroups:
                    ancestorGroups[vertGroupIndex] = getAncestorGroups(
                        parentGroup, vertexGroup, armatureObj, obj
                    )
    ...
    for drawLayer, materialFaces in groupFaces.items():
        ...
        for material_index, bFaces in materialFaces.items():
            ...
            fMaterial, texDimensions = saveOrGetF3DMaterial(
                material, fModel, obj, drawLayer, convertTextureData
            )
...
def saveOverrideDraw(
    ...
    fOverrideMat, texDimensions = saveOrGetF3DMaterial(
        material, fModel, obj, drawLayer, convertTextureData
    )
    overrideIndex = str(len(fMesh.drawMatOverrides))
    if (material, specificMat, overrideType) in fMesh.drawMatOverrides:
        overrideIndex = fMesh.drawMatOverrides[
            (material, specificMat, overrideType)
        ].name[-1]
    meshMatOverride = GfxList(
        fMesh.name + "_mat_override_" + toAlnum(material.name) + "_" + overrideIndex,
        GfxListTag.Draw,
        fModel.DLFormat,
    )
    ...
    for command in meshMatOverride.commands:
        if isinstance(command, SPDisplayList):
            for (modelMaterial, modelDrawLayer, modelAreaIndex), (
                ...
                shouldModify = (
                    overrideType == "Specific" and modelMaterial in specificMat
                ) or (overrideType == "All" and modelMaterial not in specificMat)
                if command.displayList == fMaterial.material and shouldModify:
                    ...
                    triCommand = meshMatOverride.commands[
                        meshMatOverride.commands.index(command) + 1
                    ]
    ...
    if fOverrideMat.revert is not None:
        for command in triCommands:
            meshMatOverride.commands.insert(
                meshMatOverride.commands.index(command) + 1,
...
def splitSkinnedFacesIntoTwoGroups(
...
def saveSkinnedMeshByMaterial(
    ...
    (
        ...
        loopDict,
        notInGroupBlenderVerts,
    ) = splitSkinnedFacesIntoTwoGroups(
        skinnedFaces, fModel, obj, uv_data, drawLayer, convertTextureData
    )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if type(obj.data) is not bpy.types.Mesh and not (
                obj.data is None
                and (obj.sm64_obj_type == "None" or obj.sm64_obj_type == "Switch")
            ):
                raise PluginError(
                    'Selected object must be a mesh or an empty with the "None" or "Switch" type.'
                )
        ...
        try:
            ...
            saveTextures = (
                ...
                or bpy.context.scene.ignoreTextureRestrictions
            )
            ...
            if context.scene.fast64.sm64.exportType == "C":
                ...
                self.report(
                    {"INFO"},
                    "Success! Data at " + context.scene.geoInsertableBinaryPath,
                )
            else:
                ...
                shutil.copy(
                    bpy.path.abspath(context.scene.exportRom), bpy.path.abspath(tempROM)
                )
                romfileExport.close()
                romfileOutput = open(bpy.path.abspath(tempROM), "rb+")

                levelParsed = parseLevelAtPointer(
                    romfileOutput, level_pointers[context.scene.levelGeoExport]
                )
                ...
                exportRange = [
                    int(context.scene.geoExportStart, 16),
                ...
                os.rename(
                    bpy.path.abspath(tempROM), bpy.path.abspath(context.scene.outputRom)
                )
...
class SM64_ExportGeolayoutArmature(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            if len(armatureObj.children) == 0 or not isinstance(
                armatureObj.children[0].data, bpy.types.Mesh
            ):
                raise PluginError(
                    "Armature does not have any mesh children, or "
            ...
            for linkedArmature in linkedArmatures:
                ...
                if len(optionObjs) > 1:
                    ...
                    raise PluginError(
                        "Error: " + linkedArmature.name + " has no mesh children."
                    )
...
class SM64_ExportGeolayoutPanel(SM64_Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.fast64.sm64.exportType == "C":
            if (
                not bpy.context.scene.ignoreTextureRestrictions
                and context.scene.saveTextures
            ):
            ...
            if context.scene.geoCustomExport:
                ...
                infoBox.label(
                    text="all other actors must also be replaced (with unique folder names)"
                )
            ...
            if context.scene.overwriteModelLoad:
                prop_split(
                    col, context.scene, "modelLoadLevelScriptCmd", "Model Load Command"
                )
...
def sm64_geo_writer_register():
    ...
    bpy.types.Scene.levelGeoExport = bpy.props.EnumProperty(
        items=level_enums, name="Level", default="HMC"
    )
    ...
    bpy.types.Scene.geoRAMAddr = bpy.props.StringProperty(
        name="RAM Address", default="80000000"
    )
    ...
    bpy.types.Scene.geoStructName = bpy.props.StringProperty(
        name="Geolayout Name", default="mario_geo"
    )
