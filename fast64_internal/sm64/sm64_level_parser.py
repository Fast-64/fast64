...
def parseLevel(romfile, startAddress, segmentData):
    ...
    romfile.seek(
        currentAddress
    )  # second seek is because reading moves read pointer forward
    ...
    while len(scriptStack) > 0 and currentCmd[0] is not L_END:
        ...
        if currentCmd[0] == L_JUMP:
            ...
            currentCmd[0] == L_LOAD_ROM_SEG
            or currentCmd[0] == L_LOAD_MIO0_SEG
            or currentCmd[0] == L_LOAD_MIO0_TEX
        ):
        ...
        if (
            currentCmd[0] != L_PUSH
            and currentCmd[0] != L_JUMP
            and currentCmd[0] != L_POP
        ):
            currentAddress += currentCmd[1]
        romfile.seek(currentAddress)
        currentCmd = romfile.read(2)
