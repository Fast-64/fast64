...
class ZoomOutMasks:
    ...
    def write(self, filepath):
        matchResult = re.search(
            "u8\s*sZoomOutAreaMasks\s*\[\]\s*=\s*\{" + "(((?!\}).)*)\}\s*;",
            self.originalData,
            re.DOTALL,
        )
        ...
        data = (
            self.originalData[: matchResult.start(1)]
            + self.to_c()
            + self.originalData[matchResult.end(1) :]
        )
...
class LevelScript:
    ...
    def get_persistent_block(self, retainType: str, nTabs=0, areaIndex: str = None):
        ...
        if areaIndex:
            lines.extend(
                self.persistentBlocks[PersistentBlocks.areaCommands].get(areaIndex, [])
            )
        else:
...
def parseCourseDefines(filepath):
    if not os.path.exists(filepath):
        raise PluginError(
            'Path "'
            + filepath
            + '" does not exist, could not read course defines file.'
        )
    ...
    if matchResult is None:
        raise PluginError(
            'Path "' + filepath + '" does not have any course define macros in it.'
        )
...
def parseLevelDefines(filepath):
    if not os.path.exists(filepath):
        raise PluginError(
            'Path "' + filepath + '" does not exist, could not read level defines file.'
        )
    scriptFile = open(filepath, "r", newline="\n")
    scriptData = scriptFile.read()
    scriptFile.close()
    ...
    if matchResult is None:
        raise PluginError(
            'Path "' + filepath + '" does not have any level define macros in it.'
        )
    headerInfo = scriptData[: matchResult.start(0)]
    defineMacros = stringToMacros(scriptData)
...
def parseZoomMasks(filepath):
    if not os.path.exists(filepath):
        raise PluginError(
            'Path "' + filepath + '" does not exist, could not read camera.c file.'
        )
...
def setStartLevel(basePath, levelEnum):
    ...
    newData = re.sub(
        "SET\_REG\((((?!\)).)*)\)", "SET_REG(" + levelEnum + ")", data, count=1
    )
    if newData != data:
...
def addActSelectorIgnore(basePath, levelEnum):
    ...
    checkResult = re.search(
        "if\s*\(gCurrLevelNum\s*==\s*" + levelEnum + "\)\s*return\s*0;", data, re.DOTALL
    )
    ...
    matchResultFunction = re.search(
        "s32\s*lvl\_set\_current\_level\s*\((((?!\)).)*)\)\s*\{" + "(((?!\}).)*)\}",
        data,
        re.DOTALL,
    )
    ...
    matchResult = re.search(
        "gCurrCourseNum\s*\=\s*gLevelToCourseNumTable(((?!\;).)*)\;",
        functionContents,
        re.DOTALL,
    )
    if matchResult is None:
        raise PluginError(
            'Could not find gCurrCourseNum setting in lvl_set_current_level in "'
            + filepath
            + '".'
        )
    ...
    newData = (
        data[: matchResultFunction.start(3)]
        + functionContents
        + data[matchResultFunction.end(3) :]
    )
...
def removeActSelectorIgnore(basePath, levelEnum):
    ...
    newData = re.sub(
        "if\s*\(gCurrLevelNum\s*\=\=\s*" + levelEnum + "\)\s*return\s*0\;\n",
        "",
        data,
        re.DOTALL,
    )
    ...
    for line in scriptData.splitlines():
        if curBlock and PersistentBlocks.endMagic in line:
            ...
            if endMatch and endMatch.group(1) != curBlock:
                raise PluginError(
                    f"script.c: Non-matching end block ({endMatch.group(1)}) found for {curBlock}"
                )
            curBlock = None
            areaIndex = None
            continue
...
def parseLevelScript(filepath, levelName):
    ...
    matchResult = re.search(
        "const\s*LevelScript\s*level\_\w*\_entry\[\]\s*=\s*\{" + "(((?!\}).)*)\}\s*;",
        scriptData,
        re.DOTALL,
    )
...
def overwritePuppycamData(filePath, levelToReplace, newPuppycamTriggers):
    ...
    arrayEntiresRegex = (
        "(.+struct\s+newcam_hardpos\s+newcam_fixedcam\[\]\s+=\s+{)(.+)(\};)"
    )
    ...
    if os.path.exists(filePath):
        ...
        if matchResult:
            ...
            raise PluginError(
                "Could not find 'struct newcam_hardpos newcam_fixedcam[]'."
            )
...
def exportLevelC(
    ...
    childAreas = [
        ...
        if child.data is None and child.sm64_obj_type == "Area Root"
    ]
    if len(childAreas) == 0:
        raise PluginError(
            "The level root has no child empties with the 'Area Root' object type."
        )
    ...
    for child in childAreas:
        ...
        if child.areaIndex in areaDict:
            raise PluginError(
                child.name
                + " shares the same area index as "
                + areaDict[child.areaIndex].name
            )
        ...
        if (
            child.areaIndex == 1
            or child.areaIndex == 2
            or child.areaIndex == 3
            or child.areaIndex == 4
        ):
        ...
        colFile.close()
        levelDataString += (
            '#include "levels/' + levelName + "/" + areaName + '/collision.inc.c"\n'
        )
        ...
        if child.enableRoomSwitch:
            ...
            roomFile.close()
            levelDataString += (
                '#include "levels/' + levelName + "/" + areaName + '/room.inc.c"\n'
            )
        ...
        areaString += area.to_c_script(
            child.enableRoomSwitch, persistentBlockString=persistentBlockString
        )
        ...
        macroFile.close()
        levelDataString += (
            '#include "levels/' + levelName + "/" + areaName + '/macro.inc.c"\n'
        )
        ...
        splineFile.close()
        levelDataString += (
            '#include "levels/' + levelName + "/" + areaName + '/spline.inc.c"\n'
        )
    ...
    replaceSegmentLoad(
        prevLevelScript,
        "_" + levelName + "_segment_7",
        "LOAD_" + compressionFmt.upper(),
        0x07,
    )
    if usesEnvFX:
        replaceSegmentLoad(
            prevLevelScript,
            "_effect_" + compressionFmt,
            "LOAD_" + compressionFmt.upper(),
            0x0B,
        )
    ...
    exportData = fModel.to_c(
        TextureExportSettings(savePNG, savePNG, "levels/" + levelName, levelDir),
        gfxFormatter,
    )
    ...
    if customExport:
        ...
        cameraFile = open(
            os.path.join(levelDir, "camera_trigger.inc.c"), "w", newline="\n"
        )
        ...
        if hasPuppyCamData:
            cameraFile = open(
                os.path.join(levelDir, "puppycam_trigger.inc.c"), "w", newline="\n"
            )
    ...
    if not customExport:
        ...
        puppycamAnglesPath = os.path.join(
            exportDir, "enhancements/puppycam_angles.inc.c"
        )
        if os.path.exists(puppycamAnglesPath) and puppycamVolumeString != "":
            overwritePuppycamData(
                puppycamAnglesPath, levelIDNames[levelName], puppycamVolumeString
            )
        ...
        if fModel.texturesSavedLastExport == 0:
            ...
            deleteIfFound(
                os.path.join(levelDir, "texture.inc.c"),
                '#include "levels/' + levelName + '/texture_include.inc.c"',
            )

        # This one is for backwards compatibility purposes
        deleteIfFound(
            levelDataPath, '#include "levels/' + levelName + '/texture_include.inc.c"'
        )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if obj.data is not None or obj.sm64_obj_type != "Level Root":
                raise PluginError(
                    "The selected object is not an empty with the Level Root type."
                )
        ...
        try:
            ...
            if context.scene.levelCustomExport:
                ...
                triggerName = "sCam" + context.scene.levelName.title().replace(
                    " ", ""
                ).replace("_", "")
            else:
    ...
    def draw(self, context):
        ...
        if context.scene.levelCustomExport:
            ...
            if context.scene.levelOption == "custom":
                ...
                box = col.box()
                box.label(
                    text="Adding levels may require modifying the save file format."
                )
...
def sm64_level_register():
    ...
    bpy.types.Scene.levelOption = bpy.props.EnumProperty(
        name="Level", items=enumLevelNames, default="bob"
    )
