...
enumEnvFX = [
    ...
    (
        ...
        "Jetstream Bubbles",
        "Used in JRB, DDD where jetstream is",
    ),
]
...
inlineGeoLayoutObjects = {
    ...
    "Geo Billboard": InlineGeolayoutObjConfig(
        "Geo Billboard", BillboardNode, can_have_dl=True, uses_location=True
    ),
    "Geo Scale": InlineGeolayoutObjConfig(
        "Geo Scale", ScaleNode, can_have_dl=True, uses_scale=True
    ),
    "Geo Displaylist": InlineGeolayoutObjConfig(
        "Geo Displaylist", DisplayListNode, must_have_dl=True
    ),
...
enumObjectType = [
    ...
    (
        "",
        "Inline Geolayout Commands",
        "",
    ),  # This displays as a column header for the next set of options
...
enumPuppycamFlags = [
    ...
    (
        "NC_FLAG_FOCUSX",
        "Use X Focus",
        "the camera will point towards its focus on the X axis.",
    ),
    (
        "NC_FLAG_FOCUSY",
        "Use Y Focus",
        "the camera will point towards its focus on the Y axis.",
    ),
    (
        "NC_FLAG_FOCUSZ",
        "Use Z Focus",
        "the camera will point towards its focus on the Z axis.",
    ),
    ...
    (
        "NC_FLAG_COLLISION",
        "Collision",
        "the camera will collide and correct itself with terrain.",
    ),
    (
...
class CollisionWaterBox:
    def __init__(self, waterBoxType, position, scale, emptyScale):
        ...
        self.low = (
            position[0] - scale[0] * emptyScale,
            position[2] - scale[1] * emptyScale,
        )
        self.high = (
            position[0] + scale[0] * emptyScale,
            position[2] + scale[1] * emptyScale,
...
class PuppycamVolume:
    def __init__(
        ...
        if camPos != (32767, 32767, 32767):
            self.camPos = (
                camPos[0] * camScaleValue,
                camPos[2] * camScaleValue,
                camPos[1] * camScaleValue * -1,
            )
        else:
            self.camPos = camPos
        ...
        if camFocus != (32767, 32767, 32767):
            self.camFocus = (
                camFocus[0] * camScaleValue,
                camFocus[2] * camScaleValue,
                camFocus[1] * camScaleValue * -1,
            )
        else:
...
def process_sm64_objects(obj, area, rootMatrix, transformMatrix, specialsOnly):
    translation, originalRotation, scale = (
        transformMatrix @ rootMatrix.inverted() @ obj.matrix_world
    ).decompose()
    ...
    if obj.data is None:
        ...
        if specialsOnly:
            if obj.sm64_obj_type == "Special":
                preset = (
                    obj.sm64_special_enum
                    if obj.sm64_special_enum != "Custom"
                    else obj.sm64_obj_preset
                )
                ...
                area.water_boxes.append(
                    CollisionWaterBox(
                        obj.waterBoxType, translation, scale, obj.empty_display_size
                    )
                )
        else:
            if obj.sm64_obj_type == "Object":
                ...
                mario_start = SM64_Mario_Start(
                    obj.sm64_obj_mario_start_area, translation, rotation.to_euler()
                )
                ...
                area.objects.append(
                    SM64_Whirpool(
                        obj.whirlpool_index,
                        obj.whirpool_condition,
                        obj.whirpool_strength,
                        translation,
                    )
                )
                ...
                if (
                    not puppycamProp.puppycamUseEmptiesForPos
                ) and puppycamProp.puppycamCamera is not None:
                    puppycamCamPosCoords = puppycamProp.puppycamCamera.location
                elif (
                    puppycamProp.puppycamUseEmptiesForPos
                    and puppycamProp.puppycamCamPos != ""
                ):
                    puppycamPosObject = bpy.context.scene.objects[
                        puppycamProp.puppycamCamPos
                    ]
                ...
                if (
                    ...
                    puppycamCamFocusCoords = (
                        puppycamProp.puppycamCamera.matrix_local
                        @ mathutils.Vector((0, 0, -1))
                    )[:]
                elif (
                    puppycamProp.puppycamUseEmptiesForPos
                    and puppycamProp.puppycamCamFocus != ""
                ):
                    puppycamFocObject = bpy.context.scene.objects[
        ...
        area.splines.append(
            convertSplineObject(area.name + "_spline_" + obj.name, obj, finalTransform)
        )
...
class SM64ObjectPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        return context.scene.gameEditorMode == "SM64" and (
            context.object is not None and context.object.data is None
        )

    def draw_inline_obj(self, box: bpy.types.UILayout, obj: bpy.types.Object):
        obj_details: InlineGeolayoutObjConfig = inlineGeoLayoutObjects.get(
            obj.sm64_obj_type
        )
        ...
        if obj_details.uses_scale and not obj_scale_is_unified(obj):
            warnings.add(
                "Object's scale must all be the same exact value (e.g. 2, 2, 2)"
            )
        ...
        if not obj_details.uses_rotation and not all_values_equal_x(
        ...
        if len(warnings):
            ...
            warning_box.label(
                text="Warning: Unexpected export results from these issues:",
                icon="ERROR",
            )
            for warning in warnings:
                warning_box.label(text=warning, icon="ERROR")
            warning_box.label(
                text=f'Relative location: {", ".join([str(l) for l in loc])}'
            )
        ...
        if obj_details.can_have_dl:
            ...
            if obj_details.must_have_dl or obj.useDLReference:
                ...
                prop_split(
                    box, obj, "dlReference", "Displaylist variable or hex address"
                )
        ...
        if (
            obj_details.uses_rotation
            or obj_details.uses_location
            or obj_details.uses_scale
        ):
            info_box = box.box()
        ...
        if len(obj.children):
            if checkIsSM64PreInlineGeoLayout(obj.sm64_obj_type):
                box.box().label(
                    text="Children of this object will just be the following geo commands."
                )
            else:
                box.box().label(
                    text="Children of this object will be wrapped in GEO_OPEN_NODE and GEO_CLOSE_NODE."
                )

    def draw_behavior_params(
        self, obj: bpy.types.Object, parent_box: bpy.types.UILayout
    ):
        ...
        box.prop(
            game_object, "use_individual_params", text="Use Individual Behavior Params"
        )
    ...
    def draw(self, context):
        ...
        if obj.sm64_obj_type == "Object":
            ...
            box.box().label(
                text="Macro presets defined in include/macro_preset_names.h."
            )
            ...
            box.box().label(
                text="Special presets defined in include/special_preset_names.h."
            )
            ...
            box.box().label(
                text="Water box area defined by top face of box shaped empty."
            )
            ...
            if obj.useBackgroundColor:
                ...
                if obj.background == "CUSTOM":
                    ...
                    prop_split(
                        box, levelObj, "backgroundSegment", "Custom Background Segment"
                    )
            ...
            camBox = box.box()
            camBox.label(
                text="Warning: Camera modes can be overriden by area specific camera code."
            )
            camBox.label(
                text="Check the switch statment in camera_course_processing() in src/game/camera.c."
            )
            ...
            if obj.showStartDialog:
                ...
                dialogBox.label(
                    text="See load_level_init_text() in src/game/level_update.c for conditions."
                )
            ...
            if obj.enableRoomSwitch:
                ...
                infoBox.label(
                    text="Children will ordered alphabetically, with the first child being always visible."
                )
            ...
            puppycamProp = obj.puppycamProp
            prop_split(
                column, puppycamProp, "puppycamVolumeFunction", "Puppycam Function"
            )
            ...
            if puppycamProp.puppycamUseEmptiesForPos:
                ...
                column.prop_search(
                    puppycamProp, "puppycamCamPos", bpy.data, "objects", text=""
                )

                column.label(text="Fixed Camera Focus (Optional)")
                column.prop_search(
                    puppycamProp, "puppycamCamFocus", bpy.data, "objects", text=""
                )
            else:
                column.label(text="Fixed Camera Position (Optional)")
                ...
                if puppycamProp.puppycamCamera is not None:
                    column.box().label(
                        text="FOV not exported, only for preview camera."
                    )
                    prop_split(column, puppycamProp, "puppycamFOV", "Camera FOV")
                    column.operator(
                        "mesh.puppycam_setup_camera",
                        text="Setup Camera",
                        icon="VIEW_CAMERA",
                    )
            ...
            box.box().label(
                text="This can be used as an empty transform node in a geolayout hierarchy."
            )
...
class WarpNodeProperty(bpy.types.PropertyGroup):
    warpType: bpy.props.EnumProperty(
        name="Warp Type", items=enumWarpType, default="Warp"
    )
    warpID: bpy.props.StringProperty(name="Warp ID", default="0x0A")
    destLevelEnum: bpy.props.EnumProperty(
        name="Destination Level", default="bob", items=enumLevelNames
    )
    destLevel: bpy.props.StringProperty(
        name="Destination Level Value", default="LEVEL_BOB"
    )
    ...
    warpFlagEnum: bpy.props.EnumProperty(
        name="Warp Flags Value", default="WARP_NO_CHECKPOINT", items=enumWarpFlag
    )
    instantOffset: bpy.props.IntVectorProperty(name="Offset", size=3, default=(0, 0, 0))
    instantWarpObject1: bpy.props.PointerProperty(
        name="Object 1", type=bpy.types.Object
    )
    instantWarpObject2: bpy.props.PointerProperty(
    ...
    def calc_offsets_from_objects(self, reverse=False):
        if self.instantWarpObject1 is None or self.instantWarpObject2 is None:
            raise PluginError(
                f"Warp Start and Warp End in Warp Node {self.warpID} must have objects selected."
            )
    ...
    if warpNode.expand:
        ...
        if warpNode.warpType == "Instant":
            ...
            if warpNode.useOffsetObjects:
                ...
                writeBox = box.box()
                if (
                    warpNode.instantWarpObject1 is None
                    or warpNode.instantWarpObject2 is None
                ):
                    ...
                    if usesAreaNodes:
                        writeBox.label(
                            text="(When using two area nodes, the calculation is reversed)"
                        )
            else:
...
class StarGetCutscenesProperty(bpy.types.PropertyGroup):
    star1_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="1"
    )
    star2_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="2"
    )
    star3_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="3"
    )
    star4_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="4"
    )
    star5_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="5"
    )
    star6_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="6"
    )
    star7_option: bpy.props.EnumProperty(
        items=enumStarGetCutscene, default="4", name="7"
    )
    ...
    def value(self):
        ...
        value += (
            ...
            else format(self.star1_value, "X")
        )
        value += (
            self.star2_option
            if self.star2_option != "Custom"
            else format(self.star2_value, "X")
        )
        value += (
            self.star3_option
            if self.star3_option != "Custom"
            else format(self.star3_value, "X")
        )
        value += (
            self.star4_option
            if self.star4_option != "Custom"
            else format(self.star4_value, "X")
        )
        value += (
            self.star5_option
            if self.star5_option != "Custom"
            else format(self.star5_value, "X")
        )
        value += (
            self.star6_option
            if self.star6_option != "Custom"
            else format(self.star6_value, "X")
        )
        value += (
            self.star7_option
            if self.star7_option != "Custom"
            else format(self.star7_value, "X")
        )
        value += "0"
        return value
...
def onUpdateObjectType(self, context):
    ...
    isBoxEmpty = (
        self.sm64_obj_type == "Water Box" or self.sm64_obj_type == "Camera Volume"
    )
...
class PuppycamSetupCamera(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        scene.safe_areas.title[1] = (
            8 / 240
        )  # Use the safe areas to denote where default 8 pixel black bars will be
...
class PuppycamProperty(bpy.types.PropertyGroup):
    ...
    puppycamCamera: bpy.props.PointerProperty(
        type=bpy.types.Object, poll=sm64_is_camera_poll
    )
...
class SM64_GeoASMProperties(bpy.types.PropertyGroup):
    ...
    param: bpy.props.StringProperty(
        name="Geo ASM Param",
        default="0",
        description="Function parameter. (Binary exporting will cast to int)",
    )
...
class SM64_GameObjectProperties(bpy.types.PropertyGroup):
    ...
    bparams: bpy.props.StringProperty(
        name="Behavior Parameters",
        description="All Behavior Parameters",
        default=DEFAULT_BEHAVIOR_PARAMS,
    )

    use_individual_params: bpy.props.BoolProperty(
        name="Use Individual Behavior Params",
        description="Use Individual Behavior Params",
        default=True,
    )
    bparam1: bpy.props.StringProperty(
        name="Behavior Param 1", description="First Behavior Param", default=""
    )
    bparam2: bpy.props.StringProperty(
        name="Behavior Param 2", description="Second Behavior Param", default=""
    )
    bparam3: bpy.props.StringProperty(
        name="Behavior Param 3", description="Third Behavior Param", default=""
    )
    bparam4: bpy.props.StringProperty(
        name="Behavior Param 4", description="Fourth Behavior Param", default=""
    )
...
def sm64_obj_register():
    ...
    bpy.types.Object.sm64_obj_type = bpy.props.EnumProperty(
        name="SM64 Object Type",
        items=enumObjectType,
        default="None",
        update=onUpdateObjectType,
    )
    ...
    bpy.types.Object.startDialog = bpy.props.StringProperty(
        name="Start Dialog", default="DIALOG_000"
    )
    bpy.types.Object.actSelectorIgnore = bpy.props.BoolProperty(
        name="Skip Act Selector"
    )
    ...
    bpy.types.Object.useDLReference = bpy.props.BoolProperty(
        name="Use displaylist reference"
    )
    bpy.types.Object.dlReference = bpy.props.StringProperty(
        name="Displaylist variable name or hex address for binary."
    )

    bpy.types.Object.geoReference = bpy.props.StringProperty(
        name="Geolayout variable name or hex address for binary"
    )

    bpy.types.Object.customGeoCommand = bpy.props.StringProperty(
        name="Geolayout macro command", default=""
    )
    bpy.types.Object.customGeoCommandArgs = bpy.props.StringProperty(
        name="Geolayout macro arguments", default=""
    )

    bpy.types.Object.enableRoomSwitch = bpy.props.BoolProperty(
        name="Enable Room System"
    )
