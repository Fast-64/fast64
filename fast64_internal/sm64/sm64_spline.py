...
class SM64SplinePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if curve.splines[0].type != "NURBS":
            ...
            if (
                curve.sm64_spline_type == "Cutscene"
                or curve.sm64_spline_type == "Vector"
            ):
                pointIndex = 0
...
def assertCurveValid(obj):
    ...
    if len(curve.splines) != 1:
        ...
        raise PluginError(
            "Exported curves should have only one single segment, found "
            + str(len(curve.splines))
        )
    return True
