...
def readSegmentInfo(baseDir):
    ...
    for matchResult in re.finditer(
        ...
        segDict[matchResult.group(7).strip()] = (
            "_" + matchResult.group(7) + "_segment_7SegmentRomStart",
            7,
            None,
        )
    return segDict
...
def writeSegmentROMTable(baseDir):
    ...
    writeIfNotFound(
        os.path.join(baseDir, "src/game/memory.h"),
...
def writeScrollTextureCall(path, include, callString):
    ...
    if include not in data:
        ...
        if callScrollIndex != -1:
            ...
            data = (
                data[:callScrollIndex] + " scroll_textures();" + data[callScrollIndex:]
            )
        else:
...
def writeTexScrollBase(baseDir):
    ...
    if "#define SCROLL_CONDITION" not in scrollData:
        if macroIndex != -1:
            ...
            raise PluginError(
                'Cannot find \'#include "texscroll.h" in src/game/texscroll.c'
            )
    ...
    writeScrollTextureCall(
        os.path.join(baseDir, "src/game/level_update.c"),
        '#include "texscroll.h"',
    ...
    if os.path.exists(starSelectPath):
        writeScrollTextureCall(
            starSelectPath, '#include "src/game/texscroll.h"', "area_update_objects();"
        )
...
def createTexScrollHeadersGroup(exportDir, groupName, dataInclude):
    ...
    if matchResult:
        ...
        texscrollDataC = (
            texscrollDataC[: matchResult.start(1)]
            + functionCalls
...
def modifyTexScrollHeadersGroup(
    exportDir, includeC, includeH, groupName, scrollDefines, dataInclude, hasScrolling
):
    if not bpy.context.scene.disableScroll and hasScrolling:
        fileStatus = writeTexScrollHeadersGroup(
            exportDir, includeC, includeH, groupName, scrollDefines, dataInclude
        )
        return fileStatus
    else:
        removeTexScrollHeadersGroup(
            exportDir, includeC, includeH, groupName, scrollDefines, dataInclude
        )
        return None
...
def writeTexScrollHeadersGroup(
    ...
    groupPathH = os.path.join(
        exportDir, "src/game/texscroll/" + groupName + "_texscroll.inc.h"
    )
    ...
    groupPathC = os.path.join(
        exportDir, "src/game/texscroll/" + groupName + "_texscroll.inc.c"
    )
    ...
    if includeIndex != -1:
        if includeC not in groupDataC:
            groupDataC = (
                groupDataC[:includeIndex] + includeC + "\n" + groupDataC[includeIndex:]
            )
    else:
    ...
    if matchResult:
        ...
        groupDataC = (
            groupDataC[: matchResult.start(1)]
            + functionCalls
...
def removeTexScrollHeadersGroup(
    exportDir, includeC, includeH, groupName, scrollDefines, dataInclude
):
